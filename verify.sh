#!/usr/bin/env bash
# verify for: bg-dashboard-s2-identity-view-and-update
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('broker_guard/webui.py').read())"; then
  echo "  ok: broker_guard/webui.py parses"
else
  echo "  FAIL: broker_guard/webui.py does not parse"; fails=$((fails+1))
fi
