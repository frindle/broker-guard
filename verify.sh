#!/usr/bin/env bash
# verify for: bg-webui-s3-escalation-audit
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('broker_guard/webui_data.py').read())"; then
  echo "  ok: broker_guard/webui_data.py parses"
else
  echo "  FAIL: broker_guard/webui_data.py does not parse"; fails=$((fails+1))
fi
