...
Only edit `broker_guard/webui.py`; do not edit `verify.sh`, `test_fixture.py` or
`TASK.md`. requirements.txt already declares `fastapi` and `httpx` (added ahead
of this slice) so no dependency file needs touching.
