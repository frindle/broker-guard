...
TARGET = pathlib.Path('broker_guard/webui.py')

# frozen from TASK.md -- re-run `ollama-dispatch-scaffold --freeze-literals .`
LITERALS = [[None, "PROFILE_PATH = 'profile.json'"], [None, '@app.get("/identity", response_class=HTMLResponse)'], [None, 'def identity_view():'], [None, 'profile.load_profile(PROFILE_PATH)'], [None, '@app.post("/identity")'], [None, 'def identity_update('], [None, 'first_name: str = Form("")'], [None, 'RedirectResponse(url="/identity", status_code=303)']]
