...
- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.OPTOUT_BLOCKED`
  ...
  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle, its own single-target prober invocation because the URL contains optout/opt-out/remove) 2026-09-25, batch 40. DATASET DEFECT FIRST: the recorded opt_out_url https://www.unmask.com/optout is DEAD -- it 200-renders as the site's own 404 page, title '404 - This page could not be found', body '404 / This page could not be found / Please continue your search in the form below', i.e. it hands the visitor a search box where the removal form should be. Flagged and not fixed. The real rights page is the one link that 404 page offers, 'DO NOT SELL OR SHARE MY PERSONAL INFORMATION' -> https://unmask.com/privacy-rights/, and THAT is walled: rendering it lands on /privacy-rights/?__cf_chl_rt_tk=... with HTTP 403, title 'Just a moment...', the body 'Performing security verification / This website uses a security service to protect against malicious bots', a Cloudflare Turnstile script (challenges.cloudflare.com/turnstile/v0/b/d76008a69eab/api.js) and a single hidden input cf-turnstile-response#cf-chl- widget-3ncd0_response. A managed challenge, not a form. NOTE THE ODDITY for whoever revisits: the homepage is behind RADWARE and the rights page behind CLOUDFLARE, so two vendors guard one site and neither verdict can be read off the other. Recorded contact for a human: operations@unmask.com.

### `usa-official-com`

- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.NO_OPTOUT_SURFACE`

  > HOST UNRESOLVABLE -- and this is a LAME DELEGATION, not one of this machine's eight DNS blackholes, established by querying the authoritative servers directly on 2026-09-25. dig @1.1.1.1 usa- official.com returns SERVFAIL, and the resolver says why in an EDNS Extended DNS Error: 'at delegation usa-official.com.'. Asked upward, a.gtld-servers.net still delegates the zone to kurt.ns.cloudflare.com and hazel.ns.cloudflare.com -- and both of those nameservers answer REFUSED for it, which is what Cloudflare returns for a zone no longer in any account. So the .com delegation outlived the hosting: the domain was dropped from its Cloudflare account and nobody repointed it. dig +cd (DNSSEC validation disabled) returns nothing either, and curl cannot resolve the name. There is therefore no host to serve anything, and no opt-out surface can exist. The dataset records no opt_out_email for this row and no operating company, only the unresolvable https://usa-official.com/optout, so there is not even an address to fall back to. DATASET DEFECT: dead domain, no contact, company name inferred from the domain itself; flagged and not fixed. Distinguish this from a paused-but-live site (Vercel 402, HTTP 500, cert mismatch) and from a local blackhole: the authoritative nameservers themselves disown the zone.

- **scope:** dataset | **kind:** unclassified | **from:** `search_forms.NO_SEARCH_SURFACE`

  > HOST UNRESOLVABLE -- and this is a LAME DELEGATION, not one of this machine's eight DNS blackholes, established by querying the authoritative servers directly on 2026-09-25. dig @1.1.1.1 usa- official.com returns SERVFAIL, and the resolver says why in an EDNS Extended DNS Error: 'at delegation usa-official.com.'. Asked upward, a.gtld-servers.net still delegates the zone to kurt.ns.cloudflare.com and hazel.ns.cloudflare.com -- and both of those nameservers answer REFUSED for it, which is what Cloudflare returns for a zone no longer in any account. So the .com delegation outlived the hosting: the domain was dropped from its Cloudflare account and nobody repointed it. dig +cd (DNSSEC validation disabled) returns nothing either, and curl cannot resolve the name. There is therefore no host to serve anything, and no search surface can exist. The row was already thin -- the dataset says the company name was 'inferred from domain' and the opt-out URL was 'discovered' -- so there is no operating company to look for elsewhere. DATASET DEFECT: a row whose only identifier is a domain that no longer resolves; flagged and not fixed.

### `usa-people-search-com`

- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.OPTOUT_OUT_OF_SCOPE`

  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle) 2026-09-25, batch 40. A FOUR-STEP FLOW WHOSE FIRST STEP IS AN EMAILED LINK, behind a visible reCAPTCHA Enterprise checkbox -- both blockers at once, and the dataset's URL is stale too. The recorded https://www.usa-people-search.com/manage 302s to /removal (DATASET DEFECT, minor: stale path; flagged and not fixed), title 'USA People Search - Opt out process - Initial form'. That page serves form#verifyEmailForm, POST to /removal, with a hidden Laravel _token, select#user-type ('The subject of the request' / 'An authorized agent of the subject'), subject- firstname (required), subject-middlename, subject-lastname (required), subject-email (required), three off-layout agent-* fields that the select reveals, a REQUIRED checkbox#agreement ('I authorize USA People Search to send communications regarding...') and button#BRP 'Begin Removal Process'. The page numbers its own steps: 'Enter your email address and name and complete the captcha below. We will send a link to your email address that will take you to the opt-out form.' -> 'Click the link sent to your email... If you wait more than 24 hours to click this link you will need to request a new one.' -> 'Enter your information on the form.' -> a confirmation page. So the actual removal form is not at a URL at all, which is the identical shape already recorded for fastpeoplesearch-com, and it is filed the same way. The captcha is a SECOND independent blocker: cap[] carries google.com/recaptcha/enterprise.js and the child frame is recaptcha/enterprise/anchor at size=normal with a live .recaptcha-checkbox, i.e. reCAPTCHA Enterprise v2 with a tickbox. Nothing was submitted. Another instance of OPEN POLICY QUESTION 1, filed consistently with fastpeoplesearch-com and peopleconnect-us and NOT resolved here. The dataset's note that this is an independent flow rather than the PeopleConnect suppression center is CONFIRMED: different host, different form, no redirect off-domain.

### `usrecords-net`

- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.OPTOUT_BLOCKED`

  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle) 2026-09-25, batch 40. A real, complete, self-hosted rights form behind a CUSTOM TEXT CAPTCHA. https://www.usrecords.net/request-portal answers HTTP 200 ('Request Portal | US Records') and serves form#wpforms- form-120184, a WPForms (WordPress) form posting to the page, with: a REQUIRED checkbox group wpforms[fields][3][] offering 'I am submitting this request for myself.' and 'I am an authorized agent submitting this request on behalf of another' as TWO VALUES ON ONE NAME (the filler trap this repo has met before -- filling everything asserts both at once); a request-type checkbox group wpforms[fields][4][] (Access / Delete / Correct / Opt-out of the sale); name wpforms[fields][2][first] and [last]; wpforms[fields][5][primary] plus [secondary] (a type-it-twice email confirmation, in-page, not by mail); a full address block wpforms[fields][6][address1|address2|city|state|postal] with a 52-option state select; a free-text wpforms[fields][7]; and the wall: input[name='wpforms[fields][8][a]'] labelled 'Custom Captcha *', REQUIRED, paired with a hidden [8][q] holding the question. That is a question a human must read and answer, so the form cannot be driven -- blocked, even though cap[] and widget[] are empty (there is no reCAPTCHA/Turnstile here at all; the check is WPForms' own). TWO HONEYPOTS, both to be declined if this is ever revisited: input#alt_s[name=alt_s] labelled 'Alternative:' and an off-layout input[name=nadmob8354] with a per-install random name. NEW SHARED-OPERATOR FINDING, and it corrects this row's dataset note ('no shared-operator evidence linking it to the other sites in this batch'): uswarrants.org serves the SAME WPForms privacy-request portal -- same field numbering 2/3/4/5/6/7/8, same labels word for word, same alt_s + random-name honeypot pair -- so the two are one operator or one vendor template. See uswarrants-org, where the only difference is an additional Cloudflare Turnstile.

### `uswarrants-org`

- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.OPTOUT_BLOCKED`

  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle) 2026-09-25, batch 40. TWO bot checks on one form, and a confirmed shared platform. https://www.uswarrants.org/request- portal-2/ answers HTTP 200 ('Privacy Request Portal | Us Warrants') and serves form#wpforms-form-816, which is the SAME WPForms privacy-request portal template as usrecords.net -- identical field numbering and identical labels: required checkbox group wpforms[fields][3][] ('I am submitting this request for myself.' / 'I am an authorized agent...' as two values on one name, the filler trap), request types [4][] (Access / Delete / Correct / Opt-out of the sale), name [2][first]+[last], email [5][primary]+[secondary] (type-it- twice), address block [6][address1..postal] with a 52-option state select, free text [7], and the captcha pair input[name='wpforms[fields][8][a]'] labelled 'Captcha *' (REQUIRED) with its hidden question in [8][q]. The same two honeypots are present: input#alt_s 'Alternative:' and an off- layout input[name=pqmhfn3422] whose random name differs from usrecords.net's nadmob8354, i.e. per-install. WHAT DIFFERS from usrecords.net: this tenant adds a Cloudflare Turnstile as well -- cap[] carries challenges.cloudflare.com/turnstile/v0/api.js?render=explicit, widget[] carries .cf-turnstile, and a hidden cf-turnstile- response#cf-chl-widget-vua70_response sits inside the form. Blocked on either check alone. DATASET NOTE, partly corrected: this row's note associates the site with InfoTracer/InfoPay 'unconfirmed' -- this pass found no InfoPay markers here at all (InfoPay's shared form is recognisable by InfoPay_Core_Components_OptOuts_DataRemovalServiceModel field names, which appear on verifyrecords-com this batch but NOT here). What IS confirmed is a shared template with usrecords.net; see that entry.

### `veeva-com`

- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.OPTOUT_BLOCKED`

  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle) 2026-09-25, batch 40. THE RECORDED PAGE IS AN EXPLAINER; THE REAL FORM IS A ONETRUST TENANT WITH A BOTDETECT CAPTCHA ON IT. https://www.veeva.com/privacy/do-not-sell-share/ answers HTTP 200 but carries no rights fields at all -- its two forms are site search -- and its own copy points onward: 'Exercise Your Rights' -> https://www.veeva.com/privacy-requests (DATASET DEFECT, minor: the recorded opt_out_url is the notice, not the form; flagged and not fixed). Rendering that URL lands on https: //privacyportal.onetrust.com/webform/3d676ed2-16b1-4c48-97f8- a911923a3adf/a0df0a98-d990-40ff-9bdb-35b5f9e06620, HTTP 200, title 'Privacy Web Form' -- a OneTrust DSAR webform (form.dsar- webform, Angular, 202608.1.0). Transcribed while open: input#countryDSARElement 'Country of Residence' (required, autocomplete), input#stateDSARElement 'State of Residence' (required), input#firstNameDSARElement (required), input#formField80DSARElement 'Middle Name', input#lastNameDSARElement (required), input#emailDSARElement (required), and button#dsar-webform-submit-button 'Submit Request'. THE WALL: input#captchaCode labelled 'Captcha' alongside the four hidden BDC_* fields (BDC_VCID_angularBasicCaptcha, BDC_BackWorkaround_, BDC_Hs_, BDC_SP_) -- that is BOTDETECT, an image-code captcha a human must read, and it is the seventeenth BotDetect instance this sweep has recorded. Note the per-tenant nature of OneTrust: this sitekey-and-captcha posture belongs to Veeva's tenant and says nothing about any other OneTrust row. The page also states Veeva does not offer a limit-sensitive-information option, and an appeal route exists. Human contact: privacy@veeva.com, plus a toll-free line on the page.

### `vendelux-com`

- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.NO_OPTOUT_SURFACE`

  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle) 2026-09-25, batch 40. EMAIL-ONLY, and the recorded opt_out_url is simply the privacy policy. https://vendelux.com/privacy 200-renders 'Privacy Policy | Vendelux' (effective 2026-05-04) with ZERO <form> elements and zero inputs -- the only controls are navigation and footer- accordion buttons, cap[] and widget[] empty, no frames. It enumerates state privacy rights in prose behind a 'click here to learn more' anchor and offers, as its only actionable opt-out links, the INDUSTRY self-regulatory pages optout.aboutads.info and optout.networkadvertising.org -- which are browser-cookie opt-outs at third parties, not a removal of Vendelux's own records, and must not be mistaken for this broker's surface. So the only channel is the recorded privacy@vendelux.com. DATASET NOTE: this row's opt_out_method is recorded as 'web-form', which this render contradicts; flagged and not fixed.

### `venpath-net`

- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.NO_OPTOUT_SURFACE`

  > THE WEBSITE IS GONE BUT THE MAILBOX IS NOT, and the two halves were checked separately on 2026-09-25. venpath.net's zone is live and hosted at Cloudflare (NS desi/dimitris.ns.cloudflare.com, SOA serial 2414475180) and the query returns NOERROR -- so this is not NXDOMAIN and not one of this machine's DNS blackholes -- but there is NO A record at the apex and none at www either, verified against dig @1.1.1.1 and 8.8.8.8, and curl cannot resolve www.venpath.net. What the zone DOES still carry is a full Google Workspace MX set (aspmx.l.google.com and the four alt hosts), so mail to the domain is still accepted. So the recorded opt_out_url https://www.venpath.net/legal/opt-out cannot be loaded at all -- no address to connect to, which is a distinct failure from a 403 wall, an HTTP 500 or a paused-but-live site. Recorded as no web surface, with the honest qualification that the live MX records mean the dataset's accounting@venpath.net may still deliver; that is an email channel this repo cannot use, not a form. DATASET DEFECTS, flagged and not fixed: a dead opt_out_url, and a contact that is an ACCOUNTING address rather than a privacy one, at a company whose site has been taken down. VenPath was the subject of an FTC Privacy Shield enforcement action, which fits a wound-down operation.

- **scope:** dataset | **kind:** entity-mismatch | **from:** `search_forms.NO_SEARCH_SURFACE`

  > THE WEBSITE IS GONE BUT THE MAILBOX IS NOT, and the two halves were checked separately on 2026-09-25. venpath.net's zone is live and hosted at Cloudflare (NS desi/dimitris.ns.cloudflare.com, SOA serial 2414475180) and the query returns NOERROR -- so this is not NXDOMAIN and not one of this machine's DNS blackholes -- but there is NO A record at the apex and none at www either, verified against dig @1.1.1.1 and 8.8.8.8, and curl cannot resolve www.venpath.net. What the zone DOES still carry is a full Google Workspace MX set (aspmx.l.google.com and the four alt hosts), so mail to the domain is still accepted. No web surface therefore exists to search, and none would: VenPath brokered mobile app and location data keyed to device identifiers, with no consumer lookup. DATASET DEFECT: the recorded opt_out_url https://www.venpath.net/legal/opt-out points at a hostname with no address record; flagged and not fixed.

### `veraset-com`

- **scope:** dataset | **kind:** dead-url | **from:** `optout_forms.OPTOUT_OUT_OF_SCOPE`

  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle) 2026-09-25, batch 40. DATASET DEFECT FIRST: the recorded opt_out_url https://www.veraset.com/do-not-sell-my-personal- information answers HTTP 404 ('Page Not Found / The page you are looking for doesn't exist or has been moved'). Flagged and not fixed. The live surface is the footer's 'Do-Not-Sell Request' link, https://www.veraset.com/legal/do-not-sell-request (HTTP 200, title 'Do Not Sell Request'), which serves form#wf-form- Privacy-Form (Webflow): input[name=Name] (required), input[name=Last-Name] (required), input[name=Email] (required), input[name=Advertising-ID] REQUIRED, and six request-type checkboxes -- Opt-Out, Deletion, Right-to-access, Right-to-know, Correction, Limited-use-of-sensitive-information. FILED OUT OF SCOPE for the same reason as venntel-com in this batch: the required field is a Mobile Advertising ID, which no profile in this repo holds and which cannot honestly be fabricated. Two further blockers are recorded rather than relied on: the site loads Cloudflare Turnstile (challenges.cloudflare.com/turnstile/v0/api.js in cap[], no widget element at rest, so an invisible or deferred check may fire on submit), and this row's dataset note already records an auto-reply that sends the requester to an identity-verification portal before anything is processed -- so even a completed submission starts a human exchange. Nothing was submitted.

### `verecor-com`

- **scope:** dataset | **kind:** unclassified | **from:** `optout_forms.NO_OPTOUT_SURFACE`

  > DOMAIN SEIZED BY COURT ORDER -- the fourth instance of this in the dataset and the reason this row's 'DEFUNCT? NXDOMAIN 2026-07-09' note is now wrong. verecor.com resolves again (Cloudflare, 104.21.57.41) and https://verecor.com/optout answers HTTP 200 with the title 'This Domain Has Been Transferred by Court Order -- Verecor.com' and a notice reading: 'Atlas Data Privacy Corporation, et al. v. Radaris.com, et al., Superior Court of New Jersey, Law Division, Middlesex County, Docket No. MID-L-000847-24... Pursuant to a final judgment of the New Jersey Superior Court, the domain name verecor.com has been transferred to Atlas Data Privacy Corporation and is no longer under the control of its former operators', with the Daniel's Law background (suit filed 2024-02-08, amended complaint 2025-05-27, claims assigned by roughly 21,760 covered persons). ZERO forms, zero inputs, cap[] and widget[] empty: the page is a notice and nothing else. So the recorded opt_out_url https://verecor.com/optout is now that notice page and there is nothing to opt out of: the records are no longer served and the domain is controlled by the plaintiffs in a Daniel's Law action, not by a broker who could act on a request. This is the same terminal state already recorded for radaris-com, rehold-com and peekyou-com, and is recorded as no surface rather than as dead- host, because the host is emphatically alive and saying something. Anyone with a live grievance about historical Verecor data belongs in the docket, not in a webform. DATASET DEFECT: the row's own notes still describe it as an NXDOMAIN clone site; flagged and not fixed.

- **scope:** dataset | **kind:** unclassified | **from:** `search_forms.NO_SEARCH_SURFACE`

  > DOMAIN SEIZED BY COURT ORDER -- the fourth instance of this in the dataset and the reason this row's 'DEFUNCT? NXDOMAIN 2026-07-09' note is now wrong. verecor.com resolves again (Cloudflare, 104.21.57.41) and https://verecor.com/optout answers HTTP 200 with the title 'This Domain Has Been Transferred by Court Order -- Verecor.com' and a notice reading: 'Atlas Data Privacy Corporation, et al. v. Radaris.com, et al., Superior Court of New Jersey, Law Division, Middlesex County, Docket No. MID-L-000847-24... Pursuant to a final judgment of the New Jersey Superior Court, the domain name verecor.com has been transferred to Atlas Data Privacy Corporation and is no longer under the control of its former operators', with the Daniel's Law background (suit filed 2024-02-08, amended complaint 2025-05-27, claims assigned by roughly 21,760 covered persons). ZERO forms, zero inputs, cap[] and widget[] empty: the page is a notice and nothing else. So there is no search surface, and there will not be one: the site is in the plaintiffs' hands, not the broker's. Note the dataset's other claim about this row -- 'suspected shared operator/engine with the TruePeopleSearch/FastPeopleSearch family (heuristic grouping)' -- is now positively CONTRADICTED: the judgment places verecor.com in the Radaris case, alongside radaris-com, rehold-com and peekyou-com, which this sweep has already recorded as seized in the same or a parallel Atlas action. DATASET DEFECT: both the defunct/NXDOMAIN note and the tps- family grouping are stale; flagged and not fixed.

### `verifyrecords-com`

- **scope:** dataset | **kind:** unclassified | **from:** `search_forms.SEARCH_UNDECIDED`

  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle) 2026-09-25, batch 40. SURFACE CONFIRMED, CAPTCHA-FREE, AND PROVED NOT URL-ADDRESSABLE. http://verifyrecords.com/ (served over plain HTTP, as the dataset notes) answers 200 with the title 'Background Records | VerifyRecords.com' and carries form#form1.form-horizontal, method GET, action https://verifyrecords.com/reg7/loader.php, with input[name=firstname], input[name=lastname], a 52-option select[name=state]#exampleSelect1, a hidden input[name=page_name] and button#hdnSearchForm 'Search Now'. cap[] and widget[] are EMPTY. Because that is a GET with named parameters it was tested directly: /reg7/loader.php?firstname=Jo nathan&lastname=Quiverstone&state=Nevada&page_name=home answered HTTP 200 and served THE HOMEPAGE BACK, the same empty #form1, with no result content -- so the parameters are discarded without a session and the results page is NOT reachable by URL. That is a finding, not a failure: it rules out the cheap URL- addressable recipe shape. 'reg7' in the path is a hint that the results route is a registration funnel, and the site carries a Login link, so a form-driven run should expect a paywall. Held undecided pending that run; no both-ways discrimination could be measured.

### `veripages-com`

- **scope:** dataset | **kind:** dead-url | **from:** `optout_forms.OPTOUT_BLOCKED`

  > Verified by browser render (Playwright, throwaway long-settle wrapper importing JS/UA from tools/probe_broker_forms.py, 14s settle, its own single-target prober invocation because the URL contains optout/opt-out/remove) 2026-09-25, batch 40. TWO STEPS, TWO DIFFERENT FAILURES. Step one: the recorded opt_out_url https://veripages.com/optout answers HTTP 404 ('Oops! We can't seem to find the page you're looking for') -- DATASET DEFECT, flagged and not fixed. Step two: the live footer offers 'Do Not Sell or Share My Personal Information' -> https://veripages.com/inner/removal-service?s=footer, and rendering THAT lands on the same URL with a __cf_chl_rt_tk challenge token appended, HTTP 403, title 'Just a moment...' and Cloudflare's challenge script -- a managed challenge, zero forms, nothing of the removal page served. The contrast is the finding: the SEARCH side of this site renders without a challenge (see the search leg, where even the 404 page came through intact) while the REMOVAL side is challenged, so the wall is targeted at the removal path rather than at the visitor. Filed blocked. The dataset's note that removal goes 'via the listing's Do-Not-Sell footer flow' is confirmed as to the route; the flow itself could not be reached. Human contact on the site: support@veripages.com.
