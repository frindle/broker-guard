...
NO_SEARCH_SURFACE = {
    ...
    "ncsolutions-com": (
        "Verified by browser render (Playwright, 12-20s settle) "
        "2026-09-25, batch 31. NCSolutions no longer exists as its own "
        "site: ncsolutions.com 302s to www.circana.com, and the Circana "
        "corporate home page carries zero forms, zero inputs and no "
        "people-search of any kind -- it is a market-research vendor's "
        "marketing site (Solutions / Industries / Company / Contact Us). "
        "The acquisition is confirmed on their own page "
        "circana.com/post/circana-completes-acquisition-of-ncsolutions, "
        "dated Jun 2 2025. NCS sells household-level audience segments to "
        "advertisers, so there is nothing for a consumer to search even "
        "in principle. cap[] and widget[] both empty."
    ),
    "ncr-com": (
        "Verified by browser render (Playwright, 12-16s settle) "
        "2026-09-25, batch 31. NCR Corporation has SPLIT INTO TWO "
        "COMPANIES and neither half is a consumer-searchable anything. "
        "www.ncr.com now serves a 792-character interstitial titled 'NCR "
        "Voyix and NCR Atleos' whose whole job is to send you to one of "
        "the two successors, and www.ncr.com/privacy 301s to "
        "www.ncrvoyix.com/legal/privacy-policy (30k characters, effective "
        "January 2026). Zero forms, zero inputs, no search of any kind, "
        "on either page. cap[] on the privacy policy holds only the site- "
        "wide recaptcha api.js with nothing to guard (widget[] empty, no "
        "form). NCR is a point-of-sale and ATM vendor; the data-broker "
        "listing is about payment/transaction data held as a processor, "
        "and there is no consumer lookup surface."
    ),
    "neighbor-report": (
        "DEAD DOMAIN, verified at the DNS layer 2026-09-25, batch 31, and "
        "this one is worth distinguishing from an ordinary NXDOMAIN. "
        "Playwright returned net::ERR_NAME_NOT_RESOLVED for "
        "https://neighbor.report/. That is NOT a local resolver or WARP "
        "artefact: 1.1.1.1 and 8.8.8.8 both return SERVFAIL, and "
        "Cloudflare's extended DNS error says in as many words 'at "
        "delegation neighbor.report.' / '78.27.225.33:53 returned REFUSED "
        "for neighbor.report A'. A +trace shows the .report registry "
        "delegating the zone to ns1/ns2/ns3.mirohost.net, and those "
        "delegated authoritative servers REFUSE the zone they are "
        "delegated. The registration itself is still status ACTIVE, so "
        "this is a broken/abandoned delegation rather than an expiry -- "
        "the site is unreachable network-wide for everyone, not just from "
        "here. NOTE THE PAIR: officialusa-com, mapped in the same batch, "
        "fails in the IDENTICAL way from the same mirohost.net "
        "nameservers at the adjacent address 78.27.225.34, so two "
        "separate rows in this dataset are one operator's dead hosting. "
        "The dataset's claim that person pages exist at "
        "/person/<Last>-<id> is therefore unverifiable and moot."
    ),
    "officialusa-com": (
        "DEAD DOMAIN, verified at the DNS layer 2026-09-25, batch 31, in "
        "exactly the same way as neighbor-report and from the same "
        "operator's nameservers. Playwright returned "
        "net::ERR_NAME_NOT_RESOLVED for https://www.officialusa.com/; "
        "1.1.1.1 and 8.8.8.8 both SERVFAIL; Cloudflare's extended error "
        "reads 'at delegation officialusa.com.' / '78.27.225.34:53 "
        "returned REFUSED for officialusa.com A'. whois shows the domain "
        "status ACTIVE, registrar Internet Invest Ltd dba Imena.ua, name "
        "servers NS2.MIROHOST.NET and NS3.MIROHOST.NET -- the same "
        "mirohost.net cluster that refuses neighbor.report at "
        "78.27.225.33. So the delegated authoritative servers refuse "
        "their own zone and the site is unreachable network-wide, not "
        "merely from this machine. The dataset's 'Public-records "
        "directory. Online removal tool.' describes a site that no longer "
        "resolves."
    ),
    "numberville-com": (
        "DEAD DOMAIN, verified at the DNS layer 2026-09-25, batch 31 -- a "
        "third variant of the same failure mode found in this batch, with "
        "a DIFFERENT operator, which is why it is worth writing down "
        "separately. Playwright returned net::ERR_NAME_NOT_RESOLVED for "
        "https://numberville.com/optout (probed as a single target on its "
        "own invocation, since the URL contains 'optout'). 1.1.1.1 "
        "returns SERVFAIL with the extended error 'at delegation "
        "numberville.com.'. A +trace shows the .com registry delegating "
        "to dns1.registrar-servers.com and dns2.registrar-servers.com -- "
        "Namecheap BasicDNS -- and querying dns1.registrar-servers.com "
        "directly for numberville.com A returns REFUSED, i.e. the domain "
        "is delegated to a DNS provider that holds no zone for it. "
        "Unreachable network-wide. The dataset already noted this row had "
        "no database of its own and merely redirected to "
        "ReversePhoneCheck for phone lookups; there is now not even that."
    ),
    "nymblr-com": (
        "Verified 2026-09-25, batch 31: the site is in an INFINITE SELF- "
        "REDIRECT LOOP and serves nothing to anyone. Playwright returned "
        "net::ERR_TOO_MANY_REDIRECTS for BOTH https://www.nymblr.com/ and "
        "the bare https://nymblr.com/. DNS is fine (Cloudflare A records "
        "172.67.193.47 / 104.21.20.159, Google Workspace MX), so this is "
        "not the DNS-dead shape of the three other dead rows in this "
        "batch. curl -I https://nymblr.com/ returns 'HTTP/2 301' with "
        "'location: https://nymblr.com/' -- the origin 301s the URL to "
        "itself, behind Cloudflare (cf-cache-status: DYNAMIC, x-seen-by "
        "header). There is therefore no page at all, let alone a people- "
        "search surface. Nymblr d/b/a Nimbler sold B2B contact data, so a "
        "consumer search surface was never likely; the point recorded "
        "here is that the whole host is non-functional, which no static "
        "fetch of a redirect-following client would have told you "
        "clearly."
    ),
    "home-neustar": (
        "Verified by browser render (Playwright, 12-16s settle) "
        "2026-09-25, batch 31. Neustar is now TransUnion -- home.neustar "
        "says so itself ('Neustar's Marketing, Fraud, and Communications "
        "offerings are now a part of TransUnion's newly unified B2B "
        "portfolio'), and the Privacy Choices portal banner says 'Neustar "
        "is now a TransUnion company'. The home page carries no search of "
        "any kind. The ONLY form on it is a site-wide Oracle Eloqua B2B "
        "lead-capture form (form#form592.elq-form posting to "
        "s335973.t.eloqua.com/e/f2) parked off-layout in a hidden 'Get in "
        "touch' modal: emailAddress, firstName, lastName, company, "
        "busPhone, title, three product/industry/campaign selects, "
        "country (196 options), stateProv (52), plus ~15 hidden tracking "
        "fields (elqSiteId, elqCampaignId, neustarSFDCCampaignID1, "
        "utmsource, utmmedium, disclaimerEmail, disclaimerPhone, "
        "leadsource-recent...). That is a sales-contact form, not a "
        "consumer lookup and not an opt-out. cap[] and widget[] both "
        "empty on home.neustar."
    ),
    "nexsales-com": (
        "Verified by browser render (Playwright, 12s settle) 2026-09-25, "
        "batch 31. nexsales.com is a B2B 'Global Capability Center for "
        "B2B Revenue Pipeline' marketing site with no consumer lookup "
        "surface. The ONLY form on the whole page is a footer newsletter "
        "signup: form#footer-newsletter-form, method GET back to "
        "nexsales.com/, holding one required input[type=email] with "
        "placeholder 'your@company.com' (no name attribute at all) and a "
        "Subscribe submit button. cap[] and widget[] both empty. Nexsales "
        "sells outsourced SDR/lead-research services; what it holds is "
        "B2B contact records on business buyers, which are not searchable "
        "by the public."
    ),
    "thinkdatagroup-com": (
        "Verified by browser render (Playwright, 12-16s settle) "
        "2026-09-25, batch 31. www.thinkdatagroup.com is a Squarespace "
        "marketing site for a data marketplace ('Periodic Table of Data "
        "Elements', Data Sellers / Data Buyers & Users / Digital "
        "Audiences for Marketers). Zero forms and zero inputs on the home "
        "page at a 12s settle, and no people-search anywhere on it. cap[] "
        "and widget[] both empty on the home page (the invisible "
        "reCAPTCHA Enterprise only attaches on the opt-out page -- see "
        "this row's OPTOUT_BLOCKED entry). New Gold Ventures LLC trades "
        "audience segments to marketers, so there is nothing for a "
        "consumer to look themselves up in."
    ),
    "nextroll-com": (
        "Verified by browser render (Playwright, 12-16s settle) "
        "2026-09-25, batch 31. www.nextroll.com/privacy renders the "
        "NextRoll Service Privacy Notice, effective June 10 2026 -- "
        "128,234 characters of policy prose with ZERO forms and zero "
        "inputs on it. cap[] and widget[] both empty. NextRoll (AdRoll) "
        "is a demand-side ad platform keyed to cookies and device "
        "identifiers, so there is no consumer-facing search surface even "
        "in principle. DATASET DEFECT CONFIRMED: the row's own note "
        "already suspected that opt_out_url pointed at a general privacy "
        "policy rather than a form, and that is exactly right -- the "
        "actual request surface is the separate relyance-ai row "
        "(nextroll-privacy.relyance.ai), and the ad opt-out is "
        "app.adroll.com/optout."
    ),
    "relyance-ai": (
        "Verified by browser render (Playwright, 16-20s settle) "
        "2026-09-25, batch 31. nextroll-privacy.relyance.ai is NextRoll's "
        "hosted privacy-request portal, not a data-broker site of its "
        "own, and it has no search surface: the landing route "
        "(#/?locale=en-US) renders 1,171 characters of instructional "
        "prose and nothing else -- zero forms, zero inputs, zero frames, "
        "cap[] and widget[] both empty -- at BOTH a 16s and a 20s settle, "
        "so this is not the thin-at-2.5s illusion. Relyance AI is the "
        "vendor; the broker of record is nextroll-com. See this row's "
        "OPTOUT_OUT_OF_SCOPE entry for what the portal actually asks for."
    ),
    "nexxen-com": (
        "Verified by browser render (Playwright, 16s settle) 2026-09-25, "
        "batch 31. nexxen.com is an ad-tech (advertiser/publisher) "
        "marketing site and has no consumer lookup surface: the home page "
        "renders 1,899 characters with zero forms and zero inputs, cap[] "
        "and widget[] both empty. Every privacy-flavoured footer link on "
        "it -- 'Consumer Opt Out of Targeted Advertising', 'Do Not "
        "Sell/Share My Personal Information', 'Your Privacy Choices' -- "
        "points at the single page nexxen.com/privacy-data-subject- "
        "rights-request/, which is the opt-out leg (see this row's "
        "OPTOUT_BLOCKED entry). Nexxen International Ltd trades cookie- "
        "and device-keyed audience data, so there is nothing a person "
        "could search themselves in."
    ),
    "n-focus-com": (
        "Verified by browser render (Playwright, 12-16s settle) "
        "2026-09-25, batch 31. www.n-focus.com is a direct-marketing list "
        "vendor's WordPress site (WHO WE ARE / PRODUCTS & SERVICES / "
        "BUILD YOUR LIST / PARTNER) with no consumer lookup surface. The "
        "forms present are a site search and a 'Get My Quote' Gravity "
        "Form (gform_3: Name, Company, Email, Phone, 'Tell Us About Your "
        "Project', plus an off-layout 'Instagram' honeypot at input_17 "
        "and an Akismet ak_hp_textarea honeypot) -- a B2B sales enquiry, "
        "not a people search. NFocus sells consumer mailing lists to "
        "marketers; 'BUILD YOUR LIST' is a list-count tool for buyers, "
        "not a self-lookup for data subjects."
    ),
    "namericanmedia-com": (
        "Verified by browser render (Playwright, 12s settle) 2026-09-25, "
        "batch 31. namericanmedia.com is a Divi/WordPress agency site "
        "(direct mail, newspaper and magazine advertising) with no "
        "consumer lookup surface. Two forms: a theme site search "
        "(form.et-search-form, GET, input[name=s]) and a Contact Form 7 "
        "general contact form (wpcf7-form init: your-name, your-email, "
        "your-subject required, your-message textarea, plus the usual "
        "seven _wpcf7* hidden fields and a _wpcf7_recaptcha_response). "
        "Neither is a people search. North American Media LLC places "
        "advertising; the broker listing is about the mailing lists "
        "behind that, which are not publicly searchable."
    ),
    "novalist-com": (
        "Verified by browser render (Playwright, 12-16s settle) "
        "2026-09-25, batch 31. NOVA LIST COMPANY NO LONGER EXISTS UNDER "
        "THAT NAME AND THIS ROW IS A DUPLICATE OF ANOTHER ROW IN THIS "
        "DATASET. novalist.com 302s to onpointdatastrategy.com, whose own "
        "site banner states 'News: NOVA and Sunrise Data Services have "
        "now merged into OnPoint Data Strategy.' onpointdatastrategy-com "
        "is a SEPARATE row in source-brokers.json and is mapped "
        "identically in this same batch -- one operator, two rows, so any "
        "per-row opt-out would be sent twice to the same company. The "
        "OnPoint site is a list-management / data-strategy / service- "
        "bureau B2B marketing site for non-profit fundraising data with "
        "no consumer lookup surface: zero search forms, and "
        "cap[]/widget[] show only the site-wide invisible reCAPTCHA v3 "
        "that belongs to the opt-out Gravity Form. DATASET DEFECT: this "
        "row's domain, name and (unverified) contact ckoch@novalist.com "
        "are all stale."
    ),
    "onpointdatastrategy-com": (
        "Verified by browser render (Playwright, 16s settle) 2026-09-25, "
        "batch 31. onpointdatastrategy.com is a B2B list-management / "
        "data-strategy / service-bureau site selling donor and prospect "
        "data to non-profits, with no consumer lookup surface -- zero "
        "search forms at a 16s settle. THIS ROW AND novalist-com ARE THE "
        "SAME COMPANY: novalist.com redirects here and the site's own "
        "banner reads 'News: NOVA and Sunrise Data Services have now "
        "merged into OnPoint Data Strategy.' Mapped identically in this "
        "batch. Recording the duplication explicitly because the two rows "
        "carry different names and different contact emails, and a per- "
        "row opt-out run would otherwise file the same request twice."
    ),
    "kycdata-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. kycdata.com "
        "renders as a B2B lead vendor (title 'Buy Fresh Homeowner Leads | "
        "KYC Data -', 4414 characters, nav of OUR SERVICES / WHY KYC DATA "
        "/ RESOURCES / CONTACT US / SIGN IN / ACCESS FREE DATA NOW) with "
        "ZERO forms on the page at a 14s settle and only Astra-theme "
        "menu-toggle buttons loose. On Hold-America compiles courthouse "
        "deed data for direct mail; there is nothing a consumer can "
        "search."
    ),
    "onaudience-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. onaudience.com "
        "renders as a B2B audience-data vendor (title 'OnAudience: Global "
        "Data Provider', 4968 characters) with ZERO forms on the homepage "
        "at a 14s settle. SAME-OPERATOR FINDING, recorded on both rows: "
        "onaudience.com and oan.pl are the same surface built from the "
        "same template by the same operator, not two coincidentally "
        "similar sites. Evidence: identical Polish nameservers "
        "dns.home.pl / dns2.home.pl / dns3.home.pl on both, the same "
        "Google Workspace MX set on both, adjacent AWS us-east-2 "
        "addresses (onaudience.com 3.128.214.89, oan.pl 3.130.142.194), "
        "the same Civic Cookie Control consent widget (ccc-notify-accept, "
        "ccc-notify-reject, ccc-notify-dismiss), the same three-mechanism "
        "opt-out page (email / third-party cookie / mobile ID) with the "
        "same copy, and the same reCAPTCHA Enterprise invisible "
        "integration differing only in sitekey. OnAudience Ltd and Online "
        "Advertising Network Sp. z o.o. are distinct dataset rows with "
        "distinct domains so both are mapped, but a fix applied to one "
        "applies to the other."
    ),
    "oan-pl": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. oan.pl renders "
        "as a B2B gaming-audience data vendor (title 'OAN - Global Gaming "
        "Audiences', 3134 characters); the only homepage form is "
        "#mainPageContactForm, a sales contact form. SAME-OPERATOR "
        "FINDING, recorded on both rows: onaudience.com and oan.pl are "
        "the same surface built from the same template by the same "
        "operator, not two coincidentally similar sites. Evidence: "
        "identical Polish nameservers dns.home.pl / dns2.home.pl / "
        "dns3.home.pl on both, the same Google Workspace MX set on both, "
        "adjacent AWS us-east-2 addresses (onaudience.com 3.128.214.89, "
        "oan.pl 3.130.142.194), the same Civic Cookie Control consent "
        "widget (ccc-notify-accept, ccc-notify-reject, ccc-notify- "
        "dismiss), the same three-mechanism opt-out page (email / third- "
        "party cookie / mobile ID) with the same copy, and the same "
        "reCAPTCHA Enterprise invisible integration differing only in "
        "sitekey. OnAudience Ltd and Online Advertising Network Sp. z "
        "o.o. are distinct dataset rows with distinct domains so both are "
        "mapped, but a fix applied to one applies to the other."
    ),
    "omginc-xyz": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. omginc.xyz is "
        "a three-page brochure site (Home / Privacy Policy / Contact Us) "
        "for a B2B company-data and website-visitor-identification "
        "vendor; the homepage renders 728 characters with ZERO forms and "
        "ZERO loose controls at a 14s settle, and the privacy page "
        "likewise has ZERO forms. Nothing to search."
    ),
    "opendatausa-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. DEAD SITE, and the row's claims are "
        "falsified. opendatausa.com now resolves to a NAMECHEAP PARKING "
        "PAGE -- the homepage renders 633 characters at a 14s settle with "
        "the title 'opendatausa.com is registered at Namecheap' and ZERO "
        "forms. The row's opt_out_url, opendatausa.com/optout, returns "
        "HTTP 404 with a bare nginx/1.28.3 (Ubuntu) error body, 35 "
        "characters, no forms. Probed as a single target on its own "
        "prober invocation since the URL contains optout. The row's notes "
        "describe a working flow -- personal information plus political "
        "donations, 'Search for your records' at /optout, and checking a "
        "box next to each record you want removed -- and NONE of that "
        "exists any more: there is no search, no record list and no "
        "checkboxes, because there is no site. This row had been flagged "
        "in the batch-32 brief as a likely real people-search surface "
        "deserving extra attention; it is not one, it is a parked domain."
    ),
    "opensend-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. opensend.com "
        "is a B2B visitor-identity-resolution vendor (it resolves "
        "anonymous site visitors into email addresses for its retail "
        "clients); the rendered pages carry only marketing navigation, a "
        "HubSpot live-chat frame and a cookie banner, with ZERO consumer- "
        "facing search form. Nothing a person can search about "
        "themselves."
    ),
    "openx-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. openx.com is a "
        "programmatic supply-side ad platform (title 'Programmatic "
        "Advertising | Supply-Side Platform - OpenX', 4777 characters) "
        "with ZERO forms on the homepage at a 14s settle. Nothing a "
        "person can search about themselves."
    ),
    "optimalfusion-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. "
        "optimalfusion.com is a media/ad-tech shop (title 'Optimal Fusion "
        "| Performance Driven Integrated Media', 4494 characters) with "
        "ZERO forms on the homepage at a 14s settle."
    ),
    "oracle-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. The row is "
        "Oracle Data Cloud / BlueKai, an audience-data business, not a "
        "people-search site, and the business itself is end-of-lifed (see "
        "the opt-out entry). The only form reachable on the pages probed "
        "is Oracle's own site search (#u38searchForm, GET to "
        "search.oracle.com/results), which searches Oracle's website, not "
        "people."
    ),
    "pushint-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. DEAD DOMAIN. pushint.com does not resolve: "
        "dig returns NOTHING for both pushint.com and www.pushint.com, "
        "and Chromium fails net::ERR_NAME_NOT_RESOLVED on both "
        "https://pushint.com/manage-my-data (the row's opt_out_url) and "
        "https://pushint.com/. NXDOMAIN, not a timeout, not a TLS failure "
        "and not an anti-bot wall -- the name is gone from DNS entirely. "
        "This corroborates and completes the row's existing note that "
        "compliance@pushint.com hard-bounced on 2026-08-24 because the "
        "domain had no working mail server: by this batch it has no DNS "
        "record of any kind. ORIGIN8 Inc has no reachable surface of "
        "either sort."
    ),
    "outbrain-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. Outbrain is a "
        "content-recommendation and native-advertising network; its "
        "consumer-facing property is the interest-profile portal "
        "described in the opt-out entry, which shows you what it has "
        "inferred about your browser rather than letting you search for a "
        "person. No people search exists."
    ),
    "outlogic-io": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. outlogic.io is "
        "a location-data broker (X-Mode Social renamed) selling movement "
        "data to buyers; the homepage renders 2353 characters with no "
        "consumer search of any kind."
    ),
    "outwardmedia-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. UNREACHABLE HOST, and the failure is "
        "specific enough to name. outwardmedia.com and "
        "www.outwardmedia.com both resolve to 34.111.179.208, a Google "
        "Cloud load-balancer address, and TCP connects -- but the TLS "
        "handshake never completes: the server accepts the connection and "
        "then drops it having sent ZERO bytes. Confirmed four ways: "
        "Chromium fails net::ERR_CONNECTION_CLOSED on both "
        "https://www.outwardmedia.com/do-not-sell (the row's opt_out_url) "
        "and https://www.outwardmedia.com/; curl fails with LibreSSL "
        "SSL_ERROR_SYSCALL; openssl s_client reports 'unexpected eof "
        "while reading' with 'SSL handshake has read 0 bytes and written "
        "1558 bytes' and 'no peer certificate available' at default TLS, "
        "and the same at forced TLS 1.2 (0 read, 225 written). Plain HTTP "
        "is no way in either: http://www.outwardmedia.com/ answers 301 "
        "Moved Permanently to https://www.outwardmedia.com:443/, i.e. "
        "straight back into the dead listener. This is NOT an anti-bot "
        "wall -- a WAF sends a block page and a challenge sends an "
        "interstitial; this sends nothing at all, at a layer below HTTP "
        "-- and it is not a certificate-name mismatch either, since no "
        "certificate is presented. The DNS record is live and the origin "
        "behind it is not serving, so neither leg has a surface. DATASET "
        "DEFECT: the row records a live web-form opt-out at a host that "
        "cannot complete a TLS handshake."
    ),
    "owneriq-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. DEAD DOMAIN. owneriq.com does not resolve: "
        "dig returns NOTHING for both owneriq.com and www.owneriq.com, "
        "and Chromium fails net::ERR_NAME_NOT_RESOLVED on the row's "
        "opt_out_url www.owneriq.com/privacy-notice.html. NXDOMAIN, not a "
        "wall. This corroborates the row's existing note that "
        "tpowell@owneriq.com hard-bounced on 2026-08-21 as address/domain "
        "invalid -- the whole domain is now gone from DNS. Corroborated "
        "independently from a THIRD party in this same batch: "
        "optimalfusion.com's ad-preferences page still publishes an "
        "'OWNERIQ OPT OUT' footer link pointing at "
        "www.owneriq.com/privacy-notice, which is a dead link, and that "
        "surviving cross-reference is also the reason the optimalfusion- "
        "com and owneriq-com rows look related. Neither leg has a "
        "surface."
    ),
    "pacificeast-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. "
        "pacificeast.com is a B2B identity- and phone-data verification "
        "vendor (title 'Home - PacificEast', 4725 characters) with ZERO "
        "forms on the homepage at a 14s settle."
    ),
    "infillion-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. infillion.com "
        "is an advertising platform (title 'The First Agent-Native "
        "Composable Advertising Platform - Infillion', 2559 characters); "
        "its only forms are an AddSearch site-search box and two Marketo "
        "lead forms (#mktoForm_1067). Nothing a person can search about "
        "themselves."
    ),
    "paramountdirectmarketing-com": (
        ...
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. "
        "paramountdirectmarketing.com is a mailing-list broker (title "
        "'Paramount Direct Marketing', 4849 characters, nav of Browse "
        "Lists / Insurance Data / Email Marketing / Social Marketing / "
        "intentLINK); the single homepage form is a list-enquiry box. "
        "Nothing a person can search about themselves."
    ),
    "parasolleads-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. "
        "parasolleads.com is an insurance lead vendor (title 'Exclusive "
        "Insurance Leads for Agents, Brokers and Carriers', 6276 "
        "characters). Its five forms are all SELLING-side or lead- "
        "capture: one posts to /blogs/, one to /#, and three to "
        "updatedata4.php. None of them searches for a person on the "
        "consumer's behalf."
    ),
    "partnerscredit-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. No consumer search surface. "
        "partnerscredit.com renders a minimal brochure site (title "
        "'Home', 1490 characters) with ZERO forms and ZERO loose controls "
        "at a 14s settle -- the only links are two 'Request Proposal' "
        "pointers at request-proposal.html. Partners Credit and "
        "Verification Solutions sells mortgage-credit and verification "
        "services to lenders; nothing a consumer can search."
    ),
    "protectdataprivacy-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. www.protectdataprivacy.com serves ONE page "
        "at a 14s settle and it is not a people-search: title 'California "
        "Consumer Privacy Policy Preference Center', 1968 characters, and "
        "the only form on it is the rights-request wizard #ccpa_form (see "
        "the OPTOUT_UNDECIDED entry for the full transcription). The page "
        "states its own business model -- 'Protect Data Privacy is a "
        "Business-to-Business Software provider, we collect, store and "
        "share information only as directed by our clients through the "
        "use of our software. We do not respond to any requests or "
        "question by consumers directly.' No person lookup, no "
        "name/phone/address query box, no nav to one. cap[] and widget[] "
        "both EMPTY."
    ),
    "peekyou-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. PEEKYOU IS NO LONGER A PEOPLE-SEARCH SITE "
        "AT ALL, and this is a pattern worth naming: LITIGATION- "
        "SETTLEMENT DOMAIN TRANSFER. www.peekyou.com returns HTTP 200 "
        "with title 'Domain Update - PeekYou.com', 1901 characters, ZERO "
        "forms, ZERO inputs, ZERO controls of any kind, cap[] and "
        "widget[] both empty, and body copy reading 'The domain name "
        "peekyou.com is controlled by Atlas Data Privacy Corporation "
        "(Atlas)' -- referencing Atlas Data Privacy Corporation, et al. "
        "v. PeekYou LLC, et al., Superior Court of New Jersey, Law "
        "Division, Essex County, Docket No. ESX-L-003299-25, a New Jersey "
        "Daniel's Law suit brought on behalf of ~21,700 law enforcement "
        "officers and other covered persons, now settled. The domain is "
        "held by the PLAINTIFF. There is nothing to search. This is not a "
        "dead host and not a falsified claim -- it is the adversary-owns- "
        "the-domain case, and it should be distinguished from the whole- "
        "product-line sunset pattern (Oracle BlueKai) in future passes."
    ),
    "peopleconnect-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. peopleconnect.us is the PARENT COMPANY's "
        "suppression portal, not a search product. Rendered "
        "https://suppression.peopleconnect.us/privacy-center at a 14s "
        "settle (HTTP 200, title 'PeopleConnect - Suppression Center', "
        "5316 characters, cap[] and widget[] EMPTY) and "
        "https://suppression.peopleconnect.us/guided-mode at a 16s settle "
        "(which 302s to /login). Neither carries any person lookup -- the "
        "portal's four tabs are Start Here, Identity, Suppression "
        "Settings and Privacy Center, all about suppressing YOUR OWN "
        "record, and the only inputs anywhere are email boxes. ENTITY "
        "CHECK PERFORMED, and it came back clean: PeopleConnect is the "
        "parent of Intelius, TruthFinder and Instant Checkmate, and all "
        "three are ALREADY separately mapped rows in this checklist "
        "(intelius-com, truthfinder-com, instantcheckmate-com), so their "
        "search surfaces belong to those rows and are deliberately NOT "
        "double-filed here. This row covers the parent's own suppression "
        "host only."
    ),
    "peoplefindersdaas-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. DESPITE THE NAME, this is not a people- "
        "search: rendered https://peoplefindersdaas.com/ at a 22s settle "
        "(HTTP 200, title 'Home - PeoplefindersDaaS', 6309 characters, "
        "cap[] and widget[] EMPTY) and the site describes itself as 'a "
        "B2B-focused data solutions advisory firm that delivers strategic "
        "guidance and precise targeting execution to executives, "
        "management teams, and marketing professionals'. Nav is ABOUT / "
        "SERVICES / WORK / PROCESS / CONTACT. The only form on it is the "
        "WordPress WPForms CONTACT form #wpforms-form-577 "
        "(wpforms[fields][1] First Name, [5] Last Name, [2] Email, [3] "
        "Comment or Message, plus wpforms[id], wpforms[time_token], "
        "page_title, page_url, url_referer hidden fields) -- a contact "
        "box, not a person lookup. The single privacy-relevant anchor is "
        "'Remove Your Information :: "
        "https://peoplefindersdaas.com/personal-information/' (see "
        "OPTOUT_BLOCKED). DATASET DEFECT recorded separately: the row's "
        "opt_out_url uses the www host, and www.peoplefindersdaas.com "
        "serves a certificate whose common name does not match "
        "(net::ERR_CERT_COMMON_NAME_INVALID) -- only the apex resolves "
        "cleanly."
    ),
    "interseller-io": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. Rendered https://interseller.io/ at a 14s "
        "settle (HTTP 200, title 'Interseller', 2176 characters) and "
        "https://www.interseller.io/privacy at a 20s settle (HTTP 200, "
        "20682 characters): ZERO forms and ZERO controls on both, cap[] "
        "and widget[] empty on both. Interseller is a recruiter outbound- "
        "email tool ('Find email addresses and send automated emails in "
        "one platform'), gated behind LOGIN -- no consumer-facing lookup "
        "of any kind, and nothing to search without an account. SECOND "
        "CONFIRMED INSTANCE OF THE WHOLE-PRODUCT SUNSET PATTERN (after "
        "Oracle BlueKai's opt-out registry EOL): a site-wide banner on "
        "every page reads 'Interseller will be sunset on December 15 "
        "2026. Please transition any remaining work before that date.' "
        "Unlike BlueKai this is announced while the site still serves "
        "normally, so it is recorded here as a dated expiry rather than "
        "an already-dead surface -- a pass after 2026-12-15 should expect "
        "both legs to change."
    ),
    "permutive-com": (
        "Verified 2026-09-25, batch 33. DEAD HOST, and by an unusual "
        "mechanism worth naming: permutive.com is NOT NXDOMAIN -- it "
        "resolves, to the null route. dig +short permutive.com A returns "
        "0.0.0.0, and so does www.permutive.com. Consequently every "
        "connection attempt is refused rather than failing to resolve: "
        "Playwright returned net::ERR_CONNECTION_REFUSED for "
        "https://permutive.com/privacy/, "
        "https://www.permutive.com/privacy/ and http://permutive.com/, "
        "and curl returned exit 7 (failed to connect, port 443). "
        "Distinguish this from the ERR_NAME_NOT_RESOLVED shape (plmrkg- "
        "com, same batch): an explicit 0.0.0.0 A record is someone "
        "deliberately black-holing a domain they still control, not an "
        "expired registration. Nothing to search, and nothing served at "
        "the dataset's recorded opt_out_url."
    ),
    "pimeyes-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. PimEyes has a search, but it is IMAGE- "
        "KEYED -- a face-recognition reverse image search -- so there is "
        "no name-keyed search surface for this repo to drive, and none "
        "can be synthesised. Confirmed on the live opt-out flow "
        "(https://pimeyes.com/en/opt-out-request-form, which 302s to "
        "/en/opt-out, HTTP 200 at a 22s settle, title 'Opt-Out Request "
        "Form | PimEyes', 4203 characters): the ONLY way to identify a "
        "person to PimEyes is to hand it a photograph, and the request "
        "form's own step 1 is 'Please upload a clear photo of your face' "
        "backed by an input type=file. There is no first-name/last-name "
        "query anywhere. Filed no-surface rather than undecided "
        "deliberately: the answer will not change on a re-render, and "
        "leaving it undecided would invite a fourth agent to re-probe a "
        "face-search engine for a name box that does not exist. Cookiebot "
        "(CybotCookiebotDialog*) consent internals are the only loose "
        "controls."
    ),
    "pitchbook-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. Rendered https://pitchbook.com/privacy- "
        "policy at a 16s settle: HTTP 200, title 'Privacy Policy - "
        "PitchBook', 24981 characters, cap[] and widget[] both EMPTY. The "
        "single form on the page is the SITE CONTENT search -- class "
        "SearchResultsModuleTopFilters-query, method GET, action "
        "https://pitchbook.com/search, controls input text name=q plus a "
        "hidden name=gaClientId and two chrome buttons -- i.e. a site "
        "search over PitchBook's own editorial, not a person lookup. "
        "PitchBook's product (a private-capital market database covering "
        "people as business contacts) is entirely behind a paid login; "
        "the public site's only calls to action are 'Request a free "
        "trial', 'Request Research' and 'Request your PitchBook profile'. "
        "No free consumer presence check exists. Worth recording from the "
        "same page: PitchBook's own California Data Broker Disclosure "
        "states it received 21,714 right-to-delete requests from "
        "California data subjects in 2025 and 'processed 68 deletion "
        "requests and did not have information on the remaining 21,646'."
    ),
    "placeexchange-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. Rendered https://www.placeexchange.com/do- "
        "not-sell-my-information at a 22s settle: HTTP 200, title 'Do Not "
        "Sell My Information - PlaceExchange', 3550 characters. Place "
        "Exchange is a programmatic out-of-home (DOOH) advertising "
        "exchange; its full nav is visible on the rendered page (For "
        "Demand Partners / Place Exchange SSP / Clear / Dynamic Creative "
        "/ PerView Measurement, For Supply Partners, Publisher Portal, "
        "Portal Login, About, Markets and Partners, News & Events, Join "
        "Our Team, Contact Us) and contains no person lookup. Its own "
        "form copy states what it holds about individuals: 'In the course "
        "of operating Place Exchange, we may receive your device\u2019s "
        "mobile advertising identifier from our Data Providers' -- device "
        "identifiers, not searchable name records. Nothing for a consumer "
        "to search, and the identity it keys on is a MAID (see "
        "OPTOUT_OUT_OF_SCOPE)."
    ),
    "placer-ai": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. Rendered https://www.placer.ai/privacy- "
        "policy/consumer-privacy-policy at a 16s settle: HTTP 200, title "
        "'Privacy Policy (Consumers) - Placer.ai', 26762 characters, "
        "cap[] and widget[] both EMPTY. Placer Labs is a foot- "
        "traffic/location analytics vendor; the site's nav (Products, "
        "Solutions, Resources, Company, Free Tools, Pricing, The Anchor, "
        "Login) has no person lookup, and the policy explains why one is "
        "impossible by design: 'The data we receive from third-party "
        "mobile apps is stripped of identifiers, such as mobile "
        "advertising identifiers, names, other persistent device "
        "identifiers... We intentionally built our Platform Services to "
        "produce aggregated market research reports without relying on "
        "such User Identifiers', and again 'because we intentionally do "
        "not collect User Identifiers, it is very unlikely that we will "
        "be able to associate any information in our systems with you "
        "directly'. The ONLY form on the page is a HubSpot SALES form "
        "(portal 5995051, form 6b445dfe-feab-4e5f-a698-2820b8cf19d9) "
        "asking firstname/lastname/Work "
        "email/phone/jobtitle/company/sdr_vertical -- a demo request, not "
        "a rights request. Nothing to search."
    ),
    "plexuss-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. Rendered https://www.plexuss.com/ccpa-opt- "
        "out (302 to https://plexuss.com/ccpa-opt-out) at a 22s settle: "
        "HTTP 200, title 'CCPA Opt Out | Plexuss.com', 2675 characters, "
        "cap[] and widget[] both EMPTY. Plexuss is a college- "
        "recruitment/student-matching platform behind Log In / Sign Up; "
        "the only form on the page is the site's own content search "
        "(class search-form, method GET, one unnamed input labelled "
        "'Search Plexuss'). No person lookup, and no consumer presence "
        "check without an account. See NO_OPTOUT_SURFACE for the more "
        "interesting finding on this host."
    ),
    "hsforms-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. DATASET DEFECT FIRST, because it changes "
        "what this row even is: the domain recorded for this broker, "
        "hsforms.com, is NOT the broker's domain -- it is HUBSPOT's form- "
        "hosting domain (share.hsforms.com). The actual company is "
        "Plunge, LLC, which also holds its own separate row in this "
        "dataset as plungedigital-com. Confirmed by rendering the row's "
        "URL, https://share.hsforms.com/1erM4FtHJQY2p2JaWuo4YbQ2dsq6, at "
        "a 16s settle: HTTP 200, title 'Form', 6975 characters, and the "
        "form's own heading is 'US Consumer Opt-Out Request' with body "
        "copy 'Opting-out, or choosing to have data about you removed "
        "from Plunge\u2019s US marketing data products...' -- so the form "
        "IS Plunge's, merely hosted by HubSpot. There is no search "
        "surface here of any kind: share.hsforms.com serves one hosted "
        "form and nothing else, and no one can look a person up on it. "
        "ENTITY NOTE: hsforms-com and plungedigital-com are the same "
        "company at two different hosts, which is a same-operator pair "
        "rather than a key collision -- both keys are distinct and both "
        "are legitimately mapped, because the opt-out mechanisms they "
        "reach are DIFFERENT (HubSpot-hosted form here, an Osano "
        "DataSubject portal there)."
    ),
    "plungedigital-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. Rendered "
        "https://www.plungedigital.com/privacy-policy/ at a 16s settle: "
        "HTTP 200, title 'Privacy Policy - Plunge Digital', 7166 "
        "characters, cap[] and widget[] both EMPTY, ZERO forms. Plunge "
        "Digital is a marketing/data agency; nothing on the site offers a "
        "person lookup and the only controls present are Osano cookie- "
        "consent internals (osano-cm-dialog__close, osano-cm-manage "
        "'Manage Preferences', osano-cm-window__widget, osano-cm-info- "
        "dialog-header__close). Same company as the hsforms-com row in "
        "this dataset -- see that entry for the entity note."
    ),
    "plmrkg-com": (
        "Verified 2026-09-25, batch 33. DEAD HOST: Playwright returned "
        "net::ERR_NAME_NOT_RESOLVED for https://plmrkg.com/ -- the domain "
        "does not resolve at all, which is the ordinary expired/abandoned "
        "shape and should be distinguished from permutive-com in this "
        "same batch, whose A record deliberately resolves to 0.0.0.0. "
        "Consistent with the dataset's own note that george@plmrkg.com "
        "hard-bounced on 2026-08-24 with the recipient server rejecting "
        "all delivery attempts. Nothing to search; the row already "
        "records opt_out_url null and opt_out unknown, so there is no "
        "falsified claim here, just a confirmed dead operator."
    ),
    "pmg-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. Rendered https://www.pmg.com/dsr-form at a "
        "16s settle: HTTP 200, title 'DSR Form', 1439 characters. PMG "
        "Worldwide is a media/marketing agency; the rendered nav is WHAT "
        "WE DO / OUR WORK / TEAM & CULTURE / INSIGHTS & NEWS / ABOUT PMG "
        "/ OPEN ROLES / LET'S CONNECT and contains no person lookup. The "
        "only main-frame form is a single-field HubSpot email capture "
        "(portal 7578490, form c3dbe919-3ec6-4051-b9c1-8e956585bf49, "
        "input email name=email plus hidden hs_context) -- a newsletter "
        "box, not a search. The actual rights form is an embedded "
        "OneTrust iframe; see OPTOUT_BLOCKED."
    ),
    "popacta-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 34. popacta.com renders HTTP 200, title 'Pop "
        "Acta - Targeted Media', zero forms, zero controls, cap[] and "
        "widget[] empty. A political/advocacy media buyer with no "
        "consumer-facing lookup of any kind. No people search."
    ),
    "possiblenow-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.possiblenow.com renders HTTP 200, title 'Helping B2B & B2C "
        "Brands Stay on Top of Marketing Regulations | PossibleNOW'. The "
        "only form is a HubSpot site-feedback widget "
        "(feedback___website_experience / feedback___easy_to_understand / "
        "feedback___content_valuable radio scales plus feedback_message). "
        "PossibleNOW is a preference/consent-management vendor selling to "
        "brands; it exposes no consumer record lookup."
    ),
    "postie-com": (
        "Verified by browser render 2026-09-25, batch 34. postie.com "
        "renders HTTP 200, title 'Direct Mail Platform | Postie', zero "
        "forms, cap[] and widget[] empty. A B2B direct-mail platform; no "
        "consumer search surface."
    ),
    "postpilot-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.postpilot.com renders HTTP 200, title 'Direct Mail Marketing "
        "for Shopify & Ecommerce | PostPilot', zero forms in the main "
        "frame. B2B direct mail for ecommerce brands; no consumer lookup. "
        "Worth recording that PostPilot self-identifies in its own "
        "footer: 'PostPilot is a data broker under Texas law. To conduct "
        "business in Texas, a data broker must register with the Texas "
        "Secretary of State'."
    ),
    "predactiv-com": (
        "Verified by browser render 2026-09-25, batch 34. predactiv.com "
        "renders HTTP 200, title 'Predactiv | AI-Powered Intelligence | "
        "Infinite Opportunities'. The only form is the WordPress site "
        "search (input name=s). Predactiv is the rebranded data arm of "
        "ShareThis (its own privacy notice reads 'Predactiv, Inc. and its "
        "U.S. based affiliate ShareThis'); the product is audience "
        "segments sold to advertisers, with no consumer-facing record "
        "lookup."
    ),
    "preferredcommunications-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "preferredcommunications.com/consumer-opt-out/ redirects to "
        "preferredcommunications.com/lander and serves a GoDaddy PARKED- "
        "DOMAIN page: 'preferredcommunications.com is parked free, "
        "courtesy of GoDaddy.com. Get This Domain', with GoDaddy's own "
        "related-search topics and copyright notice. The operator no "
        "longer controls a live site here, so there is no search surface. "
        "DATASET DEFECT: the dataset records a live consumer opt-out web- "
        "form at this host."
    ),
    "preqin-com": (
        "Verified by browser render 2026-09-25, batch 34. www.preqin.com "
        "renders HTTP 200, title 'Preqin, a part of BlackRock', zero "
        "forms. Preqin is now a BlackRock subsidiary and its privacy "
        "policy at /policies/privacy-policy IS the BlackRock Privacy "
        "Notice (last revised 1 March 2025). A private-markets data "
        "terminal behind a paid login; no consumer people search."
    ),
    "privco-com": (
        "Verified by browser render 2026-09-25, batch 34. www.privco.com "
        "renders HTTP 200, title 'Private Company Database | PrivCo "
        "Financial Intelligence', with only a site-search text input (no "
        "name attribute). PrivCo sells private-COMPANY financials, not "
        "consumer records, and its lookup is behind Sign In / Create Free "
        "Account. No consumer people search."
    ),
    "propertyreach-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.propertyreach.com renders HTTP 200 at an 18s settle, title "
        "'Lead Search & Property Data: Unlock Real Estate Insights', 6748 "
        "characters, ZERO forms and only two tab buttons ('Search by "
        "State', 'Popular Cities') in the main frame; the sole child "
        "frame is the Clym consent widget (widget-next.clym-sdk.net). "
        "Every product surface -- Property Search, Property Owner Lookup, "
        "Skip Tracing -- sits behind LOGIN / START YOUR TRIAL. No "
        "anonymous search surface exists to drive."
    ),
    "propertyradar-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.propertyradar.com renders HTTP 200 at an 18s settle, title "
        "'Find Motivated Property Owners | Grow Your Business | "
        "PropertyRadar', 9080 characters. The ONLY form on the page is a "
        "HubSpot newsletter subscribe (email + Subscribe, action forms.hs "
        "forms.com/.../9494982/e5193131-3fea-4f3f-8938-d51149c8b02b), "
        "off-layout in the footer. All lookup is behind Login / Try it "
        "Free. No anonymous search surface."
    ),
    "prospectordatabase-com": (
        "Verified 2026-09-25, batch 34. DEAD HOST. Both "
        "prospectordatabase.com and www.prospectordatabase.com fail with "
        "net::ERR_NAME_NOT_RESOLVED in the browser, and an authoritative "
        "lookup against 1.1.1.1 (deliberately bypassing this host's local "
        "resolver) returns NO A record for either name. Confirmed "
        "genuinely dead, not a locally blackholed name. No search "
        "surface."
    ),
    "proximaplatform-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.proximaplatform.com renders HTTP 200, title 'HOME'. It is a "
        "DudaOne site whose only form is a generic contact widget "
        "(dmform-1, label-dmform-1, dmformsendto, dmformsubject, "
        "dmformfrom, dmformsubmitparams, page_uuid) plus a PhotoSwipe "
        "gallery's buttons. No consumer lookup of any kind; the footer's "
        "only privacy control is an 'OPT OUT PAGE' link (see the opt-out "
        "entry -- it is password-gated)."
    ),
    "publicdatausa-com": (
        "Verified 2026-09-25, batch 34. DEAD HOST. publicdatausa.com "
        "fails with net::ERR_NAME_NOT_RESOLVED in the browser; an "
        "authoritative lookup against 1.1.1.1 returns NO A record for "
        "publicdatausa.com or www.publicdatausa.com, while whois shows "
        "the domain itself still ACTIVE at NameCheap. Registered but "
        "unpublished -- no host to serve a search surface. DATASET "
        "DEFECT: the dataset records a live web-form at "
        "publicdatausa.com/remove.php."
    ),
    "pubmatic-com": (
        "Verified by browser render 2026-09-25, batch 34, with DNS pinned "
        "past this host's resolver (see below). pubmatic.com/legal/opt- "
        "out/ renders HTTP 200, title 'PubMatic Opt-Out | PubMatic', 4548 "
        "characters; the only forms are two copies of the site search "
        "(input name=s). PubMatic is a programmatic sell-side ad exchange "
        "-- no consumer record lookup exists anywhere on it. TOOLING GAP "
        "WORTH RECORDING: on a first pass both pubmatic.com and "
        "www.pubmatic.com failed with net::ERR_CONNECTION_REFUSED and "
        "resolved to 0.0.0.0, which reads exactly like a dead host. They "
        "are not dead -- this machine's resolver blackholes adtech "
        "domains, and 1.1.1.1 returns real Apple/AWS edge addresses "
        "(18.154.144.x). Any adtech row previously recorded as a dead "
        "host from an ERR_NAME_NOT_RESOLVED or ERR_CONNECTION_REFUSED on "
        "this machine should be re-checked against an external resolver "
        "before it is trusted."
    ),
    "pulsepoint-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.pulsepoint.com renders HTTP 200, title 'PulsePoint "
        "Healthcare Marketing Technology', zero forms; loose controls are "
        "Swiper carousel bullets and slide arrows only. A healthcare "
        "programmatic advertising platform; no consumer lookup."
    ),
    "purecars-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.purecars.com renders HTTP 200, title 'PureCars - Automotive "
        "Digital Marketing & Business Intelligence For Dealers', ZERO "
        "forms and zero loose controls. Sells marketing services to car "
        "dealers; no consumer record lookup."
    ),
    "pwc-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.pwc.com/us/en/site/california-privacy.html renders HTTP 200, "
        "title 'Privacy Statement for California Residents: PwC'. The "
        "only form is the AEM site search (id=slimSearchForm, input "
        "name=searchfield plus hidden pwcSiteSection / pwcLang / pwcGeo). "
        "PwC Product Sales LLC is a professional-services firm; there is "
        "no consumer people-search product to search."
    ),
    "quad-com": (
        "Verified by browser render 2026-09-25, batch 34. www.quad.com "
        "renders HTTP 200, title 'Quad MX Solutions: Smarter Marketing, "
        "Better Results'. The only forms are four copies of the "
        "Avada/Fusion WordPress site search (input name=s), each carrying "
        "a pair of CleanTalk anti-spam honeypot fields -- input text "
        "name=apbct__email_id__search_form and input submit "
        "name=apbct__label_id__search_form, both off-layout and "
        "invisible, the submit's value a rotating integer (26419, 38286, "
        "28303 observed on one load). Quad/Graphics is a commercial "
        "printer and marketing-services firm; no consumer lookup."
    ),
    "quadrant-io": (
        "Verified by browser render 2026-09-25, batch 34. www.quadrant.io "
        "renders HTTP 200, title 'Quadrant | Location Data and POI Data "
        "Solutions'. The only form is a HubSpot newsletter capture (email "
        "+ LEGAL_CONSENT.subscription_type_459685260, action forms.hsform "
        "s.com/.../48152462/9d3a9c62-96d7-49a8-9d72-7eafe63b17fc). "
        "Quadrant sells mobile location and POI data to businesses; no "
        "consumer lookup. Incidental finding: two footer 'Contact Us' "
        "links point at staging.quadrant.io rather than production."
    ),
    "qualfon-com": (
        "Verified by browser render 2026-09-25, batch 34. www.qualfon.com "
        "renders HTTP 200, title 'Customer Experience Solutions & BPO "
        "Company | Qualfon'. The only forms are two copies of the "
        "Elementor site search (input name=s, hidden e_search_props); "
        "everything else loose is the Cookiebot consent dialog. A "
        "BPO/contact-centre operator; no consumer record lookup."
    ),
    "quantcast-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.quantcast.com/privacy-choices renders HTTP 200, title 'Your "
        "Privacy Choices', 16367 characters at a 20s settle, and ZERO "
        "forms. Quantcast is a DSP/measurement platform whose consumer- "
        "facing surface is opt-out documentation only; there is no record "
        "lookup. (Quantcast Measure's public audience rankings were "
        "retired years ago and nothing on the current site searches "
        "individuals.)"
    ),
    "supplier-io": (
        "Verified by browser render 2026-09-25, batch 34. ENTITY CHECK "
        "FIRST, because the dataset's 'Qurium Solutions, Inc.' / "
        "supplier.io pairing looked wrong: it is correct. Qurium "
        "Solutions, Inc. is the corporate entity behind supplier.io -- "
        "SAP's partner directory lists the listing verbatim as 'Qurium "
        "Solutions, Inc. | Supplier Diversity Data Enrichment' from "
        "supplier.io, and supplier.io absorbed CVM Solutions and "
        "Tealbook. It is NOT the unrelated Qurium Media Foundation (the "
        "Swedish digital-rights organisation), and nothing here suggests "
        "a misfiled row. supplier.io renders HTTP 200, title 'Supplier "
        "Data & Diversity Intelligence'; the only form is a HubSpot demo "
        "request (firstname, lastname, email, jobtitle all required, a "
        "235-option country phone-code select with NO name, tel input "
        "with no name backed by a hidden name=phone, and a "
        "which_best_describes_your_role_ radio group). The product is "
        "data about SUPPLIER COMPANIES, not consumers -- 820 million "
        "insights on 20 million suppliers, per its own marketing. No "
        "consumer search surface."
    ),
    "radaris-com": (
        "Verified by browser render 2026-09-25, batch 34. LITIGATION / "
        "ADVERSARIAL DOMAIN CONTROL -- the second confirmed instance of "
        "this pattern after peekyou-com, and the same plaintiff. "
        "radaris.com renders HTTP 200 with title 'This Domain Has Been "
        "Transferred by Court Order - Radaris.com' and serves only a "
        "settlement/judgment notice: 'Atlas Data Privacy Corporation, et "
        "al. v. Radaris.com, et al., Superior Court of New Jersey, Law "
        "Division, Middlesex County, Docket No. MID-L-000847-24 ... "
        "Pursuant to a final judgment of the New Jersey Superior Court, "
        "the domain name radaris.com has been transferred to Atlas Data "
        "Privacy Corporation (Atlas) and is no longer under the control "
        "of its former operators.' The notice describes claims assigned "
        "by roughly 21,760 law-enforcement officers and prosecutors under "
        "New Jersey's Daniel's Law, an Amended Complaint filed 27 May "
        "2025 on a suit filed 8 February 2024. ZERO forms, ZERO controls, "
        "no search. This is distinct from a dead host and from a product "
        "sunset: the domain is live and owned by the broker's ADVERSARY. "
        "Radaris was expected to be one of the more hostile opt-out flows "
        "in this dataset; it no longer has one at this domain at all."
    ),
    "rampedup-io": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. rampedup.io is a B2B contact-data "
        "vendor ('800 Million Professionals updated Everyday'), sold by "
        "subscription through a login. The public site renders no people- "
        "search input at all: the only form on the marketing site is "
        "nothing, and the only input anywhere on the app host is the "
        "navbar site-search box (off-layout, class 'form-control input-sm "
        "no-bg no-border ng', placeholder 'Search...'), which searches "
        "the vendor's own app, not a consumer index. No free lookup, no "
        "name-keyed GET URL."
    ),
    "rayinsights-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. rayinsights.com (Ray Cdp, Inc.) is "
        "a B2B CDP / managed-marketing-services shop -- Acquisition, "
        "Loyalty, Data & Analytics, Ecommerce, Managed & Professional "
        "Services. The rendered homepage has ZERO forms and exactly one "
        "loose control, a 'Toggle navigation' button. No consumer lookup "
        "of any kind, no login-gated search advertised to the public, no "
        "name-keyed GET."
    ),
    "reachmarketing-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. reachmarketing.com is a B2B lead- "
        "generation and marketing-technology agency. The only form on the "
        "rendered homepage is the WordPress site search (ct-search-form, "
        "input name='s', off-layout inside a modal) plus CleanTalk's "
        "anti-spam honeypot pair (apbct__email_id__search_form and "
        "apbct_submit_id__search_form, both computed invisible). No "
        "consumer people-search surface of any kind."
    ),
    "realintent-co": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. realintent.co sells real-estate "
        "consumer-intent leads by subscription ('Become a member', 'Our "
        "Zipcodes', 'Automatic Listings'). The rendered do-not-sell page "
        "and nav expose no search control at all -- the only loose "
        "controls are a 'Contact us' button and an 'Open Nav' button. No "
        "consumer lookup surface."
    ),
    "realeflow-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. realeflow.com sells a white-label "
        "real-estate AI platform to brokerages, educators, influencers "
        "and tech companies ('150M+ proprietary property records'), gated "
        "behind Apply Now / Login. The rendered homepage has no search "
        "control whatsoever -- no name field, no address field, nothing "
        "public. Its data reaches consumers only through partners' "
        "branded deployments."
    ),
    "realitydebtsolutions-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. realitydebtsolutions.com (Reality "
        "Media, Inc.) is a debt-relief lead site -- Home / FAQ / About / "
        "Contact / FREE CONSULTATION. The rendered do-not-sell page's "
        "only loose control is a 'Main Menu' button and there are zero "
        "forms on it. No consumer record lookup exists."
    ),
    "realsourcedata-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. realsourcedata.com sells targeted "
        "marketing lists (auto-dealer, auto-warranty, consumer-demo, "
        "home-warranty, insurance, new-homeowner, timeshare-owner, travel "
        "leads) and data-append services to businesses. The rendered do- "
        "not-sell page has no search control -- only Squarespace nav "
        "buttons. There is no consumer-facing lookup."
    ),
    "recruitbot-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. recruitbot.com is an AI "
        "recruiting/sourcing platform selling access to a 'Proprietary "
        "Database of 655+ Million Candidate Profiles' to employers. "
        "Nothing on the rendered public site is a search surface -- the "
        "only controls on the homepage are the Complianz cookie-consent "
        "widget's four category checkboxes and its Accept/Deny/View- "
        "preferences/Save-preferences and 'Manage consent' buttons. The "
        "candidate index is behind a customer login."
    ),
    "yello-co": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. yello.co (RECSOLU, Inc. dba Yello, "
        "and WayUp, Inc. dba Yello Sourcing) sells campus-recruiting and "
        "sourcing software to employers -- Request a Demo everywhere, no "
        "consumer entry point. The rendered pages carry no search input; "
        "every loose control is either the CookieYes consent widget or a "
        "'Request a Demo' button. Candidate data lives inside customers' "
        "ATS tenants."
    ),
    "redbirddirect-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. redbirddirect.com is a SIX-PAGE "
        "static brochure site -- index.html, who.html, what.html, "
        "different.html, success.html, talk.html, and nothing else; every "
        "anchor on the site was enumerated. The homepage is one paragraph "
        "('Your source for strategic planning and list acquisition for "
        "direct mail and email marketing'). Zero forms, zero inputs, zero "
        "scripts of interest, cap[] and widget[] empty. No search surface "
        "can exist here."
    ),
    "redidata-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. redidata.com (Redi-Data, Inc.) "
        "sells healthcare, business and consumer mailing lists ('14M "
        "Proven Healthcare Professional Records', '1.4M US-based "
        "Physicians', '7M State-Licensed Nurses'). Its two rendered "
        "search-shaped forms are neither of them a people index: the "
        "WordPress site search (search-form form-inline, input name='s') "
        "and a Redi-Counts list-sizing tool (input name='users' "
        "id='txtSearch' labelled 'Zip Code', submit 'View Counts') that "
        "returns aggregate counts, not records. No name-keyed lookup."
    ),
    "redmob-io": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. redmob.io (Redmob FZCO, IFZA "
        "Business Park, Dubai) is an IAB-TCF-certified adtech seller. The "
        "rendered site is a three-link brochure (Privacy Policy / General "
        "Terms / Data Sharing) with ZERO forms and ZERO inputs. No "
        "consumer lookup surface of any kind."
    ),
    "refinitiv-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. ENTITY COLLISION / BRAND "
        "ABSORPTION: refinitiv.com no longer serves Refinitiv -- it 301s "
        "to lseg.com/en/data-analytics ('LSEG Data & Analytics'), the "
        "London Stock Exchange Group property that absorbed Refinitiv US "
        "LLC. Nothing on it is a consumer search: the only forms are a "
        "sales enquiry form (danda-dandasf-with-product-list-embedded-hr, "
        "requiring product_family, firstName, lastName, emailAddress, "
        "phoneNumber, country, company and jobTitle) and the SiteFooter- "
        "search site search (input name='q' posting to /en/search- "
        "results). LSEG's actual person-screening product, World-Check, "
        "is sold under contract and is not publicly searchable."
    ),
    "regis24-de": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. regis24.de (Regis24 GmbH, Alte "
        "Rhinstr. 16, 12681 Berlin) is a GERMAN credit-reference and "
        "address-tracing bureau selling to collections, housing, "
        "banks/insurers, fintech, transit and payment providers "
        "(Anschriften-Ermittlung, CreditCheck, Bonitaetspruefung, EMA- "
        "Praevention). The rendered Datenschutzerklaerung has ZERO forms "
        "and only three loose controls, all cookie-banner buttons "
        "(Akzeptieren / Ablehnen / weitere Informationen). There is a "
        "'Verbraucher-Service' consumer area but no public record search "
        "-- the bureau's traces are run for business clients. SCOPE NOTE: "
        "non-US, GDPR-shaped, and the dataset already records that "
        "Regis24 replied on 2026-08-21 that they only process data for "
        "individuals with a German or Austrian address."
    ),
    "rehold-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. LITIGATION / ADVERSARIAL DOMAIN "
        "CONTROL -- the THIRD confirmed instance of this pattern (after "
        "peekyou.com and radaris.com) and the same lawsuit as radaris. "
        "rehold.com no longer serves the address directory the dataset "
        "describes; every path renders one page titled 'This Domain Has "
        "Been Transferred by Court Order - Rehold.com'. Per that notice: "
        "Atlas Data Privacy Corporation, et al. v. Radaris.com, et al., "
        "Superior Court of New Jersey, Law Division, Middlesex County, "
        "Docket No. MID-L-000847-24; suit filed 2024-02-08, Amended "
        "Complaint 2025-05-27 asserting Daniel's Law claims assigned by "
        "roughly 21,760 covered persons; FINAL JUDGMENT BY DEFAULT "
        "entered 2026-08-27 against Radaris.com, Rehold.com and "
        "Trustoria.com, with seizure of this domain to Atlas and "
        "permanent injunctive relief. There is no search surface because "
        "there is no longer a broker site -- the domain is held by the "
        "plaintiff. HEADS-UP for a future batch: the same judgment names "
        "TRUSTORIA.COM, which is not currently a row in this dataset but "
        "is the same seizure."
    ),
    "reklaimyours-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. DOMAIN CHANGE: reklaimyours.com "
        "now redirects to REKLAIMYOU.COM (Reklaim Inc., TSXV: MYID / OTC: "
        "MYIDF) -- worth recording because the dataset key and every "
        "recorded URL are on the old host. Reklaim is the inverse of a "
        "people-search: it pays consumers for their own data and sells "
        "them removal services. Nothing on the rendered site is a lookup "
        "of other people -- the only inputs anywhere are the privacy- "
        "request forms themselves, and account access is behind Login. No "
        "search surface."
    ),
    "relpro-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. relpro.com sells B2B "
        "relationship/prospecting intelligence on companies and "
        "executives to financial and professional-services firms, behind "
        "'Schedule a Demo' and a customer login. The rendered public "
        "pages carry no lookup input -- every loose control is nav (Why "
        "RelPro?, Solutions, Web Platform, Technology, Customers, "
        "Resources) or the opt-out form itself. No public people search."
    ),
    "rentrisk-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. rentrisk.com (RentRisk on Command "
        "LLC, dba RentRisk, a Virginia LLC) sells tenant-screening tools "
        "to DIY landlords and agents behind 'TRY RISK FREE' and a login "
        "at app.rentrisk.com. The rendered homepage's only form is a "
        "Gravity Forms newsletter capture (gform_5: input_1 First Name, "
        "input_2 Last Name, required input_3 email, plus the Akismet "
        "honeypot pair ak_hp_textarea / ak_js). No public record lookup; "
        "screening reports are ordered inside the authenticated app, and "
        "the privacy policy confirms the search index is account-gated."
    ),
    "resonate-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. resonate.com (Resonate, a US-only "
        "AI consumer-data and intelligence vendor, Reston VA) sells "
        "audience segments and software to marketers behind 'Book a "
        "Meeting' and Login. The rendered privacy and request pages carry "
        "no lookup control; the only form in the main document is a "
        "computed-invisible Gravity Forms newsletter capture (gform_5, "
        "input_1 email). ENTITY NOTE: resonate.com and reson8.com are ONE "
        "company -- reson8.com is Resonate's data-serving domain and "
        "hosts the DSAR form resonate.com embeds."
    ),
    "reson8-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. ENTITY COLLISION: reson8.com is "
        "RESONATE Networks, Inc.'s data-serving domain -- the same "
        "company the dataset also carries as resonate.com. It is not a "
        "website with a search surface at all: ds.reson8.com is a "
        "Cloudflare-fronted pixel endpoint that answers HTTP 204 No "
        "Content, and optout-form.reson8.com serves exactly one page, the "
        "DSAR form that resonate.com embeds (title 'Optout Form', form "
        "id='user-privacy-form'), whose only controls are that form's own "
        "fields. There is no consumer lookup, no index, and no named- "
        "record search anywhere on the domain; Resonate's consumer "
        "identity is an ad identifier, sold to marketers as audience "
        "segments."
    ),
    "rocadvertising-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. DNS FALSE NEGATIVE FIRST: "
        "rocadvertising.com resolves to 0.0.0.0 on this machine's local "
        "resolver and Chromium reported ERR_CONNECTION_REFUSED; 'dig "
        "@1.1.1.1' returns 35.215.66.43 and the site renders fine once "
        "Chromium is given --host-resolver-rules. NOT a dead host. "
        "Results Only Consulting and Advertising is a direct-mail and "
        "print shop (Direct Mail Services, ROC Print Capabilities, "
        "Enhanced Digital Presence). Its only two forms are "
        "WordPress/Divi site search (et-search-form and rmp-search-form, "
        "both input name='s'). No consumer record lookup."
    ),
    "revcontent-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. DNS FALSE NEGATIVE FIRST: "
        "revcontent.com resolves to 0.0.0.0 locally and Chromium reported "
        "ERR_CONNECTION_REFUSED; 'dig @1.1.1.1' returns 44.193.241.58 / "
        "184.193.4.32 / 44.196.20.230 and the site renders fine through "
        "--host-resolver-rules. NOT a dead host. RevContent is a native- "
        "ad recommendation network (Advertisers / Publishers / Email "
        "Monetization). The only form on the rendered privacy policy is a "
        "Webflow footer newsletter capture (email-form, required input "
        "name='email-2', submit 'subscribe'). There is no people index "
        "and nothing to search: RevContent's consumer identity is a "
        "browser cookie, not a named record."
    ),
    "revealmobile-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. revealmobile.com (Reveal Mobile, "
        "Inc., Raleigh NC) is an out-of-home advertising measurement "
        "platform selling location-derived audiences to buyers, sellers "
        "and platforms behind 'Request a Meeting' and Log In. The "
        "rendered pages carry no lookup control at all -- every loose "
        "control is nav or the cookie widget. Its consumer identity is a "
        "mobile device identifier, not a named record, so there is "
        "nothing name-keyed to search."
    ),
    "reveliolabs-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. reveliolabs.com (Revelio Labs, "
        "Inc.) sells workforce-intelligence products built on public "
        "professional profiles -- Terminal, COSMOS, Data Builder, Job "
        "Architecture, Company Benchmarking, Compensation Data -- to "
        "enterprises, behind 'Talk to an Expert', 'Start Free Trial' and "
        "Sign In. Its public 'Company Directory' is company-keyed, not "
        "person-keyed, and the rendered do-not-sell page carries no "
        "person lookup: the only loose controls are nav buttons, two "
        "'Sign Out' buttons and the cookie 'I agree' button. No name- "
        "keyed individual search surface."
    ),
    "revoptimal-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. revoptimal.com (RevOptimal, LLC) "
        "sells deterministic audience segments and omni-channel "
        "targeting/attribution to advertisers across B2B, B2C, healthcare "
        "and political verticals -- its own privacy policy states it is "
        "'a data broker in certain US states'. The rendered privacy and "
        "DSAR pages carry no lookup control; the only child frames are a "
        "HubSpot chat widget and the DSAR vendor iframe. No consumer "
        "search surface."
    ),
    "rhetorik-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. rhetorik.com (Rhetorik Limited) "
        "sells B2B professional and company data by subscription -- 'Book "
        "a Demo', 'Free Trial', Log In. ENTITY NOTE worth recording: the "
        "site now carries a banner 'Exciting News: Rhetorik is now part "
        "of Lightcast', and its CCPA request-metrics link points at "
        "legal.lightcast.io, so this row is a Lightcast brand going "
        "forward. No lookup control appears on the rendered pages -- "
        "every loose control is nav, the CookieYes consent widget, or the "
        "embedded DSAR frame. No public people search."
    ),
    "rocketreach-co": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. rocketreach.co (Rocketreach, LLC) "
        "sells B2B contact intelligence -- 700M profiles, 'Verified "
        "Contact Data. Real Results.' -- and the homepage exposes NO "
        "lookup field of any kind at a 15s settle: the only controls are "
        "Sign Up, Sign Up for Free, Get a Demo, Try It For Free and the "
        "OneTrust cookie panel, and reCAPTCHA v3 (sitekey "
        "6Le8JXkUAAAAABWqhg7ud4UjL6yCBDirQhWh5CHD) loads sitewide. Person "
        "search is inside the logged-in product. No public people-search "
        "surface to drive."
    ),
    "roq-ad": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 36. roq.ad (Roq.Ad Inc., now ROQAD, "
        "which announced its acquisition of Zeotap Data) sells identity "
        "resolution and audience data to advertisers. Every rendered page "
        "carries only the site-wide newsletter form (email + company + a "
        "required consent checkbox) and invisible reCAPTCHA v3 (sitekey "
        "6LeMgKMtAAAAABwvqLdFDvhNQJMhrRuiVv3DEvl0). No consumer lookup "
        "exists anywhere on the site."
    ),
    "roundsky-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. roundsky.com (Round Sky, Inc.) is "
        "a consumer-lending lead marketplace -- its own nav is AFFILIATES "
        "/ LENDERS / CLICKS / ABOUT US / CONTACT / SIGNUP / LOGIN. The "
        "rendered supplemental privacy notice (9971 characters) contains "
        "ZERO forms and zero inputs. Nothing on the site accepts a name "
        "to look up."
    ),
    "exlservice-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. The dataset row is 'RPM Data "
        "Solutions LLC' on domain exlservice.com -- an ABSORPTION, same "
        "shape as other renamed rows: the live site is ExlService "
        "Holdings, Inc. (EXL), an analytics and BPO company, and its "
        "privacy notice is EXL's, naming no RPM entity anywhere in the "
        "rendered text. The only form controls on it are the site's own "
        "GPT/keyword search boxes (act=/gpt-search) and a HubSpot "
        "newsletter email capture. No consumer record lookup."
    ),
    "realpage-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. realpage.com (dataset row: RP On- "
        "Site LLC) sells property-management and tenant-screening "
        "software (LeasingDesk) to landlords. Both the corporate privacy "
        "policy and the Consumer Support page render with only the site's "
        "navigation search box (act=/search/) and the OneTrust cookie "
        "panel. The policy is explicit that RealPage is a service "
        "provider and that consumer requests belong with the property "
        "manager. No consumer-facing record search."
    ),
    "sabio-inc": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. ENTITY COLLISION, CONFIRMED LIVE: "
        "sabio.inc/do-not-sell-my-info 30x-redirects to "
        "www.sabioctv.com/do-not-sell-my-info -- one company (Sabio, "
        "Inc.), two dataset rows, ONE surface, same page byte-for-byte "
        "(4062 characters, same Cookiebot banner, same embedded HubSpot "
        "form id 154f003f-b74c-49f5-98c2-9cfae9364c42 on portal "
        "20012662). Sabio is a CTV/mobile advertising platform; it keys "
        "everything on devices, not names, and no lookup field exists on "
        "either host."
    ),
    "sabioctv-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. Same entity as sabio-inc and the "
        "SAME PAGE -- sabio.inc redirects here (see that entry). "
        "CTV/mobile ad platform, device-keyed, no consumer search "
        "surface."
    ),
    "safegraph-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. safegraph.com (SafeGraph LLC) "
        "sells places/points-of-interest and foot-traffic data and its "
        "own policy stresses the product is 'data about physical "
        "locations - not identifiable individuals'. The privacy policy "
        "(18838 characters) and the CCPA addendum (11998 characters) both "
        "render with ZERO forms; the only controls are three nav dropdown "
        "buttons. No people search."
    ),
    "saferentsolutions-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. saferentsolutions.com (SafeRent "
        "Solutions, LLC) is a tenant-screening consumer reporting agency "
        "selling to landlords; the site's only entry points are CLIENT "
        "SIGN IN and Contact Sales. The recorded /privacy/ URL 301s to "
        "/privacy-policy-2/ and renders 37497 characters with ZERO forms. "
        "Consumer material is all PDF request forms and a mailto. No "
        "consumer-facing record search."
    ),
    "findtrueowner-com": (
        "Verified 2026-09-25, batch 36. DNS cross-checked against the "
        "local resolver AND 1.1.1.1 precisely because this machine has "
        "been caught blackholing hosts to 0.0.0.0: this is not that. "
        "findtrueowner.com (Saha Ventures LLC) returns NXDOMAIN from "
        "1.1.1.1 -- no A record for the apex, none for www, the domain is "
        "simply gone. The dataset records no opt_out_url and no "
        "opt_out_email for this row either. Nothing to search."
    ),
    "salesintel-io": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15-22s "
        "settle) 2026-09-25, batch 36. salesintel.io (Salesintel Research "
        "Inc.) sells B2B contact and intent data; its own copy says it "
        "manages 'the largest database of professional contact "
        "information on the planet'. Contact lookup is entirely inside "
        "the logged-in platform -- the public pages render no query field "
        "at all, only a scheduler.ai chat widget iframe. No public search "
        "surface."
    ),
    "salutarydata-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. DATASET HOST DEFECT FIRST: the "
        "recorded URL uses www.salutarydata.com, whose CNAME points at "
        "www.cdn.cloudflare.net which does not resolve "
        "(ERR_NAME_NOT_RESOLVED); the APEX salutarydata.com works and was "
        "used. salutarydata.com (SALUTARY DATA LLC) sells B2B contact "
        "records; the site's only query box is the WordPress site search "
        "(name=s). No people search."
    ),
    "sawyerlists-com": (
        "Verified 2026-09-25, batch 36. DNS cross-checked locally and "
        "against 1.1.1.1 (the blackhole check): sawyerlists.com (Sawyer "
        "Lists, LLC) is NXDOMAIN at 1.1.1.1 -- no apex A record, no www "
        "-- and Playwright confirms ERR_NAME_NOT_RESOLVED. Dead domain, "
        "and the dataset records no opt_out_url or opt_out_email for the "
        "row. Nothing to search."
    ),
    "sbfe-org": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. sbfe.org (Sbfe, LLC -- the Small "
        "Business Financial Exchange) is a lender-only data cooperative "
        "and says so on the rendered page: 'SBFE has no direct "
        "relationship with individuals or small businesses.' The site is "
        "membership/partner marketing, with zero inputs on the CCPA page. "
        "No consumer search surface."
    ),
    "scanbuy-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15-16s "
        "settle) 2026-09-25, batch 36. ENTITY ABSORPTION: "
        "scanbuy.com/rights/data redirects to kezzler.com/scanbuy/, and "
        "Kezzler's own privacy policy names 'Kezzler Holdings LLC "
        "(formerly Scanbuy)'. Scanbuy was a QR-code engagement/smart- "
        "packaging business, not a people-data business, and neither the "
        "Scanbuy landing page nor kezzler.com carries any lookup field -- "
        "only a newsletter signup whose hidden text input name=hp is a "
        "honeypot. No search surface."
    ),
    "scileads-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. scileads.com (Scileads Ltd) sells "
        "B2B life-sciences researcher contact data by subscription (Login "
        "/ Free Trial). The public pages carry no lookup field; the only "
        "form on the rendered opt-out page is the opt-out form itself, "
        "plus a HubSpot chat iframe. No public search surface."
    ),
    "scrapingrobot-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. scrapingrobot.com (Scraping Robot, "
        "LLC) sells a scraping API, not people records. The privacy "
        "policy renders 15888 characters with ZERO forms (the only "
        "captcha trace is Divi's bundled recaptcha.js, with no form to "
        "attach to). No search surface."
    ),
    "reonomy-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. reonomy.com (dataset row: Scryer, "
        "Inc.) is a commercial-real-estate data platform -- 'property and "
        "owner records' -- and its homepage exposes NO query field: every "
        "call to action is 'Request a free trial' or Login, and the only "
        "controls rendered are the Cookiebot consent panel. Owner lookup "
        "is inside the paid web app. Note also that the surface is "
        "address/property-keyed rather than name-keyed even inside the "
        "product. No public search surface."
    ),
    "seamless-ai": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. seamless.ai (Seamless Contacts, "
        "Inc.) sells B2B contact search inside a logged-in product; the "
        "recorded host preferences.seamless.ai is a DataGrail privacy "
        "request center, not a search page, and carries no lookup field. "
        "No public search surface."
    ),
    "semasio-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. semasio.com (Semasio, Inc.) is a "
        "cookie-based audience/targeting platform and says on its own "
        "privacy-choices page that it does 'not store names, unhashed "
        "email addresses or other direct unique identifiers'. Nothing on "
        "the site accepts a name to look up."
    ),
    "semcasting-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. semcasting.com (Semcasting, Inc.) "
        "sells identity-resolution and audience targeting; the recorded "
        "privacy-choice page 301s to /privacy_choice and its only form "
        "controls are the two HubSpot site-search boxes (name=term, "
        "act=/hs-search-results) plus a newsletter iframe. The consumer- "
        "facing 'online application' it advertises is privacychoice.com, "
        "which is an account signup, not a search (see the opt-out "
        "entry). No people-search surface."
    ),
    "srv1st-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. srv1st.com (Service 1St "
        "Information Solutions LLC) sells credit-reporting, 4506-C income "
        "and SSA-89 identity verification services to mortgage lenders "
        "(Client Login). The Wix homepage renders one newsletter email "
        "field and a Wix chat iframe -- no lookup field. No consumer "
        "search surface."
    ),
    "sg360-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. sg360.com (SG360 degrees) is a "
        "direct-mail and direct-marketing production house selling to "
        "brands. The rendered do-not-sell page carries the opt-out "
        "Gravity Form plus the WordPress site search (name=s) and nothing "
        "else. No people-search surface."
    ),
    "sharelocalmedia-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "and 25s settles) 2026-09-25, batch 36. sharelocalmedia.com "
        "(Share Local Media, Inc) sells direct-mail acquisition campaigns "
        "to brands; the only consumer-facing host is opt- "
        "out.sharelocalmedia.com, which is itself behind a Cloudflare "
        "managed challenge (see the opt-out entry) and is an opt-out, not "
        "a search. No search surface."
    ),
    "sharethrough-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle, rendered through Chromium --host-resolver-rules) "
        "2026-09-25, batch 36. DNS FALSE NEGATIVE FIRST, BLACKHOLE #6 ON "
        "THIS MACHINE: privacy-center.sharethrough.com resolves to "
        "0.0.0.0 locally and to 108.167.181.204 via 1.1.1.1; the page "
        "renders fine once the resolver is overridden, so 'dead host' "
        "would have been wrong. Sharethrough Inc. is an SSP/ad exchange "
        "whose privacy center is device- and cookie-keyed; no lookup "
        "field exists on it. No search surface."
    ),
    "sheerid-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15-18s "
        "settle) 2026-09-25, batch 36. sheerid.com (SHEERID Inc) sells "
        "identity/eligibility verification and, through its Audience "
        "Development Service, audience profiles to its customers. Both "
        "the audience-development privacy notice and the global privacy "
        "policy were rendered and enumerated for inputs explicitly "
        "(document.querySelectorAll over "
        "input/select/textarea/contenteditable/role=textbox/role=combobox "
        "returned an EMPTY list on both). No lookup field anywhere. No "
        "search surface."
    ),
    "simiocloud-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. simiocloud.com (SIMIO CLOUD LLC) "
        "sells marketing data and list products; the only forms on the "
        "site are its three consumer request forms (opt-out, access, "
        "deletion). No lookup field, no people search."
    ),
    "siteimpact-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. siteimpact.com (Site Impact) sells "
        "email/omnichannel audience data to agencies ('Identity Graph', "
        "'Email Enhanced', Book a demo). The homepage renders only a "
        "newsletter subscribe form (name=Email) with reCAPTCHA v2 "
        "attached. No consumer lookup field. No search surface."
    ),
    "skipsmasher-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. skipsmasher.com (Skipmasher, Inc. "
        "/ Skip Smasher LLC) is a skip-trace data vendor for "
        "investigators, and its homepage is a LOGIN, nothing else: the "
        "only form is ASP.NET __VIEWSTATE plus "
        "input[name=ctl00$ButtonLoginSubmit] value LOGIN, with 'Forgot "
        "username or password?' and JOIN alongside. Search happens only "
        "inside the paid account. Their own FCRA notice states results "
        "are not consumer reports. No public search surface."
    ),
    "skyhook-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. ENTITY COLLISION plus a DNS oddity "
        "worth recording. DNS: skyhook.com's APEX A record is "
        "authoritatively 0.0.0.0 (nameservers ns1/ns2.qualcomm-inc.com "
        "and ultradns) -- 0.0.0.0 at 1.1.1.1 and 8.8.8.8 alike, so this "
        "one is NOT this machine's blackhole; www.skyhook.com is live on "
        "Cloudflare and redirects to qualcomm.com. Skyhook Holding, Inc. "
        "is now Qualcomm's Terrestrial Positioning Service: the recorded "
        "opt-out URL lands on "
        "qualcomm.com/site/privacy/qualcommtpsoptout. The product is a "
        "database of cell towers, Wi-Fi access points and IP addresses, "
        "not of named people, and there is no lookup field. No search "
        "surface."
    ),
    "slashdotmedia-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. slashdotmedia.com (Slashdot Media, "
        "LLC) sells B2B tech-audience advertising and lead gen across its "
        "brands. The rendered cookie/opt-out page (20126 characters) "
        "contains ZERO forms and the homepage exposes no query field. No "
        "people-search surface."
    ),
    "smacomm-com": (
        "Verified 2026-09-25, batch 36. HOST-WIDE UNREACHABLE, and "
        "distinguished from a bot wall deliberately: smacomm.com (SMA "
        "Communications, LLC.) resolves identically locally and at "
        "1.1.1.1 (13.223.25.84) so this is not the DNS blackhole, but "
        "BOTH the recorded /do-not-sell/ path and the bare homepage time "
        "out with no response at all -- Playwright Page.goto exceeded 45s "
        "on three separate attempts (15s, 20s and 12s settles) and curl "
        "exceeded 30s and 40s on the two URLs, returning HTTP 000. No "
        "bot-check page, no challenge, no bytes. Nothing reachable to "
        "search."
    ),
    "snov-io": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. snov.io (SNOVIO Inc) sells email- "
        "finder and sales-engagement tooling; lookup is inside the "
        "logged-in app (Log in / Sign up / Talk to sales) and the public "
        "do-not-sell page carries only its DSAR form. No public search "
        "surface."
    ),
    "sourceitmarketing-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. ENTITY RENAME: "
        "sourceitmarketing.com redirects to sourceit.co (Sourceit "
        "Technologies, Inc), a holding company for cold-email/SMS "
        "marketing SaaS brands (Coldlist.io and others). The homepage's "
        "only form is a sales contact form -- notable in its own right "
        "for OBFUSCATED CONTROL IDS (every id is a random 32-character "
        "token such as LVZHQq0xzPusunswuxUS8ASoKh9L1CO8 while the names "
        "stay stable: name, company, email, phone, text) which is the "
        "rotating-field-name pattern in its id-only variant. No lookup "
        "field. No search surface."
    ),
    "sourcepathdigital-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. sourcepathdigital.com (Source Path "
        "Digital) sells digital marketing and audience solutions; the "
        "only form on the rendered California Consumer Portal is the "
        "consumer request form itself. No lookup field, no search "
        "surface."
    ),
    "sourcescrub-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. sourcescrub.com (Sourcescrub LLC, "
        "announcing that 'Sourcescrub and Grata are joining forces') "
        "sells deal-sourcing company intelligence to investors; access is "
        "Log In / Request Demo. The homepage's only form is a newsletter "
        "email capture and the only request links are demo requests. No "
        "people-search surface."
    ),
    "sovrn-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. sovrn.com (Sovrn, Inc.) is a "
        "publisher monetisation/ad platform; its privacy.sovrn.com host "
        "serves only the opt-out portal, which is cookie- and email-keyed "
        "and contains no lookup field. No search surface."
    ),
    "sms-inc-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. COMPANY OUT OF BUSINESS, stated on "
        "its own homepage: 'Specialists Marketing Services, Inc. Has "
        "Ceased Business Operations ... Following the successful sale of "
        "our partnership marketing, insert media and data divisions' (596 "
        "characters, zero forms), pointing only at contact@sms-inc.com "
        "for legacy invoices. The recorded do-not-sell URL returns HTTP "
        "404 with an empty body. Nothing to search."
    ),
    "rooftopdigital-com": (
        "Verified 2026-09-25, batch 36. NO WEB HOST AT ALL, established "
        "by DNS after the blackhole cross-check: rooftopdigital.com "
        "(Rooftop Digital, LLC) answers NOERROR from 1.1.1.1 but has NO A "
        "record for the apex and NO A record for www, on Cloudflare "
        "nameservers (ali/hans.ns.cloudflare.com) -- a registered domain "
        "that serves nothing. The dataset's recorded opt_out_url "
        "(rooftopdigital.com/privacy-policy/) therefore cannot resolve to "
        "a page; it is a FALSIFIED claim of a web-form opt-out. Nothing "
        "to search."
    ),
    "rtbhouse-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 16s "
        "settle) 2026-09-25, batch 36. rtbhouse.com (RTB House S.A.) is a "
        "programmatic retargeting DSP; its privacy center (38099 "
        "characters) and its opt-out host are both cookie/device-keyed "
        "and expose no lookup field. No search surface."
    ),
    "spycloud-com": (
        "Verified by browser render 2026-09-25, batch 37, three pages at "
        "15-18s settle: spycloud.com/contact-us/, "
        "spycloud.com/legal/privacy-policy/ and spycloud.com/check-your- "
        "exposure/. SpyCloud sells darknet-recaptured credential data to "
        "enterprises for account-takeover prevention; it holds consumer "
        "breach records but exposes no individual-keyed lookup. The only "
        "lookup on the site is 'Check Your Exposure', and it is scoped to "
        "an ORGANISATION, not a person: a single email input (id cye- "
        "prax-input) with a GET FREE REPORT button, under page copy that "
        "says in terms 'Use your work email. Results are scoped to your "
        "organization's domain' and 'SpyCloud matches your domain'. That "
        "is a corporate-domain exposure assessment gated on possessing a "
        "work email at the target domain -- the impossible-for-a-private- "
        "individual required field this workstream keeps meeting -- and "
        "it returns an org-level summary, not a record about the "
        "searcher. Every other form on all three pages is the Elementor "
        "site-search widget. No name, phone or address search exists "
        "anywhere."
    ),
    "stackadapt-com": (
        "Verified by browser render 2026-09-25, batch 37, 15-18s settle, "
        "both www.stackadapt.com and the legal-document-centre opt-out "
        "page. StackAdapt is a programmatic DSP; it targets by device ID, "
        "cookie ID (sa-user-id), IP and user agent (its own 2026-08-28 "
        "reply, recorded in the dataset row, says exactly that and that "
        "it found no match on name, email or address). There is no "
        "consumer-facing lookup of any kind: zero form elements on the "
        "homepage, and every interactive control enumerated is nav chrome "
        "(sr-only radio/checkbox menu toggles) or a Request a Demo link. "
        "A broker that cannot find you by name also cannot offer you a "
        "search surface."
    ),
    "staircase-co": (
        "Verified by browser render 2026-09-25, batch 37, at 14-18s "
        "settle, and the finding is broader than the search leg: "
        "staircase.co publishes NO legal surface at all. The site is a "
        "very large B2B mortgage-infrastructure product catalogue "
        "(several hundred anchors under /data, /products, /providers, "
        "/delivery, /platform, /agents) whose only form is the site's own "
        "product search (class sd__form, a single input.sd__input "
        "labelled 'Search products, providers, schema classes, data "
        "entities', method=dialog) plus its Cmd-K opener. That searches "
        "Staircase's own documentation, not people. No people-search, no "
        "address lookup, nothing individual-keyed. See NO_OPTOUT_SURFACE "
        "for the same verification run's finding that /privacy, /privacy- "
        "policy, /privacy-notice, /legal, /terms, /contact, /do-not-sell, "
        "/opt-out and /ccpa all return 404."
    ),
    "start-io": (
        "Verified by browser render 2026-09-25, batch 37, 15-18s settle, "
        "www.start.io and the do-not-sell page. Start.io is a mobile "
        "sell-side platform / SSP for app monetisation; it keys on mobile "
        "advertising identifiers, which is exactly what its opt-out form "
        "demands (see OPTOUT_OUT_OF_SCOPE). Zero forms on the homepage; "
        "every control enumerated is nav chrome or the CookieYes consent- "
        "preference checkboxes. No individual-keyed lookup exists."
    ),
    "statara-com": (
        "Verified by browser render 2026-09-25, batch 37, 15-18s settle, "
        "statara.com and statara.com/opt-out/. Statara Solutions is a "
        "political/advocacy audience-data and media-buying shop "
        "(Avada/Fusion WordPress build). The homepage renders ZERO form "
        "elements -- not even a site search -- and the only calls to "
        "action are PLAN MY CAMPAIGN links to a sales contact. No "
        "consumer lookup. Note for whoever reads the two legs together: "
        "the homepage loads the Gravity Forms reCAPTCHA v3 script "
        "(sitekey 6LdTp7kqAAAAALkZQK5qsabU4FqwxvvlcQzrkvn3) on a page "
        "with no form on it at all, which is how the broken opt-out form "
        "recorded in NO_OPTOUT_SURFACE announces itself sitewide."
    ),
    "statsocial-com": (
        "Verified by browser render 2026-09-25, batch 37, 15-18s settle, "
        "www.statsocial.com and /optout/. StatSocial sells 'people-based "
        "audience intelligence' -- it maps individuals to the creators "
        "and brands they follow and licenses that as audience segments "
        "and 'Digital Twins'. Despite holding person-level data it "
        "publishes no person-level lookup: zero forms on the homepage, "
        "every CTA a Request a demo link, and the platform itself sits "
        "behind Log in. The only public form on the site is the opt-out "
        "(recorded separately in OPTOUT_BLOCKED)."
    ),
    "step2successmarketing-com": (
        "Verified by browser render 2026-09-25, batch 37, 14-18s settle, "
        "and the first finding is that the dataset's domain is no longer "
        "the live one: step2successmarketing.com 302-redirects to "
        "s2smarketing.com ('S2S Financial Marketing' / STEP2SUCCESS "
        "Marketing, 12561 Ulmerton Road, Largo FL 33774, 800-579-9040). "
        "That site is hand-written static HTML -- the control enumeration "
        "returns an EMPTY list, there is not one input, select, textarea "
        "or button anywhere on the homepage. It sells seminar-marketing "
        "packages and mailing-list counts to financial advisors; the only "
        "data-facing link is REQUEST A COUNT (a count request for "
        "advisors, not a consumer lookup). No search surface of any kind."
    ),
    "steppingblocks-com": (
        "Verified by browser render 2026-09-25, batch 37, 15-18s settle, "
        "www.steppingblocks.com and /personal-information-inquiry. "
        "Steppingblocks sells graduate-outcomes analytics to "
        "universities, built on career data about named alumni. It "
        "exposes no individual lookup: zero forms on the homepage and no "
        "search control anywhere; the product is dashboards sold to "
        "institutions. The only public form on the domain is the "
        "personal-information request form, recorded separately in "
        "OPTOUT_BLOCKED."
    ),
    "sterling-ai": (
        "Verified by browser render 2026-09-25, batch 37, 15-18s settle. "
        "FIRST, THE ENTITY CHECK the batch was asked to run: sterling.ai "
        "is NOT sterlingcheck.com. sterling.ai is STERLING DATA COMPANY "
        "LLC (site titled 'Sterling AI | AI Driven Data Acquisition "
        "Solution', products: Data Acquisition, Texting List Rentals, "
        "Data Enrichment; dataset contact abe.usher@sterlingd.com), while "
        "the dataset separately carries 'Sterling' at sterlingcheck.com, "
        "the employment-background-screening company. Different "
        "companies, different domains, different keys -- no collision, "
        "and no dataset fix needed. On the search leg: no people-search "
        "surface. The four forms on the homepage are all Calculated "
        "Fields Form instances (cp_calculatedfieldsf_pform_1..4) "
        "implementing a PRICE calculator -- PRODUCT TYPE select (Email "
        "Acquisition / Mobile Acquisition / blended tiers), YOUR BUDGET, "
        "Total Deliverables -- gated by a self-hosted arithmetic captcha "
        "field (hdcaptcha_cp_calculated_fields_form_post). A quote "
        "calculator is not a consumer lookup."
    ),
    "gostrata-com": (
        "Verified by browser render 2026-09-25, batch 37, 15-18s settle, "
        "www.gostrata.com and /opt-out-preferences/. Strata Company is a "
        "30-year-old direct-mail and commercial-print house ('Print that "
        "Performs. Mail that Matters.'). The homepage renders ZERO forms. "
        "No consumer lookup, no record search; the data side is audience "
        "targeting for print campaigns sold to brands. The only public "
        "form on the domain is the Complianz data-request form recorded "
        "in OPTOUT_UNDECIDED."
    ),
    "strategicdataintelligence-com": (
        "Verified by browser render 2026-09-25, batch 37, 14-18s settle, "
        "strategicdataintelligence.com and /privacy-choices/. SDI is a "
        "B2B marketing-data consultancy ('Data that moves marketing "
        "forward'). No lookup surface: zero forms on either page, and the "
        "only enumerated controls are the two cookie-banner buttons "
        "(Essential only / Allow optional). No individual-keyed search."
    ),
    "strategicinfo-com": (
        "Verified 2026-09-25, batch 37: THE COMPANY IS GONE AND THE "
        "DOMAIN IS FOR SALE. This is a genuinely dead host, not this "
        "machine's DNS lying -- checked both ways per the recurring- "
        "blackhole rule and the A record agrees at the local resolver and "
        "at 1.1.1.1 (different AWS addresses, both live). What fails is "
        "the service: https://strategicinfo.com/ and "
        "https://www.strategicinfo.com/ both hang until timeout "
        "(Playwright 45s twice, then curl 25s twice, TCP/TLS never "
        "completes), while http://strategicinfo.com/ answers instantly "
        "with Server: Kestrel and a 302 to https://www.hugedomains.com/do "
        "main_profile.cfm?d=strategicinfo.com -- the HugeDomains "
        "aftermarket sale page. WHOIS confirms the mechanism: registrar "
        "TurnCommerce, Inc. DBA NameBright.com (HugeDomains' own "
        "registrar), NameBrightDNS nameservers, and a Registrar "
        "Registration Expiration Date of 2026-09-17, which is in the "
        "PAST. The registration lapsed and the name has been flipped to "
        "the parking/sale system. Consistent with the dataset's own note "
        "that jdrziak@strategicinfo.com hard-bounced on 2026-08-24. "
        "Nothing to search and nobody to ask."
    ),
    "structure-ac": (
        "Verified 2026-09-25, batch 37: HOST IS DEAD AT THE NETWORK "
        "LAYER, and again this is not the local-resolver blackhole -- "
        "structure.ac resolves to 167.71.188.255 identically at the local "
        "resolver and at 1.1.1.1 (Porkbun nameservers, DigitalOcean "
        "address), so the name is fine. The MACHINE is not. Playwright "
        "times out at 45s on https://www.structure.ac/opt_outs/new twice "
        "(once at 18s settle, once at 25s), and a third render of "
        "https://www.structure.ac/ fails outright with "
        "net::ERR_ADDRESS_UNREACHABLE. Below the browser: curl times out "
        "at 22s on all four of https/http x apex/www, nc reports neither "
        "port 443 nor port 80 reachable, and ping gets 100% packet loss. "
        "A stale DNS record pointing at a decommissioned droplet. "
        "Consistent with the dataset's note that thoran@structure.ac "
        "hard-bounced on 2026-08-24. No surface to find."
    ),
    "swoop-com": (
        "Verified by browser render 2026-09-25, batch 37, 14-18s settle, "
        "swoop.com and swoop.com/your-privacy-choices/. Swoop is a "
        "healthcare/pharma audience adtech firm (DTC audiences, privacy- "
        "safe modelling). No consumer lookup: the only forms on either "
        "page are the SearchWP site search (swp_form[form_id] / swps) and "
        "a Gravity Forms newsletter capture (gform_3, single input_1 "
        "email, with an Akismet honeypot ak_hp_textarea that a recipe "
        "would have to leave blank). Nothing individual-keyed."
    ),
    "theswarm-com": (
        "Verified by browser render 2026-09-25, batch 37, 15-18s settle, "
        "www.theswarm.com and /privacy. The Swarm ('The Relationship "
        "Intelligence Leader') curates 500M person profiles and 50M "
        "company records and sells relationship-graph data -- and exposes "
        "none of it publicly. Zero forms on the homepage; the product is "
        "behind Log in / Get started, and every CTA is a signup or Demo "
        "link. No individual-keyed lookup on the public site."
    ),
    "samba-tv": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. DNS FALSE NEGATIVE FIRST, "
        "BLACKHOLE #7 ON THIS MACHINE: samba.tv and www.samba.tv both "
        "resolve to 0.0.0.0 at the local resolver and to 31.43.161.6 / "
        "31.43.160.6 at 1.1.1.1 (www.samba.tv is a CNAME to "
        "sites.framer.app), and the pages render normally once Chromium "
        "is given --host-resolver-rules 'MAP * 31.43.161.6', so 'dead "
        "host' would have been a false call. Samba TV, Inc. sells "
        "automatic-content-recognition TV viewing data; the only input "
        "anywhere on the rendered pages is nothing at all -- samba.tv's "
        "home, /privacy-center/consumer-privacy-center and /privacy- "
        "center/notice-of-right-to-opt-out were each enumerated "
        "explicitly (querySelectorAll over input, select, textarea, "
        "[contenteditable], [role=textbox], [role=combobox]) and ALL "
        "THREE returned an EMPTY list. No lookup of a named person "
        "exists. No search surface. Domain finding recorded on the opt- "
        "out leg."
    ),
    "schufa-de": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. SCHUFA Holding AG is the German "
        "consumer credit bureau. www.schufa.de/datenschutz-dsgvo/ (which "
        "301s to /global/datenschutz-dsgvo/) is its Art. 14 GDPR "
        "information notice, and the only control on it is the site's own "
        "content search (button.search-icon-button, 'Suche oeffnen'); an "
        "explicit enumeration over input/select/textarea/[contenteditable "
        "]/[role=textbox]/[role=combobox] returned an EMPTY list. What "
        "SCHUFA holds about a person is reached only by requesting your "
        "own file (SCHUFA-Datenkopie under Art. 15 GDPR) through an "
        "identity-verified German consumer channel -- the same shape as "
        "equifax-com and chexsystems-com, which this tool must not "
        "automate -- and not by typing a stranger's name into a box. No "
        "search surface."
    ),
    "sentilink-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. SentiLink Corp. sells synthetic- "
        "identity and first-party fraud scoring to lenders; nothing "
        "consumer-facing. sentilink.com/privacy-policy/ (301s to "
        "www.sentilink.com/privacy-policy, HTTP 200, 'SentiLink | Privacy "
        "Policy', 19415 characters) was enumerated explicitly over input/ "
        "select/textarea/[contenteditable]/[role=textbox]/[role=combobox] "
        "and the ONLY controls are a newsletter "
        "input[type=email][name=email] 'Email address' and the policy's "
        "own mobile table-of-contents select (13 options, off-layout). No "
        "lookup of a named person anywhere. No search surface."
    ),
    "shopalyst-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Shopalyst Technologies (Shopalyst "
        "Inc., Fremont CA) sells shoppable-ad and path-to-purchase "
        "measurement to brands; its own customers are businesses and the "
        "people in its data are its customers' end users. "
        "shopalyst.com/privacy-policy (301s to www.shopalyst.com/privacy- "
        "policy/, HTTP 200, 25139 characters) was enumerated explicitly "
        "over input/select/textarea/[contenteditable]/[role=textbox]/[rol "
        "e=combobox] and returned an EMPTY list -- the only control on "
        "the page is the cookie notice's close button. No lookup field, "
        "no consumer surface at all. No search surface."
    ),
    "similarweb-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Similarweb Ltd. sells website and "
        "app traffic analytics to businesses; the unit of analysis is a "
        "domain, not a person. www.similarweb.com/corp/legal/privacy- "
        "policy/ (35880 characters) and the home page were both "
        "enumerated explicitly over input/select/textarea/[contenteditabl "
        "e]/[role=textbox]/[role=combobox], and the ONLY controls on "
        "either are Cookiebot's consent-category checkboxes "
        "(CybotCookiebotDialogBodyLevelButton*). No people lookup exists. "
        "No search surface."
    ),
    "smartclip-tv": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. smartclip (smartclip Europe GmbH, "
        "Koreastrasse 7, Hamburg) is an adtech SSP selling CTV/video "
        "inventory and audience data; its data is keyed to cookies and TV "
        "devices and it says so itself -- 'smartclip will make usage of "
        "personal data, but only with regards to anonymous identifiers "
        "(e.g. stored in cookies)... smartclip does not and will not "
        "process or use data like email'. smartclip.tv/privacy-policy/ "
        "(301s to /privacy/, 10697 characters) and privacy- "
        "portal.smartclip.net were both enumerated explicitly over input/ "
        "select/textarea/[contenteditable]/[role=textbox]/[role=combobox] "
        "and BOTH returned an EMPTY list. No lookup of a named person "
        "anywhere. No search surface."
    ),
    "t-mobile-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. T-Mobile USA is a carrier, and the "
        "dataset carries it for its Magenta Advertising Platform. There "
        "is no public people lookup: www.t-mobile.com/dns (which "
        "redirects to /privacy-center/do-not-sell) was enumerated "
        "explicitly over input/select/textarea/[contenteditable]/[role=te "
        "xtbox]/[role=combobox] and the only controls are the do-not-sell "
        "toggle itself, OneTrust's consent switches and OneTrust's own "
        "vendor-list search box (input[name=vendor-search-handler]). What "
        "T-Mobile holds about a person is behind that person's own "
        "account login. No search surface."
    ),
    "taboola-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. DNS FALSE NEGATIVE FIRST, "
        "BLACKHOLE #8 ON THIS MACHINE, and it covers the whole zone "
        "rather than one host: www.taboola.com and "
        "accessrequest.taboola.com both resolve to 0.0.0.0 at the local "
        "resolver and to tls13.taboola.map.fastly.net / 151.101.1.44 at "
        "1.1.1.1, and the pages render normally once Chromium is given "
        "--host-resolver-rules, so 'dead host' would have been a false "
        "call for every taboola.com name. Taboola is a content- "
        "recommendation and native-advertising network whose data is "
        "device-keyed; it says so itself on its own request portal -- 'we "
        "do not know your identity because we do not process your name, "
        "email address, or other identifiable data. Instead, we only "
        "process digital identifiers such as cookie IDs, IP addresses..., "
        "mobile advertising IDs'. No lookup of a named person can exist. "
        "No search surface."
    ),
    "fhamortgagefinder-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Tandem Marketing, LLC runs "
        "fhamortgagefinder.com as a mortgage lead-generation funnel: it "
        "collects a consumer's details and sells them to lenders. There "
        "is no lookup of a third party. "
        "www.fhamortgagefinder.com/CCPArequest.php was enumerated "
        "explicitly over input/select/textarea/[contenteditable]/[role=te "
        "xtbox]/[role=combobox] and every control on it belongs to the "
        "CCPA request form itself (see the opt-out leg). No search "
        "surface."
    ),
    "tapad-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Tapad sells cross-device identity "
        "graphs to advertisers, and like taboola-com it states plainly "
        "that it holds no names: 'Because Tapad does not know your name "
        "or known identity details, we rely on device identifiers to "
        "process requests'. www.tapad.com/privacy-policy (301s to "
        "/global-privacy-notice, 35021 characters), www.tapad.com's home "
        "page and crportal.tapad.com were all enumerated explicitly over "
        "input/select/textarea/[contenteditable]/[role=textbox]/[role=com "
        "bobox] and ALL THREE returned an EMPTY list. No lookup of a "
        "named person can exist. No search surface."
    ),
    "targetsmart-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. TargetSmart Communications sells "
        "political and voter-file data to campaigns; the data subject "
        "never queries it. privacy.targetsmart.com/general/opt_out "
        "redirects to the consumer request flow (see the opt-out leg), "
        "and the only controls on it are that flow's two email boxes. No "
        "people lookup is offered to the public. No search surface."
    ),
    "teads-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Teads is an omnichannel video SSP; "
        "its user data is cookie- and device-scoped and it holds no "
        "consumer-queryable index. www.teads.com/privacy-policy/ (301s to "
        "privacy-policy.teads.com, 2040 characters), the www.teads.com "
        "home page and user-privacy.teads.tv/viewer-data-deletion were "
        "all enumerated explicitly over input/select/textarea/[contentedi "
        "table]/[role=textbox]/[role=combobox]; the deletion page and the "
        "policy returned an EMPTY list, and the home page's only controls "
        "are OneTrust consent switches and OneTrust's own vendor-search "
        "box. No lookup of a named person. No search surface."
    ),
    "zendesk-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. DATASET DEFECT FIRST, because the "
        "row's key is misleading: this row is named 'TECHTARGET Inc' and "
        "its domain field is zendesk.com, which is not TechTarget's "
        "domain and not the domain of the company whose data is at issue "
        "-- see the opt-out leg for the full account. The broker is "
        "TechTarget (now Informa TechTarget), a B2B media and intent-data "
        "business. Its own site www.informatechtarget.com and the request "
        "host techtarget.zendesk.com were both enumerated explicitly over "
        "input/select/textarea/[contenteditable]/[role=textbox]/[role=com "
        "bobox]: the former's only controls are the site content search "
        "(input[name=s][id=s-header]) and two editorial segment selects, "
        "the latter's belong to the request form itself. No people lookup "
        "anywhere. No search surface. (Zendesk Inc., the software vendor "
        "whose host this id was derived from, is NOT a row in this "
        "dataset and has not been assessed.)"
    ),
    "informatechtarget-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Informa TechTarget is a B2B media "
        "and purchase-intent data business; its data subjects are IT "
        "buyers whose activity it tracks across its publications, and it "
        "offers them no lookup. www.informatechtarget.com/ccpa-privacy- "
        "notice/ (35114 characters) and the home page were both "
        "enumerated explicitly over input/select/textarea/[contenteditabl "
        "e]/[role=textbox]/[role=combobox]: the only controls are the "
        "WordPress site search (input[name=s][id=s-header], "
        "form#searchform-header) and two editorial segment selects on the "
        "home page. No people lookup. No search surface. Note the "
        "collision recorded under zendesk-com: that row is also "
        "TechTarget."
    ),
    "telefi-app": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. The site is not serving. "
        "telefi.app returns HTTP 402 with the body 'Deployment Paused' "
        "(77 characters) -- a Vercel-hosted deployment whose owner has "
        "stopped paying for or has suspended it; DNS is fine (76.76.21.21 "
        "at both the local resolver and 1.1.1.1), so this is neither a "
        "blackhole nor a dead host but a live server refusing to serve "
        "the app. No page, therefore no lookup field. No search surface."
    ),
    "telesign-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Telesign Corporation sells phone- "
        "number intelligence, identity scoring and verification APIs to "
        "businesses. www.telesign.com/privacy-requests was enumerated "
        "explicitly over input/select/textarea/[contenteditable]/[role=te "
        "xtbox]/[role=combobox] and the only main-frame control is the "
        "site content search (input[name=s][id=search]); everything else "
        "lives in the embedded OneTrust request frame (see the opt-out "
        "leg). What Telesign holds is queried by its business customers "
        "through an authenticated API, not by the public. No search "
        "surface."
    ),
    "terminusplatform-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Terminus Software sells account- "
        "based marketing to B2B advertisers, and as of this render the "
        "company is being folded into DemandScience (see the opt-out "
        "leg). home.terminusplatform.com now serves a 'JumpCloud User "
        "Portal - Login' -- an SSO login page whose only controls are an "
        "email box and a checkbox -- and the data-deletion path redirects "
        "off the domain entirely. There is no public people lookup on "
        "either host. No search surface."
    ),
    "thedatagroup-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. The Data Group sells data append "
        "and mailing lists ('Data Append & Mailing Lists From $0.02' is "
        "its own home-page title). Its buyers query its data; consumers "
        "cannot. thedatagroup.com's home page and its consumer opt-out "
        "page were both enumerated explicitly over input/select/textarea/ "
        "[contenteditable]/[role=textbox]/[role=combobox]: the home "
        "page's only controls are an Elementor newsletter/contact email "
        "box and its four hidden Elementor bookkeeping fields, and the "
        "opt-out page's controls all belong to the request form (see the "
        "opt-out leg). No people lookup. No search surface."
    ),
    "nielsen-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. The Nielsen Company sells audience "
        "measurement and, through Nielsen Marketing Cloud, marketing "
        "identity data. Three of its pages were rendered and enumerated "
        "explicitly over input/select/textarea/[contenteditable]/[role=te "
        "xtbox]/[role=combobox] -- the Marketing Cloud Services privacy "
        "statement, www.nielsen.com/legal/optout-page/ and "
        "sites.nielsen.com's NMC opt-in/opt-out page -- and no lookup of "
        "a named person exists on any of them. The only controls on the "
        "first two are the site's own content search (input[name=s]) and "
        "a site-wide 'Get a demo' sales form; the third's two controls "
        "are the opt-out itself (see the opt-out leg). Nielsen's panels "
        "are joined by invitation and its measurement data is sold to "
        "clients, neither of which is a consumer search surface. No "
        "search surface."
    ),
    "theworknumber-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. The Work Number is Equifax "
        "Workforce Solutions' employment and income verification database "
        "-- an FCRA consumer reporting product, and the data is released "
        "to 'credentialed verifiers with a permissible purpose under the "
        "Fair Credit Reporting Act (FCRA)' in the site's own words. "
        "employees.theworknumber.com was enumerated explicitly over input "
        "/select/textarea/[contenteditable]/[role=textbox]/[role=combobox "
        "] and the ONLY control is the site's own content search "
        "(input[name=keywords][id=search], off-layout). Everything about "
        "a person -- 'View Your Data', 'Freeze Your Data', 'Dispute Your "
        "Data', 'Get A Salary Key' -- sits behind Register / Log In. Same "
        "finding as equifax-com and chexsystems-com: what is held about "
        "you is reached by requesting your own file with identity- "
    ...
    "unity-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. Unity Technologies sells a game "
        "engine and runs the Unity Ads network; there is no consumer- "
        "facing people-search product here and nothing to query for a "
        "name. Nothing was reachable to inspect either: see the opt-out "
        "leg, where every /legal path this host serves answered HTTP 403 "
        "from Akamai. Recorded as no search surface on the product's "
        "nature rather than on the block, since even a fully open "
        "unity.com would carry no lookup: the personal data at issue is "
        "mobile advertising identifiers and SDK telemetry, keyed to a "
        "device rather than to a name."
    ),
    "upcell-io": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. Upcell sells a B2B contact "
        "database to sales teams (its own navigation is COMPARE / VS "
        "ZOOMINFO / VS APOLLO / VS COGNISM / VS LUSHA / VS ROCKETREACH / "
        "PRICING CALCULATOR / BOOK A DEMO) and exposes no public lookup: "
        "the only route into the data is 'REQUEST ACCESS' -> /contact. "
        "Rendering the rights page at a 14s settle found no search "
        "control anywhere on it. There is nothing here a presence check "
        "could query without becoming a customer."
    ),
    "uplead-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. UpLead sells a B2B prospecting "
        "database by subscription and states the scope itself on the "
        "rights page: 'UpLead only collects, uses, and shares Data about "
        "companies and professionals that work within companies.' There "
        "is no free lookup and no public profile page: the only routes "
        "are 'Request a Demo' and 'Start Free Trial'. Nothing to query "
        "for a presence check."
    ),
    "upscapital-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. UPS Capital Corporation sells "
        "shipping insurance and mortgage/title support services; the only "
        "form on either page rendered was the site's own content search "
        "(form#search-form, input[name=search_term], posting to /search). "
        "There is no consumer lookup product, and the personal data at "
        "issue reaches UPS Capital through shipping and claims activity "
        "rather than from the consumer, so there is nothing addressable "
        "to query for a presence check."
    ),
    "usdatacorporation-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. US Data Corporation sells "
        "marketing lists (its navigation is MARKETING LISTS & SOLUTIONS / "
        "GET A QUOTE) and prints its own FCRA disclaimer: 'US Data "
        "Corporation is not a Consumer Reporting Agency. It does not "
        "provide consumer or credit information...'. The only search "
        "control on the pages rendered is the WordPress Ajax Search Lite "
        "site search (input[name=phrase], plus its off-layout "
        "autocomplete twin explicitly labelled 'Autocomplete input, do "
        "not use this'). No consumer lookup exists to query."
    ),
    "usa-official-com": (
        "HOST UNRESOLVABLE -- and this is a LAME DELEGATION, not one of "
        "this machine's eight DNS blackholes, established by querying the "
        "authoritative servers directly on 2026-09-25. dig @1.1.1.1 usa- "
        "official.com returns SERVFAIL, and the resolver says why in an "
        "EDNS Extended DNS Error: 'at delegation usa-official.com.'. "
        "Asked upward, a.gtld-servers.net still delegates the zone to "
        "kurt.ns.cloudflare.com and hazel.ns.cloudflare.com -- and both "
        "of those nameservers answer REFUSED for it, which is what "
        "Cloudflare returns for a zone no longer in any account. So the "
        ".com delegation outlived the hosting: the domain was dropped "
        "from its Cloudflare account and nobody repointed it. dig +cd "
        "(DNSSEC validation disabled) returns nothing either, and curl "
        "cannot resolve the name. There is therefore no host to serve "
        "anything, and no search surface can exist. The row was already "
        "thin -- the dataset says the company name was 'inferred from "
        "domain' and the opt-out URL was 'discovered' -- so there is no "
        "operating company to look for elsewhere. DATASET DEFECT: a row "
        "whose only identifier is a domain that no longer resolves; "
        "flagged and not fixed."
    ),
    "usadata-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. USADATA sells marketing lists and "
        "audience data to businesses ('FREE ANALYSIS', Solutions / Data / "
        "Products / Industries) and the rendered rights page carries NO "
        "form at all -- the only controls are Squarespace navigation "
        "buttons and an 'Ask Nova anything...' AI chat textarea. There is "
        "no consumer lookup here to query for a presence check."
    ),
    "rrd-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. RRD is a commercial print and "
        "marketing-services company and Valassis is its direct-mail arm; "
        "the only form on either page rendered is the site's ASP.NET "
        "content search (form#form1 with __VIEWSTATE and "
        "input#txtSearch). There is no consumer lookup product: the "
        "personal data at issue is mailing-list membership, which is not "
        "queryable from outside."
    ),
    "valpak-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. Valpak is a coupon-mailer network; "
        "the surface is a savings/offer lookup keyed to a location, not "
        "to a person (the header control is labelled 'Find your "
        "savings'). There is no people search to query, and the personal "
        "data at issue is a mailing ADDRESS on a distribution list, which "
        "cannot be checked from outside -- which is also why the opt-out "
        "is an address-keyed unsubscribe rather than a listing removal."
    ),
    "veeva-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. Veeva Systems sells cloud software "
        "and data to the life-sciences industry; its consumer-facing data "
        "is about healthcare providers and professionals, reached through "
        "customer products rather than a public lookup. The only forms on "
        "the rendered rights page are the site's own content search "
        "(input[name=q] to /search-results and its mobile twin). Nothing "
        "to query for a presence check."
    ),
    "vendelux-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. Vendelux sells event-intelligence "
        "software to B2B marketing teams and says in its own policy that "
        "it 'collects information about businesses and business people'; "
        "the only routes into the data are 'Log In', 'Free Trial' and "
        "'Request a Demo'. The rendered policy page carries NO form and "
        "no search control of any kind. Nothing to query."
    ),
    "venntel-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. Venntel resells device-level "
        "location data; the identifier it holds is a Mobile Advertising "
        "ID, not a name, and the site offers no lookup of any kind "
        "(Solutions / Partners / Blog / About). Its own rights pages make "
        "the point for it: the opt-out form's required field is a MAID "
        "and the sibling page is 'Request Your Information' at /right-to- "
        "know-request. There is nothing a name-keyed presence check could "
        "query. Noted in passing, as a lead and not a finding: "
        "venntel.com and veraset.com resolve to the SAME address, "
        "198.202.211.1 -- two location-data brokers on one host. That is "
        "hosting, not proven common control, and it is written down for "
        "whoever looks at either row next."
    ),
    "venpath-net": (
        "THE WEBSITE IS GONE BUT THE MAILBOX IS NOT, and the two halves "
        "were checked separately on 2026-09-25. venpath.net's zone is "
        "live and hosted at Cloudflare (NS "
        "desi/dimitris.ns.cloudflare.com, SOA serial 2414475180) and the "
        "query returns NOERROR -- so this is not NXDOMAIN and not one of "
        "this machine's DNS blackholes -- but there is NO A record at the "
        "apex and none at www either, verified against dig @1.1.1.1 and "
        "8.8.8.8, and curl cannot resolve www.venpath.net. What the zone "
        "DOES still carry is a full Google Workspace MX set "
        "(aspmx.l.google.com and the four alt hosts), so mail to the "
        "domain is still accepted. No web surface therefore exists to "
        "search, and none would: VenPath brokered mobile app and location "
        "data keyed to device identifiers, with no consumer lookup. "
        "DATASET DEFECT: the recorded opt_out_url "
        "https://www.venpath.net/legal/opt-out points at a hostname with "
        "no address record; flagged and not fixed."
    ),
    "ventiveiq-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. VentiveIQ LLC sells identity- "
        "graph, automotive, new-mover, property and demographic APPEND "
        "data to agencies, brands, publishers, DSPs and SSPs -- its own "
        "contact form's 'Select Service' list enumerates thirty such "
        "products -- and there is no consumer lookup on the site. The "
        "only search control rendered is the WordPress theme's site "
        "search (form.pix-search-form, input[name=s]). Nothing to query "
        "for a presence check."
    ),
    "veraset-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. Veraset sells device-level "
        "mobility data ('We deliver device-level, privacy-safe location "
        "data at scale') and holds advertising identifiers rather than "
        "names -- its own do-not-sell form requires an Advertising ID and "
        "its copy says 'we do not directly collect any location data'. "
        "The site's only forms are a demo request and a newsletter "
        "signup. Nothing name-keyed exists to query. See venntel-com for "
        "the shared-IP observation (both resolve to 198.202.211.1), "
        "recorded as a lead rather than a finding."
    ),
    "verecor-com": (
        "DOMAIN SEIZED BY COURT ORDER -- the fourth instance of this in "
        "the dataset and the reason this row's 'DEFUNCT? NXDOMAIN "
        "2026-07-09' note is now wrong. verecor.com resolves again "
        "(Cloudflare, 104.21.57.41) and https://verecor.com/optout "
        "answers HTTP 200 with the title 'This Domain Has Been "
        "Transferred by Court Order -- Verecor.com' and a notice reading: "
        "'Atlas Data Privacy Corporation, et al. v. Radaris.com, et al., "
        "Superior Court of New Jersey, Law Division, Middlesex County, "
        "Docket No. MID-L-000847-24... Pursuant to a final judgment of "
        "the New Jersey Superior Court, the domain name verecor.com has "
        "been transferred to Atlas Data Privacy Corporation and is no "
        "longer under the control of its former operators', with the "
        "Daniel's Law background (suit filed 2024-02-08, amended "
        "complaint 2025-05-27, claims assigned by roughly 21,760 covered "
        "persons). ZERO forms, zero inputs, cap[] and widget[] empty: the "
        "page is a notice and nothing else. So there is no search "
        "surface, and there will not be one: the site is in the "
        "plaintiffs' hands, not the broker's. Note the dataset's other "
        "claim about this row -- 'suspected shared operator/engine with "
        "the TruePeopleSearch/FastPeopleSearch family (heuristic "
        "grouping)' -- is now positively CONTRADICTED: the judgment "
        "places verecor.com in the Radaris case, alongside radaris-com, "
        "rehold-com and peekyou-com, which this sweep has already "
        "recorded as seized in the same or a parallel Atlas action. "
        "DATASET DEFECT: both the defunct/NXDOMAIN note and the tps- "
        "family grouping are stale; flagged and not fixed."
    ),
    "verisk-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. Verisk sells risk analytics to "
        "insurers and reinsurers; Verisk Marketing Solutions (formerly "
        "Infutor) sells consumer identity data to businesses. Neither "
        "exposes a consumer lookup: the only form on either page rendered "
        "is the corporate site search (input[name=keyword] to /site- "
        "search/). Part of this row's data is FCRA-regulated per the CA "
...
SEARCH_BLOCKED = {
    ...
    "nebraskacourtrecords-us": (
        "Verified by browser render (Playwright, 12s settle) 2026-09-25, "
        "batch 31. Blocked by the same registration paywall as the rest "
        "of the CourtRecords.us network, and confirmed to be the SAME "
        "surface field-by-field rather than merely a similar one: "
        "nebraskacourtrecords.us carries the identical #nameSearchForm "
        "posting to /search/loading/ with the same controls in the same "
        "order -- firstName (required), lastName (required), city, a "
        "hidden off-layout 'state' input, TWELVE off-layout checkboxes "
        "all named 'check', and the Submit input. Title 'Nebraska Court "
        "Records | Nebraskacourtrecords.us'. cap[] and widget[] both "
        "empty on the search page. Same TrustArc consent overlay as the "
        "siblings (trustarc-lang-select, truste-repop-msg-close, truste- "
        "clear-cookie-msg-close, truste-consent-button 'AGREE & PROCEED', "
        "truste-consent-reject-all, truste-consent-required, truste-show- "
        "consent, truste-consent-close) and the same three UserWay "
        "accessibility buttons (uw-skip-to-main, uw-enable-visibility, "
        "uw-open-accessibility). Compared against the eight M-state "
        "siblings, louisianacourtrecords-us and the other seven N-state "
        "siblings probed in this same batch: identical. See "
        "californiacourtrecords-us for the full write-up of driving that "
        "flow to its end and getting search theatre -- an abbreviated "
        "name and a list of courts, no results, records gated behind "
        "account registration or a report purchase. Dismiss the TrustArc "
        "overlay first if rechecking."
    ),
    "nevadacourtrecords-us": (
        ...
        "of the CourtRecords.us network, and confirmed to be the SAME "
        "surface field-by-field rather than merely a similar one: "
        "nevadacourtrecords.us carries the identical #nameSearchForm "
        "posting to /search/loading/ with the same controls in the same "
        "order -- firstName (required), lastName (required), city, a "
        "hidden off-layout 'state' input, TWELVE off-layout checkboxes "
        "all named 'check', and the Submit input. Title 'Nevada Court "
        "Records | Nevadacourtrecords.us'. cap[] and widget[] both empty "
        "on the search page. Same TrustArc consent overlay as the "
        "siblings (trustarc-lang-select, truste-repop-msg-close, truste- "
        "clear-cookie-msg-close, truste-consent-button 'AGREE & PROCEED', "
        "truste-consent-reject-all, truste-consent-required, truste-show- "
        "consent, truste-consent-close) and the same three UserWay "
    ...
    "newhampshirecourtrecords-us": (
        ...
        "of the CourtRecords.us network, and confirmed to be the SAME "
        "surface field-by-field rather than merely a similar one: "
        "newhampshirecourtrecords.us carries the identical "
        "#nameSearchForm posting to /search/loading/ with the same "
        "controls in the same order -- firstName (required), lastName "
        "(required), city, a hidden off-layout 'state' input, TWELVE off- "
        "layout checkboxes all named 'check', and the Submit input. Title "
        "'New Hampshire Court Records | Newhampshirecourtrecords.us'. "
        "cap[] and widget[] both empty on the search page. Same TrustArc "
        "consent overlay as the siblings (trustarc-lang-select, truste- "
        "repop-msg-close, truste-clear-cookie-msg-close, truste-consent- "
        "button 'AGREE & PROCEED', truste-consent-reject-all, truste- "
        "consent-required, truste-show-consent, truste-consent-close) and "
        "the same three UserWay accessibility buttons (uw-skip-to-main, "
        "uw-enable-visibility, uw-open-accessibility). Compared against "
        "the eight M-state siblings, louisianacourtrecords-us and the "
        "other seven N-state siblings probed in this same batch: "
        "identical. See californiacourtrecords-us for the full write-up "
        "of driving that flow to its end and getting search theatre -- an "
        "abbreviated name and a list of courts, no results, records gated "
        "behind account registration or a report purchase. Dismiss the "
        "TrustArc overlay first if rechecking."
    ),
    "newjerseycourtrecords-us": (
        ...
        "of the CourtRecords.us network, and confirmed to be the SAME "
        "surface field-by-field rather than merely a similar one: "
        "newjerseycourtrecords.us carries the identical #nameSearchForm "
        "posting to /search/loading/ with the same controls in the same "
        "order -- firstName (required), lastName (required), city, a "
        "hidden off-layout 'state' input, TWELVE off-layout checkboxes "
        "all named 'check', and the Submit input. Title 'New Jersey Court "
        "Records | Newjerseycourtrecords.us'. cap[] and widget[] both "
        "empty on the search page. Same TrustArc consent overlay as the "
        "siblings (trustarc-lang-select, truste-repop-msg-close, truste- "
        "clear-cookie-msg-close, truste-consent-button 'AGREE & PROCEED', "
        "truste-consent-reject-all, truste-consent-required, truste-show- "
    ...
    "newmexicocourtrecords-us": (
        ...
        "of the CourtRecords.us network, and confirmed to be the SAME "
        "surface field-by-field rather than merely a similar one: "
        "newmexicocourtrecords.us carries the identical #nameSearchForm "
        "posting to /search/loading/ with the same controls in the same "
        "order -- firstName (required), lastName (required), city, a "
        "hidden off-layout 'state' input, TWELVE off-layout checkboxes "
        "all named 'check', and the Submit input. Title 'New Mexico Court "
        "Records | Newmexicocourtrecords.us'. cap[] and widget[] both "
        "empty on the search page. Same TrustArc consent overlay as the "
        "siblings (trustarc-lang-select, truste-repop-msg-close, truste- "
        "clear-cookie-msg-close, truste-consent-button 'AGREE & PROCEED', "
        "truste-consent-reject-all, truste-consent-required, truste-show- "
    ...
    "newyorkcourtrecords-us": (
        ...
        "of the CourtRecords.us network, and confirmed to be the SAME "
        "surface field-by-field rather than merely a similar one: "
        "newyorkcourtrecords.us carries the identical #nameSearchForm "
        "posting to /search/loading/ with the same controls in the same "
        "order -- firstName (required), lastName (required), city, a "
        "hidden off-layout 'state' input, TWELVE off-layout checkboxes "
        "all named 'check', and the Submit input. Title 'New York Court "
        "Records | Newyorkcourtrecords.us'. cap[] and widget[] both empty "
        "on the search page. Same TrustArc consent overlay as the "
        "siblings (trustarc-lang-select, truste-repop-msg-close, truste- "
        "clear-cookie-msg-close, truste-consent-button 'AGREE & PROCEED', "
        "truste-consent-reject-all, truste-consent-required, truste-show- "
    ...
    "northcarolinacourtrecords-us": (
        ...
        "of the CourtRecords.us network, and confirmed to be the SAME "
        "surface field-by-field rather than merely a similar one: "
        "northcarolinacourtrecords.us carries the identical "
        "#nameSearchForm posting to /search/loading/ with the same "
        "controls in the same order -- firstName (required), lastName "
        "(required), city, a hidden off-layout 'state' input, TWELVE off- "
        "layout checkboxes all named 'check', and the Submit input. Title "
        "'North Carolina Court Records | Northcarolinacourtrecords.us'. "
        "cap[] and widget[] both empty on the search page. Same TrustArc "
        "consent overlay as the siblings (trustarc-lang-select, truste- "
        "repop-msg-close, truste-clear-cookie-msg-close, truste-consent- "
        "button 'AGREE & PROCEED', truste-consent-reject-all, truste- "
    ...
    "northdakotacourtrecords-us": (
        ...
        "of the CourtRecords.us network, and confirmed to be the SAME "
        "surface field-by-field rather than merely a similar one: "
        "northdakotacourtrecords.us carries the identical #nameSearchForm "
        "posting to /search/loading/ with the same controls in the same "
        "order -- firstName (required), lastName (required), city, a "
        "hidden off-layout 'state' input, TWELVE off-layout checkboxes "
        "all named 'check', and the Submit input. Title 'North Dakota "
        "Court Records | Northdakotacourtrecords.us'. cap[] and widget[] "
        "both empty on the search page. Same TrustArc consent overlay as "
        "the siblings (trustarc-lang-select, truste-repop-msg-close, "
        "truste-clear-cookie-msg-close, truste-consent-button 'AGREE & "
        "PROCEED', truste-consent-reject-all, truste-consent-required, "
        "truste-show-consent, truste-consent-close) and the same three "
        "UserWay accessibility buttons (uw-skip-to-main, uw-enable- "
        "visibility, uw-open-accessibility). Compared against the eight "
        "M-state siblings, louisianacourtrecords-us and the other seven "
        "N-state siblings probed in this same batch: identical. See "
        "californiacourtrecords-us for the full write-up of driving that "
        "flow to its end and getting search theatre -- an abbreviated "
        "name and a list of courts, no results, records gated behind "
        "account registration or a report purchase. Dismiss the TrustArc "
    ...
    "ohiocourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. Blocked by the same registration paywall "
        "as the rest of the CourtRecords.us network, and confirmed the "
        "SAME surface control-by-control rather than merely a similar one "
        "at a 14s settle: ohiocourtrecords.us carries the identical "
        "#nameSearchForm posting to /search/loading/ with 17 controls in "
        "the same order -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named check, and the Submit input. Title Ohio "
        "Court Records | OhioCourtRecords.us. cap[] and widget[] both "
        "EMPTY on the search page. Same eleven loose controls as every "
        "sibling: three UserWay accessibility buttons (uw-skip-to-main, "
        "uw-enable-visibility, uw-open-accessibility), trustarc-lang- "
        "select, and the seven TrustArc consent buttons (truste-repop- "
        "msg-close, truste-clear-cookie-msg-close, truste-consent-button "
        "AGREE AND PROCEED, truste-consent-reject-all, truste-consent- "
        "required REJECT ALL, truste-show-consent Manage Settings, "
        "truste-consent-close). Compared field-for-field against the "
        "eight M-state siblings, the eight N-state siblings and "
        "louisianacourtrecords-us: identical, 17 controls and 11 loose "
        "both. See californiacourtrecords-us for the full write-up of "
        "driving that flow to its end and getting search theatre -- an "
        "abbreviated name and a list of courts, no results, records gated "
        "behind account registration or a report purchase."
    ),
    "oklahomacourtrecords-us": (
        ...
        "as the rest of the CourtRecords.us network, and confirmed the "
        "SAME surface control-by-control rather than merely a similar one "
        "at a 14s settle: oklahomacourtrecords.us carries the identical "
        "#nameSearchForm posting to /search/loading/ with 17 controls in "
        "the same order -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named check, and the Submit input. Title Oklahoma "
        "Court Records | OklahomaCourtRecords.us. cap[] and widget[] both "
        "EMPTY on the search page. Same eleven loose controls as every "
        "sibling: three UserWay accessibility buttons (uw-skip-to-main, "
        "uw-enable-visibility, uw-open-accessibility), trustarc-lang- "
        "select, and the seven TrustArc consent buttons (truste-repop- "
    ...
    "oregoncourtrecords-us": (
        ...
        "as the rest of the CourtRecords.us network, and confirmed the "
        "SAME surface control-by-control rather than merely a similar one "
        "at a 14s settle: oregoncourtrecords.us carries the identical "
        "#nameSearchForm posting to /search/loading/ with 17 controls in "
        "the same order -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named check, and the Submit input. Title Oregon "
        "Court Records | OregonCourtRecords.us. cap[] and widget[] both "
        "EMPTY on the search page. Same eleven loose controls as every "
        "sibling: three UserWay accessibility buttons (uw-skip-to-main, "
        "uw-enable-visibility, uw-open-accessibility), trustarc-lang- "
        "select, and the seven TrustArc consent buttons (truste-repop- "
    ...
    "theorg-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. Blocked by a hard account wall. theorg.com "
        "is a people directory -- it publishes org charts naming "
        "individuals and their job titles -- but its search is not "
        "public: GET theorg.com/search?q=john%20smith returns HTTP 200 "
        "having redirected to "
        "theorg.com/login?next=%2Fsearch%3Fq%3Djohn%2Bsmith, serving a "
        "login form (input name=email placeholder 'Enter work email', "
        "input type=password name=password, submit 'Log in') plus a "
        "second form offering 'Continue with passkey'. A WORK EMAIL is "
        "required, not any email. The site also runs reCAPTCHA v2 in "
        "explicit-render mode site-wide (cap[] carries "
        "api.js?onload=onloadcallback&render=explicit, widget[] carries "
        "grecaptcha-badge / grecaptcha-logo / grecaptcha-error / "
        "g-recaptcha-response, and both the anchor and bframe challenge "
        "frames are present with the VERIFY button in the bframe), so "
        "even the login is captcha-guarded. Registration wall plus "
        "captcha, in the same shape already recorded for the "
        "CourtRecords.us family."
    ),
    "pennsylvaniacourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. Blocked by the same registration paywall "
        "as the rest of the CourtRecords.us network, and confirmed the "
        "SAME surface control-by-control rather than merely a similar one "
        "at a 14s settle: pennsylvaniacourtrecords.us carries the "
        "identical #nameSearchForm posting to /search/loading/ with 17 "
        "controls in the same order -- firstName (required), lastName "
        "(required), city, a hidden off-layout state input, TWELVE off- "
        "layout checkboxes all named check, and the Submit input. Title "
        "'Pennsylvania Court Records | PennsylvaniaCourtRecords.us'. "
        "cap[] and widget[] both EMPTY on the search page. Same eleven "
        "loose controls as every sibling: three UserWay accessibility "
        "buttons (uw-skip-to-main, uw-enable-visibility, uw-open- "
        "accessibility), trustarc-lang-select, and the seven TrustArc "
        "consent buttons (truste-repop-msg-close, truste-clear-cookie- "
        "msg-close, truste-consent-button AGREE & PROCEED, truste- "
        "consent-reject-all, truste-consent-required REJECT ALL, truste- "
        "show-consent Manage Settings, truste-consent-close). Field-for- "
        "field identical to ohiocourtrecords-us, the eight M-state "
        "siblings, the eight N-state siblings and louisianacourtrecords- "
        "us: 17 controls and 11 loose both. See californiacourtrecords-us "
        "for the full write-up of driving the flow to its end and getting "
        "search theatre -- records gated behind account registration."
    ),
    "pbinfo-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. THE WHOLE HOST IS WALLED to a real "
        "browser. Three renders -- https://www.pbinfo.com/crf/ at a 14s "
        "settle, https://pbinfo.com/crf/ at 22s and https://pbinfo.com/ "
        "at 20s -- ALL returned HTTP 403 carrying an UNBRANDED bot-check "
        "interstitial: title 'Checking your browser...', body text "
        "exactly 'Confirm you are human / We need to check you are not a "
        "robot before you can enter this site. / I am human'. The "
        "mechanism is worth recording because the automated "
        "cap[]/widget[] detector reports CLEAN on it -- there is no "
        "vendor script at all. What it actually ships is a bank of ten "
        "unnamed checkboxes whose ids are FRESH RANDOM EIGHT-CHARACTER "
        "STRINGS ON EVERY LOAD (observed igqoo0j0, tkgu294h, mdfbfag8, "
        "0v9wqiyb, ptej80o9, r6gplpei, 5e89du5u, beg3x740, a9mkzk1x, "
        "5dpnoe5k on one load and c95w13gc, qtn34boa, uxgk8mt1, kyutupvc, "
        "w60j65gq on the next), nine of them computing to invisible and "
        "exactly one labelled 'I am human'. No stable name or id can "
        "address any of them. The block is UA/fingerprint-gated rather "
        "than IP-gated: plain curl to the same URLs gets HTTP 200 and the "
        "real WordPress page (host is Automattic/WordPress.com -- server- "
        "timing a8c-cdn, x-ac _atomic_bur, host-header wpcloud). A static "
        "read of the 200 confirms this is PBI Research Services, an "
        "institutional pension/death-audit locator with no consumer "
        "person lookup anywhere, so the search leg is blocked rather than "
        "merely unread -- but it is filed blocked, not no-surface, "
        "because a browser cannot see the site to confirm it."
    ),
    "precisely-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. THE ENTIRE HOST is behind a Cloudflare "
        "MANAGED CHALLENGE, and it did not clear on any attempt. FOUR "
        "renders -- https://www.precisely.com/your-privacy-choices at "
        "16s, again at 22s, again at 22s, and https://www.precisely.com/ "
        "at 20s -- ALL returned HTTP 403 with title 'Just a moment...', "
        "~265 characters, body 'Performing security verification / This "
        "website uses a security service to protect against malicious "
        "bots', and a DIFFERENT Ray ID each time (a40d8fd41d20f1aa, "
        "a40d955e09d1f1aa, a40d9a91ea77f1aa among them). Naming the shape "
        "as required: this is the SOLVABLE managed-challenge form, not a "
        "terminal WAF 403 -- cap[] carries "
        "challenges.cloudflare.com/turnstile/v0/b/d76008a69eab/api.js and "
        "a hidden cf-turnstile-response input is present (cf-chl-widget-* "
        "per-load suffix) -- but four visits over ten minutes never got "
        "past it, so the surface behind it is unread. Because the wall "
        "covers the homepage too, the search leg is blocked rather than "
        "no-surface: this row (PLACEIQ Inc, now folded into Precisely) is "
        "a B2B location/data-enrichment vendor and almost certainly has "
        "no consumer lookup, but that cannot be confirmed through the "
        "challenge and will not be asserted."
    ),
    "prodpro-com": (
        "Verified by browser render 2026-09-25, batch 34. prodpro.com "
        "(the apex) returns HTTP 403 with title 'Checking your "
        "browser...' and an UNBRANDED self-hosted checkbox captcha: SIX "
        "separate checkbox inputs all labelled 'I am human', each with a "
        "freshly minted eight-character random id (this load: ogqmxdbw, "
        "li8ly430, e9psj37h, u8mbubby, l91h1ysk, 6gu7wv8r), no names, no "
        "form wrapper, and cap[] / widget[] both EMPTY -- no reCAPTCHA, "
        "hCaptcha, Turnstile or BotDetect script anywhere. This is the "
        "second confirmed instance of the 'unbranded 403 with N unnamed "
        "checkboxes on per-load random ids' family. The wall is on the "
        "apex only: app.prodpro.com serves the privacy policy and the "
        "opt-out form without challenge (see the opt-out entry). ProdPro "
        "sells TV/film production-crew intelligence; there is no consumer "
        "search surface behind the wall either, but the wall is what was "
        "actually observed, so this is blocked rather than no-surface."
    ),
    "publicsearcher-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.publicsearcher.com redirects to /nameSearch/landingPage, "
        "HTTP 200, title 'Publicsearcher', 4833 characters. A real NAME- "
        "KEYED search exists and is fully transcribed, but it is walled "
        "twice over on every visit, so this is blocked rather than "
        "undecided. WALL 1, CLOUDFLARE TURNSTILE, and the automated "
        "detector MISSES IT: the page reports widget[] EMPTY because the "
        "widget is not in the main document at all -- it lives in a SELF- "
        "HOSTED iframe at https://www.publicsearcher.com/assets/common/ca "
        "ptcha/turnstile.html?sitekey=0x4AAAAAAAGID3AJyMcEY0ET, and only "
        "cap[] inside that child frame shows "
        "challenges.cloudflare.com/turnstile/v0/api.js?render=explicit "
        "plus hidden input name=cf-turnstile-response id=cf-chl- "
        "widget-<random>_response. A self-hosted wrapper page for the "
        "Turnstile widget is a new evasion of widget[] detection worth "
        "recording. WALL 2: a full-page 'Notice' interstitial requiring "
        "an 'I AGREE' click, backed by an off-layout invisible checkbox "
        "id=privacy-agree-checkbox, with the standard non-FCRA "
        "certification text. The form itself, duplicated twice in the DOM "
        "(mobile and desktop): id=email-form (Webflow leftover id), class "
        "'advanced-search-form ng-untouched ng-pristine', method GET, "
        "action https://www.publicsearcher.com/nameSearch/landingPage -- "
        "input text name=First-Name id=First-Name ('Enter First Name'), "
        "input text name=Last-Name id=Last-Name ('Enter Last Name'), "
        "SELECT name=state id=state with 53 options (first 'Select "
        "States', and note 'All States' sorted between Alaska and "
        "Arizona), submit BUTTON 'FREE SEARCH'. Angular ng-* classes on a "
        "Webflow shell. ENTITY FINDING: the footer's 'Do Not Sell My "
        "Info' and 'Limit The Use of My Sensitive Personal Info' both "
        "point at /api/helper/optOutLight/search -- the same optOutLight "
        "platform confirmed in batch 33 on peoplesearch123.com, "
        "peoplesearchusa.org and personsearchers.com. This is the FOURTH "
        "brand on that one operator's platform; see the opt-out entry."
    ),
    "rhodeislandcourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. The last unmapped member of the "
        "CourtRecords.us network, and confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one. "
        "rhodeislandcourtrecords.us renders 'Rhode Island Court Records | "
        "Rhode IslandCourtRecords.us' carrying the identical "
        "#nameSearchForm posting to /search/loading/ with 17 controls in "
        "the same order as every sibling: firstName (required), lastName "
        "(required), city, a hidden off-layout state input, TWELVE off- "
        "layout checkboxes all named 'check', and the Submit input. cap[] "
        "and widget[] are both EMPTY on the search page. The same eleven "
        "loose controls as the siblings: three UserWay accessibility "
        "buttons ('Skip to main content', 'Enable accessibility for low "
        "vision', 'Open the accessibility menu'), a language select, two "
        "'close alert' buttons and the consent buttons AGREE & PROCEED / "
        "Reject All / REJECT ALL / Manage Settings / Close. Filed blocked "
        "on the network's established verdict -- results are behind the "
        "registration paywall the sibling entries document (the page "
        "sells 'Instant Access to Civil and Criminal Court Records' and "
        "carries a LOGIN link) -- and not re-submitted, since a "
        "reconnaissance pass must not submit."
    ),
    "richmediallc-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. TERMINAL CLOUDFLARE WAF 403, host- "
        "wide, and NOT a solvable managed challenge -- naming the shape "
        "explicitly because the two Cloudflare cases behave differently. "
        "richmediallc.com serves HTTP 403 titled 'Attention Required! | "
        "Cloudflare' reading 'Sorry, you have been blocked / You are "
        "unable to access secureservercdn2.net' with a Ray ID and a "
        "'Click to reveal' IP button -- there is no 'Just a moment...' "
        "interstitial, no turnstile, no challenge to pass, so retrying "
        "for a fresh Ray ID is pointless. Confirmed it is the whole host, "
        "not one path: / also returns 403 to a normal desktop UA. DNS is "
        "fine and was checked both ways against the local resolver and "
        "1.1.1.1 (160.153.0.32, GoDaddy) -- so this is NOT the local "
        "0.0.0.0 blackhole false negative and NOT a dead host. Filed "
        "blocked rather than no-surface: the host refuses this client "
        "outright, so whether a search surface exists behind it cannot be "
        "determined, and 'no surface' would be a claim the evidence does "
        "not support."
    ),
    "spectrummailinglists-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. TERMINAL CLOUDFLARE WAF 403, HOST- "
        "WIDE, and the not-a-challenge shape: "
        "spectrummailinglists.com/privacy-statement/ returns HTTP 403 "
        "titled 'Attention Required! | Cloudflare' reading 'Sorry, you "
        "have been blocked ... You are unable to access "
        "secureservercdn2.net' with Ray ID a40e604f7c36f1aa, and the bare "
        "homepage returns 403 to a normal-UA curl as well. There is no "
        "interactive challenge to solve and no Turnstile -- retrying for "
        "a fresh Ray ID is pointless, unlike the solvable 'Just a "
        "moment...' shape. Whatever search surface Spectrum Mailing Lists "
        "has cannot be reached from here at all."
    ),
    "speedeondata-com": (
        "Verified by browser render 2026-09-25, batch 37 (Playwright, "
        "throwaway long-settle wrapper importing JS/UA from "
        "tools/probe_broker_forms.py, 18s settle). TERMINAL CLOUDFLARE "
        "WAF 403, host-wide, and worth recording precisely because this "
        "broker's two legs sit behind two DIFFERENT walls on two "
        "different hosts. speedeondata.com itself answers 403 with "
        "Cloudflare's 'Sorry, you have been blocked / You are unable to "
        "access secureservercdn2.net' interstitial (Ray ID "
        "a40ea134c96cf1aa, 697 characters, zero forms). This is NOT the "
        "solvable 'Just a moment...' managed challenge -- there is no "
        "challenge to solve, no turnstile, no retry path; a plain curl "
        "with an ordinary desktop UA gets the same 403, so it is not a "
        "headless-browser tell. Nothing about a marketing-data company "
        "suggests a consumer people-search surface behind it either, but "
        "the honest statement is that the wall prevents confirming that, "
        "so this is blocked rather than no-surface. The opt-out leg is "
        "reachable and is recorded separately: optout.speedeondata.com is "
        "a wholly separate Azure-hosted host (spd-ccpa- "
        "app.azurewebsites.net) that answers 200."
    ),
    "swordfish-ai": (
        "Verified by browser render 2026-09-25, batch 37, and confirmed "
        "across TWO renders at 18s and 25s settle plus a separate "
        "homepage render: SITE-WIDE CLOUDFLARE TURNSTILE INTERSTITIAL "
        "that never clears. Both https://swordfish.ai/ and "
        "https://swordfish.ai/policy-ccpa return HTTP 200 with an EMPTY "
        "<title>, a body of exactly 23 characters reading 'Checking for "
        "any bots ...', zero forms, "
        "challenges.cloudflare.com/turnstile/v0/api.js loaded, a .cf- "
        "turnstile widget and a hidden cf-turnstile-response input (cf- "
        "chl-widget-*). Distinguish the shape carefully: this is not "
        "Cloudflare's classic 'Just a moment...' managed challenge and "
        "not a terminal WAF 403 -- it is an explicit Turnstile bot-check "
        "page standing in front of the whole origin, and it is only "
        "visible as such because the widget/cap inspection reaches into "
        "it. THE DATASET'S CLAIM IS FALSIFIED IN SUBSTANCE: the row's "
        "2026-09-22 note records swordfish.ai/policy-ccpa as 'live "
        "(200)', but the 200 is the bot wall, not the CCPA notice. "
        "Nothing behind it can be read, so the search leg (Swordfish "
        "sells a contact-finder over emails and phone numbers, which "
        "would be the surface) cannot be assessed."
    ),
    "sealedrecords-net": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. SIXTH AND SEVENTH MEMBERS OF A "
        "KNOWN WHITE-LABEL FAMILY SETTLED TOGETHER (see also secretinfo- "
        "org and truthrecord-org in this batch): this is the optOutLight "
        "white-label engine already established under backgroundcheckers- "
        "net, checksecrets-com, privatereports-com, mugshotlook-com, "
        "inmatessearcher-com, peoplesearch123-com and publicsearcher-com, "
        "and the identity is again structural rather than a resemblance. "
        "The dataset's opt_out_url, "
        "www.sealedrecords.net/optOut/name/landing, 301s to the SEARCH "
        "page /name/landing -- see the opt-out leg -- and that page "
        "serves the family's FCRA notice word for word ('Learning the "
        "truth about the history of your family and friends can be "
        "shocking...'), the same submit reading 'I AGREE' beside a "
        "computed-invisible #privacy-agree-checkbox, and links the "
        "removal flow twice from the footer as 'Do Not Sell Or Share My "
        "Personal Info' and 'Limit The Use of My Sensitive Personal "
        "Info', both pointing at /api/helper/optOutLight/search. Search "
        "form transcription: form#email-form (Angular, class carries ng- "
        "untouched/ng-pristine), method GET, action /name/landing; "
        "input[type=text][name=First-Name][id=First-Name] placeholder "
        "'Enter person's First Name'; input[type=text][name=Last- "
        "Name][id=Last-Name]; select[name=field][id=field] with 53 "
        "options ('All States', then the states, with 'All States' "
        "duplicated); submit button.landing-search-submit reading 'FREE "
        "SEARCH'. BLOCKED BY CLOUDFLARE TURNSTILE, exactly as on "
        "checksecrets-com and backgroundcheckers-net: cap[] and widget[] "
        "are EMPTY on the main frame, and the wall only shows up in the "
        "frame list -- a child frame at /assets/common/captcha/turnstile. "
        "html?sitekey=0x4AAAAAAAGIDOWi3uKHrRJK loading "
        "challenges.cloudflare.com/turnstile/v0/api.js?render=explicit. "
        "Per-tenant sitekey, shared path. Nothing was submitted."
    ),
    "searchquarry-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. A real people-search surface "
        "behind an invisible reCAPTCHA and a members paywall. "
        "www.searchquarry.com renders 'SearchQuarry - Background Checks, "
        "License Plate Searches, & More' (4299 characters) with FIVE "
        "forms, of which the people one is form#searchform_people, class "
        "'inl namesearch tab', method GET, action /namesearch/search: "
        "input[name=fname] 'First Name' REQUIRED; input[name=lname] 'Last "
        "Name' REQUIRED; input[name=city] 'City'; select[name=state] "
        "REQUIRED with 52 options ('Select State' then the states); "
        "select[name=recordtype] REQUIRED with 16 options (Record Type, "
        "Arrest, Background, Bankruptcy, Birth, Court, Criminal, Death, "
        "Divorce, Driving, Incarceration, Lien, Marriage, Police, Traffic "
        "Citation, Warrant); a hidden input[name=trackstat]; submit "
        "button.bigbutton 'SEARCH'. The sibling tabs are "
        "#searchform_phone (phone), #searchform_license_plate (plate + "
        "state, with hidden sqtb/trackstat/origin) and #searchform_vin, "
        "all off-layout until their tab is picked. BLOCKED, for two "
        "reasons that stack: (1) an INVISIBLE reCAPTCHA is live on the "
        "page -- recaptcha.net/recaptcha/api.js?render=explicit, sitekey "
        "6LccU3keAAAAACyl2Gcbijd7IA9GGi59IGw8MYw2, an api2/anchor frame "
        "at size=invisible and a loose textarea#g-recaptcha-response -- "
        "so the submit is scored, not free; and (2) results live on the "
        "separate members.searchquarry.com host behind JOIN/LOGIN, and a "
        "loose button.btn-primary reading 'Accept Terms' gates the page "
        "first. Nothing was submitted."
    ),
    "secretinfo-org": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Another member of the optOutLight "
        "white-label engine already established under backgroundcheckers- "
        "net, checksecrets-com, privatereports-com, mugshotlook-com, "
        "inmatessearcher-com, peoplesearch123-com and publicsearcher-com, "
        "settled in the same pass as sealedrecords-net and truthrecord- "
        "org. The dataset's opt_out_url "
        "www.secretinfo.org/optOut/name/landing 301s to the SEARCH page "
        "/name/landing, which serves the family's FCRA notice word for "
        "word and the same 'I AGREE' submit beside a computed-invisible "
        "#privacy-agree-checkbox, and links "
        "/api/helper/optOutLight/search twice from the footer. THIS "
        "TENANT IS THE CUSTOM-CONTROL BLIND SPOT IN MINIATURE AND IS "
        "WORTH THE WARNING: unlike sealedrecords-net there is NO <form> "
        "element on the page at all, so document.querySelectorAll('form') "
        "returns nothing and a form-only reader would call this page "
        "empty. The controls are all loose: "
        "input[type=text][id=firstName] 'First name' (Angular, class "
        "form-control ng-untouched ng-pristine, NO name attribute), "
        "input[type=text][id=lastName] 'Last name' (likewise no name), "
        "select[name=state][id=state] with 53 options, and submit "
        "button#perform-search.btn-primary reading 'FREE SEARCH', plus an "
        "off-layout close button. Same engine, different skin (Bootstrap "
        "here, Webflow classes on sealedrecords-net), which is the per- "
        "tenant variation checksecrets-com already warned about. BLOCKED "
        "BY CLOUDFLARE TURNSTILE, again visible only in the frame list "
        "while cap[] and widget[] are EMPTY: a child frame at /assets/com "
        "mon/captcha/turnstile.html?sitekey=0x4AAAAAAAGICzf9e3D7qlhB "
        "loading "
        "challenges.cloudflare.com/turnstile/v0/api.js?render=explicit. "
    ...
    "southcarolinacourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "southcarolinacourtrecords.us renders 'South Carolina Court "
        "Records | SouthCarolinaCourtRecords.us' carrying the identical "
        "#nameSearchForm posting to /search/loading/ with 17 controls in "
        "the same order as every sibling -- firstName (required), "
        "lastName (required), city, a hidden off-layout state input, "
        "TWELVE off-layout checkboxes all named 'check', and the Submit "
        "input. cap[] and widget[] are both EMPTY on the search page, and "
        "the same eleven loose controls as the siblings are present "
        "(three UserWay accessibility buttons, the TrustArc language "
        "select, two 'close alert' buttons and AGREE & PROCEED / Reject "
        "All / REJECT ALL / Manage Settings / Close). Filed blocked on "
        "the network's established verdict -- results sit behind the "
        "registration paywall the sibling entries document, the page "
        "sells 'Instant Access to Civil and Criminal Court Records' and "
        "carries a LOGIN link -- and NOT re-submitted, since a "
        "reconnaissance pass must not submit. See californiacourtrecords- "
        "us for the full write-up."
    ),
    "southdakotacourtrecords-us": (
        ...
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "southdakotacourtrecords.us renders 'South Dakota Court Records | "
        "SouthDakotaCourtRecords.us' carrying the identical "
        "#nameSearchForm posting to /search/loading/ with 17 controls in "
        "the same order as every sibling -- firstName (required), "
        "lastName (required), city, a hidden off-layout state input, "
        "TWELVE off-layout checkboxes all named 'check', and the Submit "
    ...
    "tennesseecourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "tennesseecourtrecords.us renders 'Tennessee Court Records | "
        "TennesseeCourtRecords.us' carrying the identical #nameSearchForm "
        "posting to /search/loading/ with 17 controls in the same order "
        "as every sibling -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named 'check', and the Submit input. cap[] and "
        "widget[] are both EMPTY on the search page, and the same eleven "
        "loose controls as the siblings are present (three UserWay "
        "accessibility buttons, the TrustArc language select, two 'close "
        "alert' buttons and AGREE & PROCEED / Reject All / REJECT ALL / "
        "Manage Settings / Close). Filed blocked on the network's "
        "established verdict -- results sit behind the registration "
        "paywall the sibling entries document, the page sells 'Instant "
    ...
    "texascourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "texascourtrecords.us renders 'Texas Court Records | "
        "TexasCourtRecords.us' carrying the identical #nameSearchForm "
        "posting to /search/loading/ with 17 controls in the same order "
        "as every sibling -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named 'check', and the Submit input. cap[] and "
    ...
    "unmask-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle, its own single-target prober invocation because the URL "
        "contains optout/opt-out/remove) 2026-09-25, batch 40. TWO "
        "DIFFERENT BOT WALLS ON ONE SITE, which is why this is recorded "
        "carefully. https://unmask.com/ answers HTTP 200 with the title "
        "'Radware Page' and the body 'Verifying your browser before "
        "proceeding... Incident ID: 43227f39-...' -- a Radware Bot "
        "Manager interstitial, zero forms, zero controls -- so the "
        "homepage search box was never served. A search surface certainly "
        "exists: the site's own 404 page (reached at /optout, see the "
        "opt-out leg) rendered normally and carried the real search form "
        "twice over, a Next.js loose-control set of "
        "input[name=firstName], input[name=lastName], input[name=city] "
        "and a 52-option select[name=state] under button.tz-search__btn- "
        "search, with NO <form> element and NO captcha on it. So the "
        "finding is that this broker's search is walled at the front door "
        "rather than absent, and that a recipe would additionally have to "
        "cope with a formless JS-driven search whose results URL was "
        "never observed. Filed blocked, not undecided, because the wall "
        "is the reason the leg cannot be resolved."
    ),
    "usmarketinggrp-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. CLOUDFLARE 403 ON THE ONLY URL "
        "THIS ROW HAS. Rendering the recorded page answered HTTP 403 "
        "titled 'Attention Required! | Cloudflare' whose body reads "
        "'Sorry, you have been blocked / You are unable to access "
        "secureservercdn2.net' with a Ray ID -- note the hostname in the "
        "block page: the site is fronted by a GoDaddy CDN origin, and the "
        "challenge names that origin rather than usmarketinggrp.com. Zero "
        "forms, zero loose controls other than Cloudflare's own 'Click to "
        "reveal' button. Nothing of the broker's own markup was served, "
        "so no statement can be made about whether a search surface "
        "exists; filed blocked, which is the honest shape of a wall on "
        "every visit."
    ),
    "utahcourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40.  Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "utahcourtrecords.us renders 'Utah Court Records | "
        "UtahCourtRecords.us' carrying the identical #nameSearchForm "
        "posting to /search/loading/ with 17 controls in the same order "
        "as every sibling -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named 'check', and the Submit input. cap[] and "
    ...
    "vermontcourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40.  Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "vermontcourtrecords.us renders 'Vermont Court Records | "
        "VermontCourtRecords.us' carrying the identical #nameSearchForm "
        "posting to /search/loading/ with 17 controls in the same order "
        "as every sibling -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named 'check', and the Submit input. cap[] and "
    ...
    "virginiacourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40.  Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "virginiacourtrecords.us renders 'Virginia Court Records | "
        "VirginiaCourtRecords.us' carrying the identical #nameSearchForm "
        "posting to /search/loading/ with 17 controls in the same order "
        "as every sibling -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named 'check', and the Submit input. cap[] and "
    ...
    "washingtoncourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40.  Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "washingtoncourtrecords.us renders 'Washington Court Records | "
        "WashingtonCourtRecords.us' carrying the identical "
        "#nameSearchForm posting to /search/loading/ with 17 controls in "
        "the same order as every sibling -- firstName (required), "
        "lastName (required), city, a hidden off-layout state input, "
        "TWELVE off-layout checkboxes all named 'check', and the Submit "
    ...
    "westvirginiacourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40.  Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "westvirginiacourtrecords.us renders 'West Virginia Court Records "
        "| WestVirginiaCourtRecords.us' carrying the identical "
        "#nameSearchForm posting to /search/loading/ with 17 controls in "
        "the same order as every sibling -- firstName (required), "
        "lastName (required), city, a hidden off-layout state input, "
        "TWELVE off-layout checkboxes all named 'check', and the Submit "
    ...
    "wisconsincourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40.  Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "wisconsincourtrecords.us renders 'Wisconsin Court Records | "
        "WisconsinCourtRecords.us' carrying the identical #nameSearchForm "
        "posting to /search/loading/ with 17 controls in the same order "
        "as every sibling -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named 'check', and the Submit input. cap[] and "
    ...
    "wyomingcourtrecords-us": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40.  Another member of the "
        "CourtRecords.us network, confirmed to be the SAME surface "
        "control-by-control rather than merely a similar one: "
        "wyomingcourtrecords.us renders 'Wyoming Court Records | "
        "WyomingCourtRecords.us' carrying the identical #nameSearchForm "
        "posting to /search/loading/ with 17 controls in the same order "
        "as every sibling -- firstName (required), lastName (required), "
        "city, a hidden off-layout state input, TWELVE off-layout "
        "checkboxes all named 'check', and the Submit input. cap[] and "
...
SEARCH_UNDECIDED = {
    ...
    "nelists-com": (
        "NOT RESOLVED, and the reason is a TOOLING FAILURE rather than "
        "anything about the site, recorded 2026-09-25, batch 31 so the "
        "next agent does not spend the same time. nelists.com RENDERS "
        "FINE TO A PLAIN CLIENT -- curl gets HTTP 200 and a 189,144-byte "
        "body in 2.0s -- but the Playwright prober HUNG TWICE on it, once "
        "at a 12s settle and once at a 20s settle, in both cases blowing "
        "the whole per-target wall-clock budget (92s, then 100s) rather "
        "than failing in goto. Per the standing one-retry rule for hangs "
        "(the faraday.io shape) it was not tried a third time. THE LIKELY "
        "CULPRIT, from the static body: the page is a Divi/WordPress "
        "build that embeds a google.com/maps/embed iframe, and the prober "
        "walks every child frame with frame.evaluate, which accepts no "
        "timeout. WHAT IS KNOWN: title 'CRM Data Scrubbing, Cleaning, "
        "Integrity, Deduplication & Enhancement Services, Phone Scrubbing "
        "Colorado Springs CO - New England List Services, Inc.', one "
        "<form> element in the served HTML. Per the standing rule, NO "
        "claim is made here about whether that form is a search surface "
        "or a bot check -- a static body is not a render. WHAT WOULD "
        "FINISH IT: probe with the child-frame walk skipped, or with a "
        "per-frame timeout, and read the rendered DOM."
    ),
    "nosis-com": (
        "REAL SEARCH SURFACE, transcribed, not resolved -- verified by "
        "browser render (Playwright, 12s settle) 2026-09-25, batch 31. "
        "Nosis is an Argentine commercial/credit bureau and "
        "www.nosis.com/es DOES carry a public person/company lookup, but "
        "it is easy to miss two ways and both are worth recording. (1) IT "
        "IS NOT IN A FORM. The search control is a LOOSE input -- outside "
        "any <form> element, so a form-keyed scan reports zero forms on "
        "this page -- and it has NO name and NO id at all, only the "
        "placeholder 'Ingrese un Nombre o Razon Social o DNI o CUIT'. "
        "Alongside it sit an anonymous submit BUTTON and a hidden "
        "input[name=hdnUrlReferer]. This is the nameless-field hazard "
        "again (cf. publicdatacheck-com's hashed field name): any recipe "
        "keyed on field names would silently skip the only input that "
        "matters, and the submission is JS-driven, not a plain POST. (2) "
        "IT IS A PAYWALL, and a steep one -- the page copy next to "
        "'BUSQUEDA DE INFORMES COMERCIALES / BUSCAR INFORME' quotes 1 "
        "Informe at AR$21,195 including IVA (AR$17,517 ex-IVA), or a "
        "monthly subscription, and the site is otherwise gated behind "
        "INICIAR SESION. So a search gets you a purchase page, not a "
        "record. Left undecided rather than blocked because the wall is "
        "commercial, not anti-bot: cap[] and widget[] are both EMPTY on "
        "the home page. The searchable population is roughly 40M "
        "Argentines ('NOSIS BIG DATA', 'Big Data Personas')."
    ),
    "okcaller-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. RECIPE-READY IN SUBSTANCE, filed undecided "
        "only because this tool cannot construct a SearchRecipe object. "
        "OKCaller is a live reverse-phone directory and the lookup is a "
        "plain GET with no bot check anywhere. Homepage form: no id, "
        "class 'rounded position-relative', method GET, action "
        "https://okcaller.com/detail.php, ONE control -- input type=tel "
        "name=number, required, label from placeholder 'Lookup Phone "
        "Number' -- plus an unnamed submit button. cap[] and widget[] "
        "both EMPTY at a 14s settle; the only loose control is the "
        "Bootstrap navbar-toggler. Driven to its end against the "
        "fictional-range number 2125551212: detail.php?number=2125551212 "
        "returns HTTP 200 and a FULL result page, 7752 characters, title "
        "'Accurate Caller ID for 2125551212 in New York City:' -- no "
        "login wall, no captcha, no paywall interstitial. So the recipe "
        "is GET https://okcaller.com/detail.php?number=<10 digits>, "
        "result keyed by phone number rather than by name, which is worth "
        "flagging: this repo's search profiles are name-keyed and a "
        "phone-keyed search may not fit the existing SearchRecipe shape "
        "at all. The result page also carries two further forms that a "
        "driver must NOT touch: a vote.php GET form (textarea "
        "name=comments plus two submit buttons both named hello, values "
        "Safe / Not Safe) which would write a public comment on the "
        "number, and an editprofile.php GET form (hidden name=phone, "
        "submit buttons named hello reading 'Send SMS PIN' and 'Call Me') "
        "which is a claim-this-number flow that would place a call or "
        "text to the number. Both belong in forbidden_selectors. Front- "
        "page copy also advertises 'Login with Google to instantly view "
        "real-time Caller Name ID', so some enrichment is account-gated "
        "even though the base lookup is not. Operated by Greenflight "
        "Venture Corp."
    ),
    "ownerly-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 32. A real, captcha-free search surface exists "
        "but it is ADDRESS-keyed, not name-keyed, so it does not fit this "
        "repo's search shape and is left undecided with a full "
        "transcription rather than called either way. www.ownerly.com "
        "renders at a 14s settle (title 'House Values & Property Data | "
        "Ownerly', 1595 characters) with cap[] and widget[] both EMPTY "
        "and THREE forms that are the same search repeated for three page "
        "positions -- each class js-search-form, method GET, action "
        "https://www.ownerly.com/, each holding exactly one input "
        "type=text name=address plus a submit: the hero form (input "
        "required, no placeholder), #secondary-search (placeholder "
        "'Search your address', not required) and #tertiary-search "
        "(placeholder 'Enter a Home Address', not required). Two loose "
        "toggle buttons reading ADDRESS and CITY OR ZIP switch what the "
        "single address field means, which is the only way to reach a "
        "non-address query and is driven by JS rather than by a form "
        "field. There is NO person-name search on the public site: "
        "Ownerly is the property-data brand of the BeenVerified family "
        "(with PeopleLooker and NeighborWho) and its public entry point "
        "is keyed on a house, not a person. What the results page does -- "
        "whether it paywalls, registers or challenges -- was NOT "
        "determined, because the same origin serves a Cloudflare managed "
        "challenge on its /app/ and /svc/ paths (see the opt-out entry), "
        "so a result render could not be trusted to be the public "
        "behaviour. Left undecided on that basis."
    ),
    "peoplebyname-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. A real, captcha-free search surface exists "
        "and is fully transcribed below, but it is PHONE-KEYED, not name- "
        "keyed, so it does not fit this repo's SearchRecipe shape and is "
        "left undecided rather than called either way. "
        "www.peoplebyname.com renders HTTP 200 at a 14s settle, title "
        "'Reverse Phone Lookup - Identify Any Phone Number Owner | "
        "PeopleByName', 5160 characters, cap[] and widget[] both EMPTY, "
        "ZERO loose controls. ONE form, id searchform, method POST, "
        "action https://www.peoplebyname.com/search.php, with exactly two "
        "controls: input type=tel name=phone id=bnum (not marked "
        "required; label resolved from the sibling 'Search Form' legend, "
        "placeholder example on the page is '555-555-5555'), and a submit "
        "BUTTON class search_box_cta with text 'Search'. Site copy: "
        "'Search Over 1.3 Billion Phone Records - US & Canada', self- "
        "described as running since 2008. DATASET NOTE: the row's note "
        "calls this a 'Phone/name directory'; the live homepage offers "
        "ONLY the reverse-phone lookup -- no name box of any kind. THE "
        "OPEN QUESTION this row raises is not about the site, it is about "
        "the repo: a phone-keyed lookup cannot be expressed as a name- "
        "keyed SearchRecipe, and presence detection for a phone number is "
        "a different question from presence detection for a person. "
        "Flagged explicitly rather than forced into the existing shape."
    ),
    "peoplefinders-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. RECIPE-READY IN SUBSTANCE, filed undecided "
        "only because this tool cannot construct SearchRecipe objects and "
        "because the results funnel was not driven. www.peoplefinders.com "
        "renders HTTP 200 at a 14s settle, title 'PeopleFinders', cap[] "
        "and widget[] both EMPTY on the homepage. ONE form, id pf- "
        "searchPanel_form, class 'headerBar__search-panel__panel "
        "mainPanel', method GET, action https://www.peoplefinders.com/ -- "
        "i.e. a querystring search on the broker's own host. Its 14 "
        "controls, in order: BUTTON id pfAdvancedToggle ('ADVANCED "
        "SEARCH'); input text name=first_name (label First Name); input "
        "text name=middle_name (Middle Initial or Name (Optional), "
        "computes invisible until Advanced is toggled); input text "
        "name=last_name (Last Name); input text name=city (City "
        "(Optional)); SELECT name=state, 53 options, first 'State "
        "(Optional)'; input text name=age (Age or Age Range (ex. 35 or "
        "25-35), invisible until Advanced); BUTTON id pfBtnNameSearch "
        "class 'btn-success disabled' text Search; then three sibling "
        "keys on the same form -- input text name=phone with BUTTON id "
        "pfBtnPhoneSearch, input text name=email with BUTTON id "
        "pfBtnEmailSearch, input text name=address_query class pf- "
        "address-single-input with BUTTON id pfBtnAddressSearch. EVERY "
        "control reports off-layout because the whole panel lives behind "
        "the header's 'Search' nav trigger (a loose BUTTON class pf- "
        "nav__trigger, text Search), which must be clicked before the "
        "panel is on-layout -- that click, and the unknown shape of the "
        "results funnel, are the two things standing between this and a "
        "recipe."
    ),
    "peoplelooker-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. RECIPE-READY IN SUBSTANCE for the name "
        "leg, filed undecided only because this tool cannot construct "
        "SearchRecipe objects and because the BeenVerified-family results "
        "funnel (see beenverified-com's own SEARCH_UNDECIDED entry for "
        "why that funnel is the blocker) was not driven. "
        "www.peoplelooker.com renders HTTP 200 at a 14s settle, title as "
        "served, cap[] and widget[] both EMPTY. FIVE forms, one per "
        "search key, all method GET: (1) id people-search-form class "
        "'search-form people', action https://www.peoplelooker.com/ -- "
        "input text name=fn (First Name, REQUIRED), input text name=ln "
        "(Last Name, REQUIRED), submit BUTTON text Search; (2) id phone- "
        "search-form, action /lp/b41af5/2/loading -- input tel name=phone "
        "required; (3) id email-search-form, action /lp/3843d9/2/loading "
        "-- input email name=emailaddress required; (4) id property- "
        "search-form, action /lp/37a7f6/2/loading -- input text "
        "name=address id=address-input required; (5) id money-search-form "
        "class 'search-form unclaimed-money', action /lp/59af95/2/loading "
        "-- name=fn, name=ln, SELECT name=state id=select-state (53 "
        "options). Forms 2-5 compute off-layout because they are tabbed "
        "behind the name form. Loose controls are OneTrust consent "
        "internals only (close-pc-btn-handler, accept-recommended-btn- "
        "handler and siblings). CONFIRMED LIVE, not assumed, that this is "
        "the same BeenVerified family build as ownerly-com and "
        "beenverified-com."
    ),
    "peoplesearch123-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. A real search surface exists and is "
        "transcribed below; it is undecided because NOT ONE OF ITS "
        "CONTROLS HAS A name ATTRIBUTE and none of them is inside a "
        "<form>. www.peoplesearch123.com renders HTTP 200 at a 14s "
        "settle, title 'PeopleSearch123', 5993 characters, cap[] and "
        "widget[] both EMPTY, and document.querySelectorAll('form') "
        "returns NOTHING -- every control is loose. Angular SPA (class "
        "suffixes ng-tns-c2126489738-0, ng-untouched ng-pristine): a "
        "full-page consent interstitial first (BUTTON class agree-btn "
        "text 'I AGREE' plus an invisible checkbox id=privacy-agree- "
        "checkbox), then the search fields addressable ONLY by id -- "
        "#firstName, #lastName and siblings, class landing-form-control. "
        "Page copy is the standard FCRA disclaimer: 'PeopleSearch123 does "
        "not provide consumer reports and is not a consumer reporting "
        "agency under the Fair Credit Reporting Act.' ENTITY FINDING: the "
        "two footer rights links both point at "
        "https://www.peoplesearch123.com/api/helper/optOutLight/search, "
        "and that endpoint serves a form BYTE-IDENTICAL in field list to "
        "the ones at peoplesearchusa.org and personsearchers.com (see "
        "OPTOUT_OUT_OF_SCOPE) -- which upgrades the dataset's 'shared "
        "operation is likely' to confirmed shared platform across all "
        "three."
    ),
    "peoplesearchnow-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        ...
        "2026-09-25, batch 33. RECIPE-READY IN SUBSTANCE for the name "
        "leg, filed undecided only because this tool cannot construct "
        "SearchRecipe objects and the results page was not driven. "
        "www.peoplesearchnow.com renders HTTP 200 at a 14s settle, title "
        "'Find People Fast, Look Up Phone Numbers - People Search Now', "
        "5501 characters, cap[] and widget[] both EMPTY, ZERO loose "
        "controls. The name form is class 'search-person clearfix', "
        "method POST, action https://www.peoplesearchnow.com/person- "
        "search, controls in order: input type=search name=name (label "
        "'First and Last Name', REQUIRED -- note ONE combined field, not "
        "first/last), input type=search name=city (City), SELECT "
        "name=state class placeholder with 52 options (first 'State'), a "
        "hidden off-layout input text name=searchType THAT MUST BE POSTED "
        "TOO, and two BUTTONs both named search-fio (one mobile, one "
        "desktop, the desktop one carrying the visible text 'Search'). "
        "Sibling forms on the same page: class 'search-tel' POST to "
        "/search (input tel name=phone required + searchType + buttons "
        "named search-tel) and class 'search-address' POST to /address- "
        "search (name=address, or name=addr1 + name=addr2 in the desktop "
        "variant, + searchType + buttons named search-address). Dataset "
        "context confirmed plausible but not proven on-page: row calls "
        "this the 'Mississippi Tornado Alley LLC engine'."
    ),
    "peoplesearchusa-org": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. A real, captcha-free, NAMED-FIELD search "
        "surface exists, fully transcribed, and it is filed undecided "
        "only because this tool cannot construct SearchRecipe objects and "
        "the funnel behind it was not driven. www.peoplesearchusa.org "
        "renders HTTP 200 at a 14s settle, title 'Peoplesearchusa', 5144 "
        "characters, cap[] and widget[] both EMPTY. ONE form, id email- "
        "form, class 'landing-search-form' (a Webflow landing shell "
        "hosting an Angular app), method GET, action "
        "https://www.peoplesearchusa.org/name/landing, four controls: "
        "input text name=First-Name id=First-Name (placeholder 'Enter "
        "person\u2019s First Name'), input text name=Last-Name id=Last- "
        "Name, SELECT name=field id=field with 53 options whose first "
        "entry is 'All States' (and which lists 'All States' a SECOND "
        "time between Alaska and Arizona -- a duplicated option in their "
        "own markup), and a submit BUTTON class 'landing-search-submit "
        "w-button' text 'FREE SEARCH'. Two loose controls: the same "
        "Angular consent interstitial as its siblings (BUTTON class "
        "agree-btn 'I AGREE' + invisible #privacy-agree-checkbox). Same "
        "FCRA disclaimer copy. Footer rights links point at "
        "/api/helper/optOutLight/search, identical to peoplesearch123.com "
        "and personsearchers.com -- see those rows."
    ),
    "peoplewin-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. peoplewin.com DOES serve its own live "
        "people-search -- the dataset's claim on that point checks out. "
        "RECIPE-READY IN SUBSTANCE, filed undecided only because this "
        "tool cannot construct SearchRecipe objects and the results "
        "funnel was not driven. HTTP 200 at a 14s settle, title 'People "
        "Search | White Pages | PeopleWin', 5892 characters, cap[] and "
        "widget[] both EMPTY. FOUR forms, all method GET, all action "
        "https://peoplewin.com/ (Tailwind build, no form ids -- address "
        "them by their input names): name form with input text "
        "name=fullName id=name-search-full-name (label Full Name) plus "
        "input text name=cityState id=name-search-city-state (City & "
        "State) and a submit BUTTON with NO text at all (icon only); "
        "email form with input text name=email id=email-search-field; "
        "phone form with input tel name=phone id=phone-search-field; "
        "address form with input text name=address id=address-search- "
        "field. NOTE the name key is ONE combined fullName field, not "
        "first/last. Loose controls are the Name/Phone/Email/Address tab "
        "switchers (BUTTONs with aria-label 'Open drop down menu' and 'Go "
        "back', no ids). Opt-out leg is delegated to Spokeo -- see "
        "OPTOUT_OUT_OF_SCOPE."
    ),
    "personsearchers-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py) "
        "2026-09-25, batch 33. A real search surface exists and is "
        "transcribed below; it is undecided for TWO specific reasons, "
        "both recorded so the next pass does not repeat them. "
        "www.personsearchers.com/ 302s to /name/landing, HTTP 200 at a "
        "16s settle, title 'Personsearchers', 5981 characters, main-frame "
        "cap[] and widget[] both EMPTY. ONE form, class 'search-form- "
        "wrapper' (Angular, NO id, method GET, action "
        "https://www.personsearchers.com/name/landing) whose three "
        "controls HAVE NO name ATTRIBUTE and are addressable only by id: "
        "#firstName (First name), #lastName (Last name), and SELECT "
        "name=state id=state with 52 options starting 'All States'. "
        "REASON ONE it is not a recipe: the submit control is not in the "
        "form at all -- it is a loose BUTTON id=perform-search class 'btn "
        "btn-primary' text 'FREE SEARCH', alongside the family's Angular "
        "consent interstitial (BUTTON class agree-btn 'I AGREE' + "
        "invisible #privacy-agree-checkbox) and a loose close button. "
        "REASON TWO, and the one the automated cap[] check MISSED on the "
        "main frame: a CHILD FRAME preloads Cloudflare Turnstile -- https "
        "://www.personsearchers.com/assets/common/captcha/turnstile.html? "
        "sitekey=0x4AAAAAAAEph5FrUwcvZCnw, carrying "
        "challenges.cloudflare.com/turnstile/v0/api.js?render=explicit "
        "and a hidden cf-turnstile-response input (id cf-chl-widget- "
        "yzqzu_response, per-load suffix). The search page itself renders "
        "fine, so this is not blocked; but a submission will have to "
        "satisfy Turnstile, which is why this cannot ship as a recipe. "
        "Same Angular platform and same FCRA disclaimer copy as "
        "peoplesearch123.com and peoplesearchusa.org."
    ),
    "privateeye-com": (
        "Verified by browser render 2026-09-25, batch 34. privateeye.com "
        "renders HTTP 200 at an 18s settle, title 'PrivateEye.com - Find "
        "People Free | People Search by Name, Phone & Email', 3020 "
        "characters, cap[] and widget[] both EMPTY, no captcha of any "
        "kind. A REAL name-keyed people search exists and is fully "
        "transcribed here, and it is NOT recorded as a recipe for one "
        "reason: not one of its inputs carries a name attribute, so no "
        "field can be addressed by a stable name. ONE form, no id, class "
        "'space-y-6', method GET, action https://privateeye.com/ -- six "
        "controls: input text id=firstName (label 'First Name', NO name "
        "attribute), input text id=lastName ('Last Name', NO name), input "
        "text id=city ('City (Optional)', NO name), BUTTON type=button "
        "id=state (label 'State (Optional)', visible text 'Select state') "
        "which is a Radix custom dropdown TRIGGER rather than a field, a "
        "SELECT with 50 options (Alabama..Wyoming) carrying NEITHER a "
        "name NOR an id, and a submit BUTTON with visible text 'Search'. "
        "Because the form is method=GET with no field names, submission "
        "cannot be a plain querystring build -- a React handler reads the "
        "values and routes. Outside the form: nine Radix accordion/tab "
        "buttons (Name Search / Reverse Phone Lookup / Background Report "
        "tabs, FAQ accordions) and a 'Start a Search' CTA. Site claims a "
        "'database of 120+ billion public records'. Left undecided rather "
        "than recipe because this repo cannot construct real SearchRecipe "
        "objects, and because the nameless fields plus the custom state "
        "dropdown would have to be driven by id and by click, not by "
        "name."
    ),
    "propertyrecord-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.propertyrecord.com renders HTTP 200 at an 18s settle, title "
        "'PropertyRecord.com - Public Property Records', 11363 "
        "characters, cap[] and widget[] both EMPTY on the marketing site. "
        "The search surface is ADDRESS-KEYED, NOT NAME-KEYED, and "
        "therefore does not fit this tool's SearchRecipe shape at all -- "
        "the same mismatch already recorded for ownerly-com. "
        "Transcription: the search control is NOT inside any form. Loose: "
        "input type=search name=search-input id=address-search-input "
        "(label 'Enter an address') plus BUTTON type=button id=address- "
        "search-btn (visible text 'FIND PROPERTY RECORDS'), with a "
        "duplicate FIND PROPERTY RECORDS button in the hero. Also loose: "
        "a 'YOUR PRIVACY CHOICES' trigger button opening a dialog holding "
        "checkbox id=privacy-toggle-dnss (label 'Do Not Sell or Share My "
        "Personal Information', class 'sr-only checkbox--toggle') and "
        "BUTTON id=privacy-choices-save-btn -- a bare click-only JS "
        "toggle, OPEN POLICY QUESTION 2. Two real forms exist on the page "
        "but neither is the search: id=contactForm (contact-name / "
        "contact-email / contact-message) and id=privacy-request-form "
        "(see the opt-out entry). Page text carries the usual non-FCRA "
        "disclaimer in full caps. ENTITY COLLISION: this site and "
        "propertyrecs.com are the same operator -- see the opt-out "
        "entries for the byte-identical opt-out dashboards and shared "
        "reCAPTCHA Enterprise sitekey."
    ),
    "propertyrecs-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.propertyrecs.com renders HTTP 200 at an 18s settle, title "
        "'Property Record Search by Address | PropertyRecs.com', 11178 "
        "characters, cap[] and widget[] both EMPTY. ADDRESS-KEYED search, "
        "not name-keyed -- does not fit the SearchRecipe shape, same "
        "mismatch as ownerly-com and propertyrecord-com. Transcription, "
        "identical in structure to propertyrecord.com down to the class "
        "strings: search control outside any form, input type=search "
        "name=search-input id=address-search-input (label 'Enter an "
        "address') plus BUTTON type=button id=address-search-btn (visible "
        "text 'SEARCH PROPERTY RECORDS'). Same id=contactForm (contact- "
        "name / contact-email / contact-message) and same id=privacy- "
        "request-form component, same 'YOUR PRIVACY CHOICES' dialog with "
        "checkbox id=privacy-toggle-dnss and BUTTON id=privacy-choices- "
        "save-btn (OPEN POLICY QUESTION 2), same all-caps non-FCRA "
        "disclaimer, same LiveChat widget in an about:blank frame. Header "
        "reads 'PROPERTY RECORDS / POWERED BY: PROPERTYRECS'. ENTITY "
        "COLLISION with propertyrecord-com -- one operator, two brands; "
        "see the opt-out entries."
    ),
    "propertyshark-com": (
        "Verified by browser render 2026-09-25, batch 34. "
        "www.propertyshark.com redirects to /mason/ (region-scoped "
        "landing, HTTP 200) at an 18s settle, title 'PropertyShark - "
        "Property Data and Research Tools', 10644 characters, cap[] and "
        "widget[] both EMPTY. ADDRESS-KEYED search, not name-keyed, so it "
        "does not fit the SearchRecipe shape. ONE form: id=search_form, "
        "class 'is_tabbed anon-search-form', method POST, action "
        "https://www.propertyshark.com/mason/# -- two controls: input "
        "text name=search_token id=search_token_address (label 'Address, "
        "neighborhood, city, or ZIP code') and input submit "
        "name=submit_button id=newshark_anon_submit_button value "
        "'Search'. A single combined location field, so not even "
        "street/city/state separable. Loose controls are navigation only "
        "(Property data / Tools / Distressed properties / Resources "
        "dropdown triggers, a region picker defaulting to New York City, "
        "testimonial and blog carousels, five FAQ accordions, OneTrust "
        "'Cookies Settings' / 'Allow All'). The page offers 'one free "
        "property report' to anonymous visitors and gates the rest behind "
        "Sign up / Sign in, and its 'Ownership research' tool is login- "
        "only. PropertyShark is a Yardi Systems property -- the privacy "
        "statement lives at resources.yardi.com and the request form at "
        "yardi.com (see the opt-out entry)."
    ),
    "reunion-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 18s "
        "settle) 2026-09-25, batch 35. Live, free, unwalled people-search "
        "-- held undecided because its shape does not fit SearchRecipe "
        "cleanly. reunion.com renders as 'Free People Finder Online & "
        "People Search - Reunion.com' with cap[] and widget[] both EMPTY "
        "at an 18s settle. There is NO <form> element: the four controls "
        "are all LOOSE and NONE carries a name or an id -- input text "
        "placeholder 'Search', input text placeholder 'Name', input text "
        "placeholder 'City, State', and a submit button reading 'SEARCH' "
        "(plus the cookie 'Accept' button). Two shape problems to resolve "
        "before this can be a recipe: (a) every field is addressable only "
        "by placeholder, and (b) the surface is a COMBINED single 'Name' "
        "box plus a single combined 'City, State' box, which is the non- "
        "recipe-shaped pattern already flagged elsewhere in this dataset "
        "-- resolve_fields has no combined city-state source. Whether "
        "submission produces an addressable results URL could not be "
        "determined without submitting, which a reconnaissance pass must "
        "not do. Entity note: the footer identifies this as a MyLife "
        "brand and the page copyright reads 'All Rights Reserved "
        "2021-2022 - Reunion.com'."
    ),
    "kidslivesafe-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. kidslivesafe.com (Scalable "
        "Commerce, LLC) IS a live people-search -- held undecided because "
        "of shape and a paywall, not a wall. The homepage (7522 "
        "characters) renders FOUR separate single-field GET forms, all "
        "posting to /?: (1) input[type=search] name=search-name "
        "id=search-name with button 'Search Now'; (2) name=search-address "
        "id=search-address, button id=btn-search; (3) input[type=tel] "
        "name=search-phone-number id=search-number, button id=phone- "
        "search-btn; (4) name=search-neighborhood id=search-neighborhood, "
        "button id=btn-neighborhood-search. cap[] and widget[] are EMPTY "
        "on the homepage (contrast the opt-out leg, which is Turnstile- "
        "walled). TWO reasons this is not a SearchRecipe: every one of "
        "the four is a SINGLE COMBINED FREE-TEXT FIELD (one box for a "
        "whole name, one for a whole address), which is the recurring "
        "non-recipe-shaped surface, and two of the four are not name- "
        "keyed at all (phone-keyed and neighborhood-keyed). Results are "
        "also a subscription product (the site sells a paid child- "
        "safety/people-lookup membership), so a query almost certainly "
        "lands on a paywall rather than a record. Needs a shape decision "
        "from Penn before it could be driven."
    ),
    "socialcatfish-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 15s "
        "settle) 2026-09-25, batch 36. socialcatfish.com (SOCIAL CATFISH "
        "LLC) runs a live, free-to-query people search -- held undecided "
        "for shape and a paywall, not a wall. Rendered controls: a header "
        "name box input id=searchInput2 placeholder 'Enter full name..' "
        "(single combined name field) with a mobile twin "
        "id=mobileSearchInput, an IMAGE search input[type=file] "
        "name=image[] id=searchInputImage, state/city/country selects "
        "id=select_state/select_city/select_country, and a stepper of "
        "buttons id=nextBtn 'Continue' / id=skipBtn 'Skip This' / "
        "id=backBtn. There is also an address refine form id=refine- "
        "address-search-form (name=address_1, state, city, zip, submit "
        "id=refine-address-search-now) and a reverse-address form posting "
        "to /reverse-address-check/?signup=1 whose ONLY field is a "
        "required email with the button 'Unlock Results Now'. That last "
        "is the paywall tell: results are gated behind signup/payment. "
        "Three reasons it is not a clean SearchRecipe: single combined "
        "name field, a multi-step wizard rather than one POST, and "
        "results behind an email/pay unlock. Cloudflare Turnstile is "
        "loaded on this host (see the opt-out entry) though it is "
        "attached to the opt-out form, not to the search."
    ),
    "spydialer-com": (
        "NO VERDICT as of 2026-09-25, batch 37, and the surface is real, "
        "free and prominent -- it is the SHAPE that is unresolved. "
        "Verified by browser render at 15s settle. spydialer.com is a "
        "reverse-phone-lookup site (operator per the footer: Spy Labs "
        "Adco, LLC -- the dataset's 'Spy Dialer, Inc.' is a stale entity "
        "name) offering four tabbed searches inside ONE ASP.NET WebForms "
        "postback form (id aspnetForm, method post, __VIEWSTATE + "
        "__VIEWSTATEGENERATOR + __EVENTVALIDATION + __EVENTTARGET): PHONE "
        "(default, input ctl00$ContentPlaceHolder1$searchinput, type tel, "
        "placeholder 123-456-7890), PEOPLE (FirstNameTextBox / "
        "LastNameTextBox / NameCityTextBox / DrpDwnLstNmeStt with 53 "
        "options including NATIONWIDE), ADDRESS (TxtBxddrss1 / "
        "TxtBxAddrssCty / DrpDwnLstddrssStt) and EMAIL. Two things block "
        "a recipe. First, the non-default tabs are all off-layout until "
        "their tab is activated, so a driver has to click through to "
        "reach the name search. Second, and the reason this is undecided "
        "rather than a transcription task: EVERY field is DUPLICATED as a "
        "decoy pair and one member of each pair has an obfuscated name -- "
        "ctl00$ContentPlaceHolder1$atXsACbz sits beside the real "
        "searchinput, TxtBxFrstNm beside FirstNameTextBox, TxtBxLstNm "
        "beside LastNameTextBox, TxtBxNmCty beside NameCityTextBox, "
        "TxtBxddrss beside TxtBxddrss1, and three separate Search submits "
        "(IgBtnSch, SearchButton, nClck_NmSrchBttn, "
        "PeopleAddressSearchButton) exist for the same visual button. "
        "Which member of each pair the server honours cannot be "
        "determined without submitting, which recon does not do. Anti- "
        "automation by deliberate name obfuscation and decoy duplication, "
        "and the same site's opt-out wizard uses the character-REVERSED "
        "variant of the same trick (see optout_forms.OPTOUT_BLOCKED)."
    ),
    "sync-me": (
        "NO VERDICT as of 2026-09-25, batch 37, and the reason is SHAPE, "
        "not access. Verified by browser render at 15s settle: sync.me is "
        "the web front end of the Sync.me caller-ID / reverse-phone- "
        "lookup app (operator per the site: Sync.me Technologies Ltd., "
        "Ramat Gan, Israel). The homepage carries a live, captcha-free, "
        "login-free search -- but it is PHONE-KEYED ONLY, and the key is "
        "the only field: one input name=nationalNumber, type=tel, "
        "labelled 'Search any phone number', plus a custom country-code "
        "selector rendered as a <button> reading '+1' (not a <select>, so "
        "a driver must operate the custom control to change country) and "
        "an unnamed submit. There is no name field, no city, no state, no "
        "email. A name-keyed SearchRecipe cannot express this, so "
        "flagging rather than force-fitting: this is the phone-keyed-only "
        "non-recipe shape, and the question of whether the search-recipe "
        "abstraction should grow a phone-keyed variant is the one that "
        "has to be answered before this row can close."
    ),
    "signalhire-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 12-18s "
        "settle) 2026-09-26, batch 38. RECIPE-READY IN SUBSTANCE, filed "
        "here because this module cannot construct a real SearchRecipe "
        "object (confirmed by direct testing across batches), and because "
        "of one JS-only control. SignalHire sells business contact data "
        "and indexes person profiles as public SEO pages at "
        "/profiles/<slug>/<id>; www.signalhire.com renders 'Find Email "
        "Addresses & Phone Numbers by Name' (8874 characters) with cap[] "
        "and widget[] both EMPTY -- no reCAPTCHA, no Turnstile, no hidden "
        "captcha fields anywhere. The search form is method GET, action "
        "/, and transcribes as: input[type=text][name=q] (placeholder "
        "'Type company name here'), a hidden input[name=mode], and a "
        "submit button.btn--primary labelled 'Search'. THE BLOCKER A "
        "RECIPE MUST HANDLE, and the reason this is undecided rather than "
        "a recipe: the People-vs-Companies choice is TWO OFF-LAYOUT "
        "BUTTONs reading 'Companies' and 'People' which set the hidden "
        "[name=mode] in JavaScript, so a person search cannot be "
        "expressed by filling named fields alone -- the mode button has "
        "to be clicked, or [name=mode] set directly, and which value "
        "means 'people' was NOT established (nothing was submitted). "
        "Everything else about it is clean and unwalled."
    ),
    "smartbackgroundchecks-com": (
        ...
        "settle) 2026-09-26, batch 38. RECIPE-READY IN SUBSTANCE, filed "
        "here because this module cannot construct a real SearchRecipe "
        "object (confirmed by direct testing across batches). "
        "www.smartbackgroundchecks.com renders 'Free Name, Address & "
        "Reverse Phone Search' (13806 characters) with cap[] and widget[] "
        "both EMPTY -- no captcha at all on the search page, which is "
        "notable given that its own opt-out page is reCAPTCHA-walled. "
        "Transcription: form#searchForm, method POST, action /search.php; "
        "input[type=text][name=inputFirstName][id=inputFirstName] 'First "
        "Name' (class carries basicAutoComplete); [name=inputMiddleName] "
        "'Middle Name'; [name=inputLastName][id=inputLastName] 'Last "
        "Name' (basicAutoComplete); [name=inputCityState] 'City, State "
        "(Optional)' (basicAutoComplete); plus two off-layout controls "
        "belonging to the other tabs, [name=inputStreet] 'Street Address' "
        "and input[type=tel][name=inputPhone]; three hidden inputs "
        "[name=searchByType], [name=searchManually] and "
        "[name=searchLanguage]; submit button.btn-primary reading "
        "'Search'. THE ONE THING A RECIPE MUST HANDLE: none of the fields "
        "is marked required and the three hidden inputs select which "
    ...
    "usrecords-net": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. SURFACE CONFIRMED, CAPTCHA-FREE, "
        "AND NOT URL-ADDRESSABLE -- checked both ways rather than "
        "assumed. https://www.usrecords.net/ answers HTTP 200 ('Easily "
        "Find Public Records in the U.S. | US Records') and carries "
        "form#records2form, method GET, action "
        "https://www.usrecords.net/results, with input[name=fname] "
        "(required), [lname] (required), [middle_name], [city], "
        "select[name=state] (52 options, two-letter codes plus "
        "'Nationwide') and four off-layout hidden fields (stage, rType, "
        "rState, rCounty), under a plain submit. cap[] and widget[] are "
        "EMPTY: no captcha script and no captcha element on the search "
        "page. A GET method with named parameters looks addressable, so "
        "it was tested: requesting "
        "/results?fname=Jonathan&lname=Quiverstone&... with the hidden "
        "fields empty landed back on the HOMEPAGE (final URL "
        "https://www.usrecords.net/ with the same #records2form), so the "
        "four hidden fields, or a session the form sets, are load-bearing "
        "and the results page is NOT reachable by URL alone. Held "
        "undecided rather than called blocked or dead: the next step is a "
        "form-driven run that fills and submits in a browser and records "
        "what /results actually serves, including whether results sit "
        "behind a paywall. No both-ways discrimination could be measured "
        "without that."
    ),
    "ussearch-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. ZERO FORM ELEMENTS AT REST, which "
        "on this network is a known shape rather than a missing product. "
        "https://www.ussearch.com/ answers HTTP 200 ('People Search & "
        "Background Check Information | US Search') and the rendered DOM "
        "at a 14s settle contains NO <form> and NO input of any kind -- "
        "the page sells 'Access billions of public records' behind a GET "
        "STARTED call to action and a Login link, and its only rights- "
        "adjacent anchor is a 'Do Not Sell or Share My Personal "
        "Information' link whose href is bare '#', i.e. JS-handled. cap[] "
        "and widget[] are EMPTY, so this is not a bot wall: it is a "
        "client-side app that builds its search box after the fact. US "
        "Search is a PeopleConnect brand sharing one database with "
        "Intelius, TruthFinder, Instant Checkmate, PeopleSmart, "
        "ZabaSearch and Addresses.com, and both sibling rows already "
        "recorded here (intelius-com, truthfinder-com) resolve their "
        "search legs to undecided because /search/ serves a multi-screen "
        "engagement wizard rather than an answer. Held undecided on the "
        "same footing, with the extra caveat that no results URL was even "
        "observed for this brand."
    ),
    "usa-people-search-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. SURFACE CONFIRMED, CAPTCHA-FREE, "
        "NOT URL-ADDRESSABLE. https://www.usa-people-search.com/ answers "
        "HTTP 200 and carries three sibling forms, all POSTing to "
        "/Results/Search and all carrying a Laravel hidden _token minted "
        "per page load: form#PeopleForm (input[name=Name]#FullName 'Full "
        "Name', input[name=Location] 'City, State'), "
        "form#ReversePhoneForm (input[name=Phone]#PhoneNumber) and "
        "form#AddressForm (AddressLine1, City, State). cap[] and widget[] "
        "are EMPTY on the search page. The per-load CSRF token plus POST "
        "means this can only ever be driven in a browser, never as a "
        "canned request or a URL, so no both-ways discrimination could be "
        "measured this pass. ONE DETAIL WORTH CARRYING FORWARD: "
        "#PeopleForm also holds a hidden input[name=IsOptOut]#IsOptOut, "
        "so the SAME search form doubles as the entry point to the "
        "removal flow -- which is consistent with the opt-out leg's "
        "finding that removal begins by identifying your own record. Held "
        "undecided pending a form-driven run."
    ),
    "uswarrants-org": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. SURFACE CONFIRMED AND NOT URL- "
        "ADDRESSABLE. The rights page itself carries the site's warrant "
        "search, form#recordsForm, method POST, action "
        "https://www.uswarrants.org/search, with input[name=fname], "
        "[lname], select[name=stateSel] (51 options, 'All States' first), "
        "[city] and input#submitBut -- under the line 'Your search "
        "remains confidential. The search subject will not be notified.' "
        "A POST with no addressable results URL, so no both-ways "
        "discrimination could be measured and nothing was submitted. Note "
        "that the SEARCH form carries no captcha of its own; the "
        "Cloudflare Turnstile on this page is bound to the separate "
        "WPForms rights form (see the opt-out leg), which is exactly the "
        "kind of distinction that must not be collapsed -- a Turnstile "
        "script in cap[] does not mean the search is walled. Held "
        "undecided pending a form-driven run that records what /search "
        "serves and whether results sit behind the site's LOGIN."
    ),
    "verifyrecords-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. SURFACE CONFIRMED, CAPTCHA-FREE, "
        "AND PROVED NOT URL-ADDRESSABLE. http://verifyrecords.com/ "
        "(served over plain HTTP, as the dataset notes) answers 200 with "
        "the title 'Background Records | VerifyRecords.com' and carries "
        "form#form1.form-horizontal, method GET, action "
        "https://verifyrecords.com/reg7/loader.php, with "
        "input[name=firstname], input[name=lastname], a 52-option "
        "select[name=state]#exampleSelect1, a hidden "
        "input[name=page_name] and button#hdnSearchForm 'Search Now'. "
        "cap[] and widget[] are EMPTY. Because that is a GET with named "
        "parameters it was tested directly: /reg7/loader.php?firstname=Jo "
        "nathan&lastname=Quiverstone&state=Nevada&page_name=home answered "
        "HTTP 200 and served THE HOMEPAGE BACK, the same empty #form1, "
        "with no result content -- so the parameters are discarded "
        "without a session and the results page is NOT reachable by URL. "
        "That is a finding, not a failure: it rules out the cheap URL- "
        "addressable recipe shape. 'reg7' in the path is a hint that the "
        "results route is a registration funnel, and the site carries a "
        "Login link, so a form-driven run should expect a paywall. Held "
        "undecided pending that run; no both-ways discrimination could be "
        "measured."
    ),
    "veripages-com": (
        "Verified by browser render (Playwright, throwaway long-settle "
        "wrapper importing JS/UA from tools/probe_broker_forms.py, 14s "
        "settle) 2026-09-25, batch 40. The search surface exists and was "
        "seen, but only incidentally and not resolved. Requesting "
        "https://veripages.com/optout (see the opt-out leg) served the "
        "site's own 404 page, HTTP 404 titled 'Page 404', and THAT page "
        "rendered normally with the site chrome intact: three search tabs "
        "(People / Phone / Address, anchors #serchPeople, #serchPhone, "
        "#serchAddr -- the site's own spelling), three <form> elements, a "
        "full A-Z 'People Directory', a LOG IN link and cap[] and "
        "widget[] EMPTY. So the search is not bot-walled at the front "
        "door, which is worth recording precisely because the site's "
        "REMOVAL page is walled. What is still unknown is whether a name "
        "search is URL-addressable and what it serves: the dataset's own "
        "note says 'name search not URL-addressable', and nothing this "
        "pass saw contradicts or confirms that. VeriPages is a Radaris "
        "affiliate, and the Radaris family's rows are a mix of seized "
        "domains and paywalled result pages, so a form-driven run should "
        "expect one of those two outcomes. Held undecided."
    ),
}
