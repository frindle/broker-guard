...
OPTOUT_UNDECIDED = {
    ...
    "hireright-com": (
        ...
        "homepage, but the real trail is clear and two-branched, and it "
        "ends at a page whose form could not be read. The footer offers "
        "BOTH 'Do Not Sell or Share My Personal Information' (/legal/do- "
        "not-sell-my-personal-information, California) and 'United States "
        "Opt-Out Rights (Outside of California)' (/legal/united-states- "
        "opt-out-rights-outside-of-california); each is a notice page "
        "with no inputs, and each says 'click here to fill out the "
        "webform', both pointing at the SAME target: "
        "http://www.hireright.com/u-s-state-consumer-privacy-rights- "
        "request-form. That page renders, is titled 'U.S. State Consumer "
        "Privacy Rights Request Form', and opens with 'IMPORTANT - READ "
        "BEFORE SUBMITTING' -- but the browser found no form elements on "
        "it, so the form is almost certainly inside an iframe (this probe "
        "does not descend into frames) or is lazily built. That is the "
        "one thing left to establish. Read its preamble before writing "
        "anything: HireRight argues in its own words that 'our background "
        "screening business falls within those exceptions, and the state "
        "consumer privacy laws do not apply', and that outside California "
        "they do not apply in an employment context -- so a recipe here "
        "may be filing a request the broker considers inapplicable, and "
        "the entry should say so. Dataset contact: dpo@hireright.com."
    ),
    "clay-com": (
